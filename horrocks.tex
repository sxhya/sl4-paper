...
\def\received{October 2, 2019}\def\revised{June 11, 2020}
\def\communicated{Max Karoubi}
%
...
\hypersetup{colorlinks=true,citecolor=NavyBlue,linkcolor=DarkPurple,urlcolor=NavyBlue}
\usepackage{color} 
...
\def\LOCAL{\horrocks.files}
%\renewcommand*{\bibfont}{\footnotesize}
%\defbibheading{bibliography}[\refname]{\section*{#1} \markboth{}{}}
...
\long\def\MSC#1\EndMSC{\def\arg{#1}\ifx\arg\empty\relax\else
     {\par\narrower\noindent%
     2020 Mathematics Subject Classification: #1\par}\fi}
...
\Address #4
\EndAddress
{\def\xs{4.8truecm}\parindent=0pt
...
\EndTitle
\Abstract 
We prove the Horrocks theorem for unstable even-dimensional orthogonal Steinberg groups. The Horrocks theorem for Steinberg groups is one of the principal ingredients needed for the proof of the $\K_2$-analogue of Serre's problem, positive solution of which currently is only known in the linear case.
\EndAbstract
\MSC 
19C20
\EndMSC
...
\begin{equation*}\xymatrix{
 \KO_3(2\ell, B) \ar[r] \ar[d] & \KO_3(2\ell, k[X^{-1}]) \ar[d]_{f'} \ar@{->>}[r] & \ar[d]_{f} C(\rD_\ell, B, I) \\
...
\begin{equation*} \xymatrix{
 ...
 \St(\rD_\ell, B) \ar[r]^{\nu_B} \ar[d]_{h} & \St(\rD_\ell, k[X^{-1}]) \ar[d]_{i} \\
...
The elements $c_\alpha(s, t)$ satisfy the following relations for all $s\in I,\ t\in J,\ \xi\in R$:
 ...
 \item \label{C4} If $\alpha+\beta\in\Phi$ then
  \begin{equation*} c_{\alpha+\beta}(s, t\xi) = [x_{\beta}(st),\ x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(\epsilon t)} \cdot c_{\alpha}(\epsilon s\xi, -\epsilon t)^{-1} \cdot x_{-\beta}(-st\xi^2),\end{equation*}
  where $\epsilon = N_{\alpha,\beta}$.
 \end{enumerate}
\end{lemma}
\begin{proof}
...
\newline \scalebox{.88}{\begin{minipage}{1.13\linewidth} \begin{align*} \nonumber [c_{\beta}(s, t),\ x_{\alpha}(\xi)] = [x_{\beta}(s),\ x_{-\beta}(t)] \cdot [x_{-\beta}(t),\ x_{\beta}(s)\cdot x_{\alpha + \beta}(N_{\alpha, \beta} \cdot s\xi)] &\text{ by~\eqref{Cdef}, \eqref{Chevalley-CCF1}, \eqref{Chevalley-CCF2}} \\
 = {}^{x_\beta(s)}\![x_{-\beta}(t),\ x_{\alpha+\beta}(N_{\alpha, \beta} \cdot s\xi)] &\text{ by~\eqref{eq:H1ii-2}} \\ = x_{\alpha}(- st\xi) \cdot x_{\alpha+\beta}(N_{\alpha,\beta} \cdot s^2t\xi) &\text{ by~\eqref{Chevalley-CCF1}.} \end{align*} \end{minipage}} \vskip \belowdisplayshortskip
 Now suppose that $\alpha - \beta \in \Phi$. We have that
\newline \scalebox{.86}{\begin{minipage}{1.16\linewidth}\begin{align*} \nonumber
...
= x_{\alpha}(st\xi) \cdot x_{\alpha-\beta}(-N_{\alpha,-\beta}\cdot st^2\xi)^{x_\beta(-s)} = x_{\alpha}(st\xi + s^2t^2 \xi) \cdot x_{\alpha-\beta}(N_{-\alpha,\beta}\cdot st^2\xi) & \text{ by~\eqref{Chevalley-CCF1}.} \end{align*} \end{minipage}} \vskip \belowdisplayshortskip
Finally, let us prove~\ref{C4}. Suppose that $\alpha + \beta \in \Phi$. The required identity can be obtained as follows:
\newline \scalebox{.84}{\begin{minipage}{1.17\linewidth}\begin{align*}
...
= [x_{\beta}(st),\ x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(\epsilon t)} \cdot c_{\alpha}(\epsilon s\xi, -\epsilon t)^{-1} \cdot x_{-\beta}(-st\xi^2)  &\ \text{by~\eqref{Chevalley-CCF1}}. \qedhere \end{align*} \end{minipage}} \vskip \belowdisplayshortskip
\end{proof}
...
\newline\scalebox{0.96}{\begin{minipage}{1.04\linewidth}
 \[z_{\alpha_i}(f_i(0), \xi_i(0))^{x_{-\alpha_i}(X\xi'_i(X))} = [x_{-\alpha_i}(-X\xi'_i(X)),\ z_{\alpha_i}(f_i(0), \xi_i(0))] \cdot z_{\alpha_i}(f_i(0), \xi_i(0)).\]\end{minipage}}\vskip \belowdisplayshortskip
...
\newline\scalebox{0.94}{\begin{minipage}{1.06\linewidth}
\[ [x_\alpha(a),\ x_\alpha(b)] = [x_\alpha(a),\ [x_{\alpha+\beta}(b),\ x_{-\beta}(\epsilon)]] = [[x_\alpha(a),\ x_{\alpha+\beta}(b)],\ {}^{x_{\alpha+\beta}(b)}\!x_{-\beta}(\epsilon)] = 1. \]\end{minipage}}
\end{rem}
...
\begin{align*} [x_\alpha(at^d), x_{\gamma}(bt^{e})] = \hspace{4cm} \\ = [x_\alpha(at^d), [x_{\beta+\gamma}(b t^{e-1}), x_{-\beta}(-\epsilon_1 t)]] & \text{ by~$\text{R2}_{e-1,1}$ } \\
 ...
 = 1 & \text{ by $\text{R3}^\bot_{1,d+e-1}$,} \end{align*}\end{minipage}}\vskip \belowdisplayshortskip
...
\newline\scalebox{0.85}{\begin{minipage}{1.15\linewidth}
 \begin{align} 
...
\newline\scalebox{0.85}{\begin{minipage}{1.15\linewidth}\begin{align*}
 ...
 = x_{-\alpha}\left(\tfrac{mX}{1+am}\right) \cdot \langle a, m\rangle_\alpha \cdot h_\alpha(1+am) &\text{ by~\eqref{eq:symbol-properties}}. \qedhere\end{align*}\end{minipage}}  \end{proof}
...
\end{align*}\end{minipage}} \vskip \belowdisplayshortskip
...
\newline\scalebox{.8}{\begin{minipage}{1.25\linewidth}\begin{align*}
...
\newline \scalebox{0.75}{\begin{minipage}{1.33\linewidth}
\begin{align*}
...
\newline \scalebox{0.80}{\begin{minipage}{1.15\linewidth}
\[\xymatrix{ G_M^0 \ar@{^{(}->}[rr]^{i_+^M} \ar@<-5pt>@{^{(}->}[dd]^{i_-^M} \ar@<-5pt>@{^{(}->}[rd] & & \overline{\St}(\Phi, A[X], M[X]) \ar[dd]_(.3){j_+^M} \ar@<-2.5pt>@{^{(}->}[rd] & \\ & \St(\Phi, A) \ar@{^{(}->}[rr]^(.3){i_+} \ar@<-2.5pt>@{^{(}->}[dd]^(.3){i_-} & & \St(\Phi, A[X]) \ar[dd]^{j_+} \\ \overline{\St}(\Phi, A[X\inv], M[X\inv]) \ar@<-2.5pt>@{^{(}->}[rd] \ar[rr]^(.35){j_-^M} & & \overline{\St}(\Phi, R, I) \ar@<-2.5pt>@{^{(}->}[rd] & \\ & \St(\Phi, A[X\inv]) \ar[rr]_{j_-} & & \St(\Phi, R).} \]\end{minipage}} \vskip \belowdisplayshortskip
...
\begin{proof}
 ...
 \newline \scalebox{0.95}{\begin{minipage}{1.05\linewidth}\begin{multline} \nonumber
 ...
 \end{multline}\end{minipage}}\end{proof}
...
\begin{proof}
 ...
 \begin{multline}\label{add-computation}
 ...
 \newline\scalebox{0.85}{\begin{minipage}{1.15\linewidth}\begin{equation}\label{add-computation2}
  \ldots = \left(x_\alpha((a + b)X^{-1})\cdot g ,\ \left[ h'' \cdot x_\alpha\left(\tfrac{-bX^{-1}}{1+\tfrac{bm}{1+am}}\right) \right],\ u \cdot (1 + am) \cdot \left(1+\tfrac{bm}{1+am}\right)\right).\end{equation}\end{minipage}}\vskip \belowdisplayshortskip
 Notice that 
...
\newline\scalebox{0.84}{\begin{minipage}{1.16\linewidth}\begin{equation} \label{R3leq0-computation2} \ldots = \left( [x_\beta(b), x_{\alpha}(aX^{-1})] \cdot g, [h'' \cdot x_{\alpha}\left(-a(1-am)X^{-1}\right) \cdot x_{\beta}(-b_0)], u \right) = \left(g, [h], u\right), \end{equation}\end{minipage}}\vskip \belowdisplayshortskip
...
\newline\scalebox{0.85}{\begin{minipage}{1.15\linewidth}\[ p\left(x_\alpha(aX^{-1})\cdot g, h', u(1 + am)\right) = S_\alpha(a, p(g, h, u)) \cdot h^{-1}_\alpha(1 + am) \cdot x_{\alpha-\beta}(-\epsilon am') \in P_\beta(\tfrac{m'}{1+am}), \]\end{minipage}}\vskip\belowdisplayshortskip
therefore we can invoke~\eqref{T_1} once again and can continue~\eqref{R3_1_1-computation} as follows:
\newline\scalebox{0.88}{\begin{minipage}{1.13\linewidth}
\begin{equation}\label{R3_1_1-computation2} \ldots = \left(x_\beta(bX^{-1}) \cdot x_\alpha(aX^{-1}) \cdot g,\ \left[h' \cdot x_\beta\left(-\tfrac{bX^{-1}}{1 + \tfrac{bm'}{1+am}}\right)\right],\ u(1 + am + bm')\right). \end{equation}\end{minipage}}\vskip \belowdisplayshortskip
...
\end{multline*}\end{minipage}}\vskip \belowdisplayshortskip
...
\newline\scalebox{0.9}{\begin{minipage}{1.1\linewidth}\begin{multline} \label{R2_0_1-computation}
...
\newline\scalebox{0.8}{\begin{minipage}{1.2\linewidth}\begin{multline} \nonumber
 ...
 = \left[ h\cdot x_\alpha\left(\tfrac{a X^{-1}}{1-am}\right) \cdot x_{\alpha + \beta}\left(\tfrac{\epsilon \cdot abX^{-1}}{c}\right)\cdot x_\alpha\left(-\tfrac{aX^{-1}}{1 - am}\right) \cdot h_{\alpha}^{-1}(1-am)\cdot h_{\beta}(c^{-1}) \right]   = \\ = \left[ h\cdot x_{\alpha + \beta}\left(\tfrac{\epsilon   abX^{-1}}{1 - \epsilon a b m'}\right) \right].\end{multline}\end{minipage}}\vskip \belowdisplayshortskip
...
\bibitem{Abe83}
 E.~Abe, 
 \emph{\href{https://dx.doi.org/10.1080/00927878308822906} 
 {Whitehead groups of {C}hevalley groups over polynomial rings}},
 Comm. Alg. \textbf{11} (1983), no.~12, 1271--1307.

\bibitem{A13}
 D.~Allcock, 
 \emph{\href{https://dx.doi.org/10.2140/ant.2016.10.1791} 
 {Steinberg groups as amalgams}},
 Alg. Number Th. \textbf{10} (2016), no.~8, 1791--1843.

\bibitem{AF17}
 A.~Asok and J.~Fasel, 
 \emph{\href{https://dx.doi.org/10.1112/topo.12007} 
 {An explicit KO-degree map and applications}},
 J. Top. \textbf{10} (2017), 268--300.

\bibitem{Ba74}
 H.~Bass, 
 \emph{\href{https://dx.doi.org/10.2307/2373586} 
...
\bibitem{DS73}
 ...
 \emph{\href{https://dx.doi.org/10.1007/BFb0073728} 
 {The functor $\K_2$: A survey of computations and problems}},
 Classical Algebraic K-Theory, and Connections with Arithmetic, Springer, 1973, pp.~241--303.

\bibitem{FRS12}
 J.~Fasel, R.~Rao, and R.~Swan,
 \emph{\href{https://dx.doi.org/10.1007/s10240-012-0041-y} 
 {On stably free modules over affine algebras}},
 Publ. Math. IH{\'E}S \textbf{116} (2012), 223--243.

\bibitem{Ho05}
 J.~Hornbostel, 
 \emph{\href{https://dx.doi.org/10.1016/j.top.2004.10.004} 
 {$\mathbb{A}^1$-representability of hermitian K-theory and Witt groups}},
 J. Top. \textbf{44} (2005), no.~3, 661--687.

\bibitem{Ho64}
 G.~Horrocks, 
 \emph{\href{https://dx.doi.org/10.1112/plms/s3-14.4.714} 
 {Projective modules over an extension of a local ring}},
 Proc. London Math. Soc. \textbf{3} (1964), no.~4, 714--718.

\bibitem{Hur77}
 J.~Hurrelbrink, 
 \emph{\href{https://dx.doi.org/10.1007/BF01351716} 
 {Endlich pr\"asentierte arithmetische gruppen und $K_{2}$ {\"u}ber Laurent-polynomringen}},
 Math. Ann. \textbf{225} (1977), no.~2, 123--129.

\bibitem{Ka80}
 M.~Karoubi, 
 \emph{\href{https://dx.doi.org/10.2307/1971147} 
 {Le th{\'e}oreme fondamental de la K-th{\'e}orie hermitienne}},
...
\bibitem{La18}
 A.~Lavrenov, 
 \emph{\href{https://dx.doi.org/10.25537/dm.2018v23.653-675}
 {A local-global principle for symplectic $\K_2$}},
 Doc. Math. \textbf{23} (2018), 653--675.

\bibitem{LS17}
 A.~Lavrenov and S.~Sinchuk,
 \emph{\href{https://arxiv.org/pdf/1606.00272.pdf} 
 {Centrality of odd orthogonal $\K_2$}},
 J. Pure Appl. Alg. \textbf{221} (2017), 1134--1145.

\bibitem{Lo78}
 J.-L.~Loday,
 \emph{\href{https://dx.doi.org/10.1016/0021-8693(78)90025-X}
 {Cohomologie et groupes de Steinberg relatifs}}, 
 J. Alg. \textbf{54} (1978), no.~1, 178--202.

\bibitem{Ma69}
 H.~Matsumoto,
 \emph{\href{http://eudml.org/doc/81843} 
 {Sur les sous-groupes arithm{\'e}tiques des groupes semi-simple d{\'e}ploy{\'e}s}},
 Ann. Sci. ENS \textbf{2} (1969), no.~1, 1--62.

\bibitem{Mo04}
 F.~Morel,
 \emph{\href{https://dx.doi.org/10.1007/s00014-004-0815-z} 
 {Sur les puissances de l'id{\'e}al fondamental de l'anneau de Witt}},
 Comm. Math. Helv. \textbf{79} (2004), no.~4, 689--703.

\bibitem{MR91}
 J.~Morita and U.~Rehmann,
 \emph{\href{https://dx.doi.org/10.1007/BF02571324} 
 {Symplectic $\K_2$ of Laurent polynomials, associated Kac--Moody groups and Witt rings}},
 Math. Z. \textbf{206} (1991), no.~1, 57--66.

\bibitem{Pa89}
 I.~Panin, 
 \emph{\href{http://www.mathnet.ru/links/a46c953dc23ca4b3c44578c283cd59c8/aa27.pdf} 
 {On stabilization for orthogonal and symplectic algebraic K-theory (Russian)}},
 Leningrad Math. J. \textbf{1} (1990), no.~3, 741--754.

\bibitem{Qu76}
 D.~Quillen, 
 \emph{\href{https://dx.doi.org/10.1007/BF01390008} 
 {Projective modules over polynomial rings}},
 Inv. Math. \textbf{36} (1976), no.~1, 167--171.

\bibitem{Re75}
 U.~Rehmann, 
 \emph{\href{http://www.math.uni-bielefeld.de/~rehmann/Publ/praesentation.pdf}
 {Pr{\"a}sentationen von Chevalleygruppen {\"u}ber $k[t]$}}, preprint (1975).

\bibitem{RS76}
 U.~Rehmann and C.~Soul{\'e},
 \emph{\href{https://dx.doi.org/10.1007/BFb0079999} 
 {Finitely presented groups of matrices}}, 
 Algebraic K-Theory, Lect. Notes. Math., vol. 551, Springer, 1976, pp.~164--169.

\bibitem{Sch16}
 M.~Schlichting, 
 \emph{\href{https://dx.doi.org/10.1016/j.jpaa.2016.12.026} 
 {Hermitian K-theory, derived equivalences and Karoubi's fundamental theorem}},
 J. Pure Appl. Alg. \textbf{221} (2017), 1729--1844.

\bibitem{S15}
 S.~Sinchuk, 
 \emph{\href{https://dx.doi.org/10.1016/j.jpaa.2015.08.003}
 {On centrality of $K_2$ for Chevalley groups of type $\mathsf{E}_\ell$}},
 J. Pure Appl. Alg. \textbf{220} (2016), no.~2, 857--875.

\bibitem{St-poly}
 A.~Stavrova, 
 \emph{\href{https://dx.doi.org/10.1017/is013006012jkt232} 
 {Homotopy invariance of non-stable $\K_1$-functors}}, 
 J. K-th. \textbf{13} (2014), no.~2, 199--248.

\bibitem{St-Ded}
 A.~Stavrova, 
 \emph{\href{https://dx.doi.org/10.1515/jgth-2019-0100} 
 {Chevalley groups of polynomial rings over Dedekind domains}}, 
 J. Gr. Th. \textbf{23} (2020), 121--132.

\bibitem{St71}
 M.~R.~Stein, 
 \emph{\href{https://dx.doi.org/10.2307/2373742} 
 {Generators, relations and coverings of Chevalley groups over commutative rings}},
 Amer. J. Math. \textbf{93} (1971), no.~4, 965--1004.

\bibitem{Ste73}
 M.~R.~Stein, 
 \emph{\href{https://dx.doi.org/10.2307/1996696} 
 {Surjective stability in dimension 0 for $\K_2$ and related functors}},
 Trans. Amer. Math. Soc. \textbf{178} (1973), 165--191.

\bibitem{St78}
 M.~R.~Stein, 
 \emph{\href{https://dx.doi.org/10.4099/math1924.4.77} 
 {Stability theorems for $\K_1$, $\K_2$ and related functors modeled on Chevalley groups}},
 Japan J. Math. \textbf{4} (1978), 77--108.

\bibitem{St67}
 R.~Steinberg, J.~Faulkner, and R.~Wilson, 
 \emph{Lectures on Chevalley groups},
 Yale Univ. New Haven, 1967.

\bibitem{Su76}
 A.~Suslin, 
 \emph{\href{http://www.mathnet.ru/links/1ebfa3efeb44dfd494087eaf83a34450/dan40545.pdf} 
 {Projective modules over polynomial rings are free}}, 
 Dokl. Akad. Nauk SSSR \textbf{229} (1976), 1063--1066.

\bibitem{Su77}
 A.~Suslin, 
 \emph{\href{https://dx.doi.org/10.1070/IM1977v011n02ABEH001709} 
 {On the structure of the special linear group over commutative rings}},
 Math. USSR Izv. \textbf{11} (1977), 221--238.

\bibitem{Su82}
 A.~Suslin and V.~Kopeiko, 
 \emph{\href{https://dx.doi.org/10.1007/BF01681481} 
...
\bibitem{ST76}
 ...
 \emph{\href{https://dx.doi.org/10.1007/BF01091768} 
 {Stabilization theorem for the Milnor $K_2$-functor}},
 J. Soviet Math. \textbf{17} (1981), no.~2, 1804--1819.

\bibitem{Ta86}
 G.~Taddei, 
 \emph{\href{http://www.ams.org/books/conm/055.2/862660/conm862660.pdf}
 {Normalit\'e des groupes \'el\'ementaires dans les groupes de Chevalley sur un anneau}},
...
\bibitem{VZ18}
 ...
 \emph{\href{https://dx.doi.org/10.1017/S0013091519000555}
 {Generation of relative commutator subgroups in Chevalley groups. II}},
 Proc. Edinburgh Math. Soc. \textbf{63} (2020), no.~2, 497--511.

\bibitem{Vo11}
 K.~V{\"o}lkel,
 \emph{\href{http://static.konradvoelkel.de/download/da-voelkel-20110809.pdf}
 {Matsumotos Satz und $\mathbb{A}^1$-Homotopietheorie}},
 Diplomarbeit, Uni. Freiburg.

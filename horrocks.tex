...
\addbibresource{horrocks.files/references.bib}
...
\EndMSC
\KEY 
Steinberg group, $K_2$-functor, Quillen--Suslin theorem, Horrocks theorem, $\mathbb{P}^1$--glueing.
\EndKEY
...
\newline
\scalebox{0.84}{\begin{minipage}{1.16\linewidth}
...
[x_{\beta+\gamma}(\epsilon_1 \cdot bt^{e-d}), [x_{-\beta}(t^d), x_{\alpha+\beta}(-\delta_1 \cdot a)] ] & \text{ by $\text{R2}_{d,0}$, $\text{R2}_{d, e-d}$} \nonumber \\ 
 ...
 = x_{\alpha+\beta+\gamma}(-\epsilon_1\epsilon_2 \cdot abt^e) \cdot [x_\gamma(bt^e), x_\alpha(at^d)] & \text{ by $\text{R3}^\angle_{e,e}$}, \label{first-computation} \end{align}\end{minipage}} \vskip \belowdisplayshortskip
...
\newline \scalebox{0.85}{\begin{minipage}{1.16\linewidth}
\begin{align*} [x_\alpha(at^d), x_{\gamma}(bt^{e})] = [x_\alpha(at^d), [x_{\beta+\gamma}(b t^{e-1}), x_{-\beta}(-\epsilon_1 t)]] & \text{ by~$\text{R2}_{e-1,1}$ } \\
 ...
 = 1 & \text{ by $\text{R3}^\bot_{1,d+e-1}$,} \end{align*}\end{minipage}} where $\epsilon_1 = N_{\beta,\gamma}$, $\epsilon_2 = N_{\alpha,\beta+\gamma}$.
 \end{enumerate}                                                           
...
\begin{lemma}\label{lem:action} For any $[p, h, u]\in \overline{V}_T$ and any $p_1 \in \overline{G}^{\geq 0}_M$ one has $j_R^{-1}(p_1) \cdot [p, h, u] = [p_1p, h, u]$. \end{lemma}
\begin{proof} 

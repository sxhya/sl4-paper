...
If $a$ is a homogeneous element of $B$ we denote its homogeneous degree by $|a|$.

In what follows we will be concerned with finding a presentation of the Steinberg group $\St(4, B)$ in terms of those generators $x_{ij}(\xi)$
 for which $\xi$ is homogeneous of degree $\leq 1$ (i.\,e. $|\xi| \leq 1$).
Our presentation should be thought of as an extension of~\cite[Lemma~3.3]{Tu83} to the case $n=4$ (with variables $t$ and $t^{-1}$ swapped).
Alternatively, our result can be thought of as a variant of the classical theorem of Rehmann~\cite{Re75} (cf. also with Rehmann--Soul{\'e} theorem in~\cite{RS76}).

There are several reasons why we can not simply refer to~\cite{Re75} or~\cite{RS76} and need to repeat arguments from these papers.
\begin{itemize}
 \item Rehmann and Soul{\'e} deal with usual polynomial rings rather than $\mathbb{Z}$-graded rings
  (i.\,e. their result corresponds to the special case $\mathfrak{m}=0$ of our result);
 \item their presentation is formulated in terms of generators $x_{ij}(\xi)$ with $|\xi| \leq 2$ while for our purposes we need a presentation in terms
  of the smaller set of generators with $| \xi |\leq 1$;
 \item it is assumed that $A=k$ is a field in~\cite{Re75} and that $A=\mathbb{Z}$ in~\cite{RS76}, while work with
  arbitrary coefficient ring $A$.
\end{itemize}


For $m \geq 1$ consider the following set of generators:
\[\XX{m} = \{ x_{ij}(\xi) \mid \xi \in B_k,\text{ for } k\leq m,\ 1\leq i, j\leq 4,\ i\neq j\}. \]
It is obvious that $\XX{m} \subseteq \XX{m+1}$, denote by $\XX{\infty}$ the union of all $\XX{n}$'s.

For $m\geq 1$ denote by $G_m$ the group presented by generators $\mathcal{X}_m$ and the set of relations $\RR{m}$ consisting of the following three families of relations:
\begin{align}
 \label{eq:am} \tag{$a_m$} x_{ij}(\xi) x_{ij}(\eta) & = x_{ij}(\xi+\eta),&  \xi,\eta\in B_k,\ k\leq m;&\\
...
\nonumber \tag{$df_1$}
[[x_{ij}(a t), x_{jk}(b t)],  x_{kl}(c)] & = [x_{ik}(a b t),  x_{kl}(c t)]\\
\nonumber \tag{$e_1$}
[[x_{\alpha}(at), x_{\beta}(bt)], x_{\gamma}(ct) ]& = 1, & \alpha + \beta \in \Phi,\ \alpha+\beta \perp \gamma \\
\nonumber \tag{$ef_1$}
[[x_{ij}(at), x_{jk}(bt)], x_{jl}(ct) ]& = 1
\end{align}
 \end{lemma}

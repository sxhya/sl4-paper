...
\begin{lemma} Suppose that $m\geq 1$. For every $\alpha, \alpha' \in \Phi$ such that $\alpha+\alpha' \in \Phi$ and
 $a\in A$, $\xi \in B_k$, $k \leq 0$ the following relation holds in $G_m$: 
\begin{equation}
\label{eq:verify-bmp1} [x_\alpha(\xi), S_{\alpha'}(at^{m+1})] = [x_\alpha(t\xi), x_{\alpha'}(at^m)].
\end{equation}
\end{lemma}
\begin{proof}
By \cite[Lemma~3.1.2]{RS76} we can assume without loss of generality that $\alpha'=\beta$ for some $\rA_3$-triple $(\alpha, \beta, \gamma)$.
\begin{align*}
...
There are two cases.
\begin{enumerate}
 \item \label{case:cm-1} Case $m=1$, $k = 0$.
 \begin{align*}
   RHS  = [x_{\alpha +\beta + \gamma}(\epsilon t\xi), x_{-\gamma}(a't^m)] & \text{ by $(d_1)$.}\\
 ...
 \item \label{case:cm-2} Case $m\geq 2$ or $m=1$, $k < 0$.
 \begin{align*}
 RHS = {}^{x_{\beta+\gamma}(t)}[x_{\alpha+\beta+\gamma}(\epsilon t^2\xi), x_{-\gamma}(a't^{m-1})]       &  \text{ by~\eqref{item:i} if $k=0$ or~\eqref{eq:cm} if $k <0$} \\

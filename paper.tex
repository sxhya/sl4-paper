...
\begin{align}
 ...
 \end{align}
For $m=1$ define $G_1$ as the group presented by generators $\mathcal{X}_m$, three families of relations $a_1, b_1, c_1$ as above and the following extra family of relations:
\begin{align}  \label{eq:d1} \tag{$d_1$} [[x_{\alpha}(\xi), x_{\beta}(\eta)], x_{\gamma}(\zeta) ] & = 1, & \xi,\eta,\zeta\in B_1,\ \alpha + \beta \in \Phi,\ \alpha + \beta + \gamma \not\in \Phi\cup\{0\}.
\end{align}
...
\begin{prop}\label{prop:tul3.3}
 For $n\geq 1$ the map $\theta_n$ is an isomorphism. Consequently, $\St(\Phi, B)$ can be presented by generators $\mathcal{X}_1$
  and four families of relations $a_1$, $b_1$, $c_1$, $d_1$.
\end{prop}

 \begin{lemma} In the group $G_1$ the following relation holds:
\begin{align} 
 [[x_{\alpha}(a t), x_{\beta}(b t)],  x_{\gamma}(c)] & = [x_{\alpha+\beta}(a b t),  x_{\gamma}(c t)].
\end{align}
 \end{lemma}

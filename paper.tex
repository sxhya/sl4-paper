...
$t(u, v)$ the rank $1$ matrix $e + uv^t$. Notice that $t(u, v)$ lies in $\E(n, R)$ provided $u$ or $v$ is unimodular. %TODO: Reference
...
Now assume that $I$ is an ideal of ring $R$ embedded as a subring into a larger ring $S$. Let $d$ be an element of the subgroup $T(n, S)$ of diagonal matrices of $\GL(n, S)$ such that $d \cdot I^n \subseteq R^n$.
Now for $u \in \Um(n, R)$, $v \in I^n$ such that $u^tv = 0$ and $d^* u = d^{-1}u \in R^n$ we want to define certain elements $X_d(u, v) \in \St(n, R)$. If $v = \sum v_i$ is a decomposition of $v$ into a sum of elements $v_i \in D(u) \cap I^n$ (e.\,g. the canonical decomposition~\eqref{eq:canonical}), we set $X_d(u, v) = \prod x(d^{-1}u, dv_i)$. 
\begin{lemma} %\label{lem:cor-conj}
 The element $X^g(u, v)$ is well-defined, i.\,e. it does not depend on the choice of  decomposition for $v$.
\end{lemma}
\begin{proof}
 The idea is to decompose each factor $x(d^{-1}u, dv_i)$ further using the canonical decomposition of $v_i$ and then using~\eqref{item:xsmall-scalar}--\eqref{item:xsmall-commute} arrive to the canonical decomposition of $v$ by reordering and collecting factors (cf.~with Tulenbaev's argument after~\cite[Lemma~1.1]{Tu83}).
 ...
 Using~\eqref{item:xsmall-scalar}--\eqref{item:xsmall-commute} we can rewrite $X_d(u, v)$ as follows:
 \begin{multline*} \prod\limits_r x(d^{-1}u, dv^r) = \prod\limits_{r}\prod\limits_{i<j} x(d^{-1} u, du_{ij}c_{ij}(v^r, w)) =  \prod\limits_{i<j} x(d_1^{-1} u, d u_{ij}c_{ij}(v, w)). \qedhere \end{multline*}
\end{proof}



%TODO: Pick a decomposition  $v = \sum v_i$
Indeed, for $u \in E(n, A[t])$ and $v \in tA[t]$ define $\sigma_1$ on $N_{0}=\St(n, A[t], tA[t])$ by the following identities: 
\begin{equation}
...
%TODO: define \delta_1 & \delta_1^{-1} -- these are not matrices

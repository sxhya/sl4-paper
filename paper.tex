...
Now suppose  $1 \leq d \leq m+1$. Clearly \eqref{lem:bmp1} and \eqref{eq:defmpn} (in the case $d\leq m$) or~\cref{lem:bmp1} (in the case $d=m+1$) imply that
...
{}^{S_\gamma(t^d)}S_\alpha(bt^{m+1}) = {}^{S_\gamma(t^d)}[x_{-\beta}(b't^{m+1}), x_{\alpha+\beta}(1)] & \text{ by~\cref{lem:bmp1}}\\
 ...
 = S_{\alpha}(b'''t^{m+1})                                                                            & \text{ by $(c_m)$ and \eqref{eq:verify-bmp1'}} \end{align*}

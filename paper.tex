...
\begin{dfn}
Let $A$ be a ring and $R = A[t, t^{-1}]$ be the ring of Laurent polynomials over $A$.

By definition, a {\it $\sigma$-pair for $\varpi$} is a quadruple $(N(\varpi), N(-\varpi), \sigma(\varpi), \sigma(-\varpi))$ consisting of the following data:
\begin{itemize}
 \item a pair of subgroups $N(\varpi), N(-\varpi) \subseteq \St(\Phi, A[t])$;
 \item a pair of mutually inverse group-theoretic bijections $\sigma(\varpi)\colon N(\varpi) \to N(-\varpi)$, $\sigma(-\varpi)\colon N(-\varpi) \to N(\varpi)$
...
\xymatrix{ N(\varpi) \ar@{-->}[r]^{\sigma(\varpi)}\ar@{^{(}->}[d] & N(-\varpi) \ar@{^{(}->}[d] \ar@{-->}[r]^{\sigma(-\varpi)} & N(\varpi) \ar@{^{(}->}[d] \\ 
          ...
          \St(\Phi, R) \ar@<-0.0ex>[r]_{\chi(\varpi, t)} & \St(\Phi, R) \ar@<-0.0ex>[r]_{\chi(-\varpi, t)} & \St(\Phi, R).} \end{equation}  
\end{itemize}
\end{dfn}

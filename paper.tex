...
$\mathcal{X}_{\Phi, R} = \{x_{\alpha}(\xi) \mid \xi\in R, \alpha\in\Phi\}$ and the set of relations $\mathcal{R}_{\Phi, R}$ 
...
It is clear the above construction is functorial in $R$ and so there is a functor $\St(\Phi, -)\colon \catname{CRings}\to \catname{Groups}$.
...
The set of morphisms between $(R, I)$ and $(S, J)$ is defined as the set of all ring-theoretic maps $f\colon R\to S$ such that $f(I)\subseteq J$.
There is an obvious fully faithful embedding $\iota\colon \catname{CRings} \to \catname{Pairs}$ defined on objects by $\iota(R) = (R, R)$.
Now if there is a functor $F\colon \catname{CRings}\to \catname{Grp}$ then its {\it relativization} is another functor $\widetilde{F}\colon \catname{Pairs} \to \catname{Groups}$
such that $\widetilde{F} \iota = F$.
...
\begin{equation}
 \xymatrix{ 1 \ar[r] & C(\Phi, R, I) \ar[r] & \St(\Phi, R, I) \ar[r] & \St(\Phi, R) \ar[r] & \St(\Phi, R/I) \ar[r] & 1 }
\end{equation}
...
Every map of abelian groups $\chi \in \Hom(\ZZ \Phi, R^*)$ specifies an action on the set of generators $\mathcal{X}_{\Phi, R}$ of the Steinberg group $\St(\Phi, R)$ via
\begin{equation} \chi \cdot x_\alpha(\xi) = x_\alpha(\chi(\alpha) \cdot \xi),\ \alpha\in \Phi,\ \xi \in R. \end{equation}
It is not hard to check that this action is compatible with Steinberg relations $\mathcal{R}_{\Phi, R}$ and 
 hence gives a well-defined automorphism of $\St(\Phi, R)$ which we denote by the same symbol $\chi$.

\begin{comment}
\begin{example}
Let $A$ be a ring, denote by $R$ the ring $A[t, t^{-1}]$ of Laurent polynomials over $A$.
Now if $\varpi \in P(\Phi^\vee)$ is a coweight then there is a pair of mutually inverse automorphisms $(\chi_{\varpi, t}, \chi_{-\varpi, t} = \chi_{\varpi, t^{-1}})$ 
 of $\St(\Phi, R)$ via $\beta_i^+ = \chi_{\varpi_i, t}$, $\beta_i^- = \chi_{\varpi_i, t^{-1}} = \chi_{-\varpi_i, t}$.
It is easy to see that
\begin{equation}\label{eq:sigma_act} \beta_i^\pm(x_\alpha(\xi)) = x_\alpha(t^{\pm m_i(\alpha)} \cdot \xi).\end{equation}
\end{example}

One of the key steps of in the proof of Horrocks theorem for $K_2$ is to define analogue of maps $\beta_i^\pm$ on the level of the group $\St(\Phi, A[t])$.
...
Denote by $\mathcal{X}(\varpi)$ the subset of $\mathcal{X}_{\Phi, A[t]}$ consisting of those generators $x_{\alpha}(\xi)$ of $\St(\Phi, A[t])$ for which 
$(\alpha, \varpi) < 0$ implies that $\xi \in A[t]$ is divisible by $t^{-(\alpha, \varpi)}$.
% either $i\neq k$ (resp. $j\neq k$) or $\xi\in R[t]$ is divisible by $t$.
Denote by $N(\varpi)$ the subgroup of $\St(\Phi, A[t])$ generated by $\mathcal{X}(\varpi)$.

\begin{dfn}
By definition, a {\it $\sigma$-pair for $\varpi$} is a pair of mutually inverse group homomorphisms $\sigma(\varpi)\colon N(\varpi) \to N(-\varpi)$, $\sigma(-\varpi)\colon N(-\varpi) \to N(\varpi)$ satisfying
\begin{equation} \label{eq:sigmadef}
\sigma(\pm \varpi)(x_\alpha(\xi)) = x_\alpha(t^{(\pm \varpi, \alpha)}\cdot \xi), 
 \text{ for all } x_\alpha(\xi) \in \mathcal{X}(\pm\varpi).
\end{equation}\end{dfn}
It is clear that any maps $\sigma(\varpi), \sigma(-\varpi)$ satisfying~\eqref{eq:sigmadef} automatically fit into the following commutative diagram:
...
\begin{rem} It is clear that the maps $\sigma(\varpi)$, $\sigma(-\varpi)$ are uniquely determined by~\eqref{eq:sigmadef}, so at most one $\sigma$-pair may exist for any given $\varpi$. However, since we have no direct description of the set of relations between the elements of $\mathcal{X}(\varpi)$, we cannot a priori be sure that formulas~\eqref{eq:sigmadef} specify well-defined maps between $N(\varpi)$ and $N(-\varpi)$.
\end{rem}


\begin{comment}
\begin{itemize}
 \item a pair of subgroups $N(\varpi), N(-\varpi) \subseteq \St(\Phi, A[t])$;
 \item a pair of mutually inverse group-theoretic bijections 
...
For a ring $R$ we denote by $\Sym(\Phi, R)$ the subgroup of $\St(\Phi, R)$ generated by Steinberg symbols $\{a, b\}_\alpha$, $a, b \in R^*$. 
 If $\rk(\Phi)\geq 2$ it is clear that $\Sym(\Phi, R)$ is a central subgroup of $\St(\Phi, R)$ contained in $\K_2(\Phi, R)$. 
Denote by $H(R) \subseteq \St(\Phi, R)$ the composite of the two subgroups $\Sym(\Phi, R)$ and $\UU(\Phi^+, R)$.

%Similarly $\Sym(\Phi, A[t, t^{-1}], M[t, t^{-1}]$ denotes the subgroup of $\St(\Phi, A[t, t^{-1}], M[t, t^{-1}])$ generated by elements of the form $h_\alpha(1+q)^{}$
As before $A$ is a commutative local ring with the maximal ideal $M$.
 
$j_+\colon A[t] \to A[t, t^{-1}]$. We write $j_+^*$ instead of $\St(\Phi, j_+)$.
 


Denote by $V_0$ the set $\St(\Phi, A[t]) \times H(A[t, t^{-1}]) \times \St(\Phi, A[t, t^{-1}], M[t, t^{-1}])$. 
We define an equivalence relation $\sim$ on $V_0$ as the minimal equivalence relation generated by the following 3 elementary transformations:
\begin{enumerate}
 \item $(a \cdot j_+^*(p), b, \beta) \sim (a, H(p) \cdot b, \beta)$, where $p$ is an element $\UU(\Phi^+,A[t])$;
 \item $(a \gamma, b, \beta) \sim (a, b, ?(\gamma)^b \beta)$, where $\gamma$ is an element of $\St(\Phi, A[t], M[t])$;
\end{enumerate}

...
It is clear the above construction is functorial in $R$, for a ring map $f\colon R\to S$ we denote the correponding map of Steinberg groups by $f_*$.

\subsection{Some distinguished subgroups}
%TODO: definition of h_\alpha(\xi)
%TODO: definition of Steinberg symbols

We denote by $\StH(\Phi, R)$ the subgroup of $\St(\Phi, R)$ generated by all elements $h_\alpha(\xi)$, $\xi\in R^*$ and by $\StB(\Phi, R)$ the product of the two subgroups $\StH(\Phi, R)$ and $\UU(\Phi^+, R)$ inside $\St(\Phi, R)$.

By definition, $\Sym(\Phi, R)$ is the subgroup of $\St(\Phi, R)$ generated by all Steinberg symbols $\{a, b\}_\alpha$, $a, b \in R^*$. 
...
\begin{rem} Despite the fact that formula~\eqref{eq:sigmadef} determines the maps $\sigma(\varpi)$, $\sigma(-\varpi)$ uniquely, we cannot a priori be sure that~\eqref{eq:sigmadef} actually {\it defines} them, i.\,e. gives well-defined maps between $N(\pm\varpi)$. This is caused by the fact there is no direct description of the set of relations which hold in $N(\pm\varpi)$ between the generators $\XX(\pm\varpi)$, so we cannot verify that $\sigma(\pm\varpi)$ preserves these relations. \end{rem}

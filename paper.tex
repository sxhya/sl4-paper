...
Thus, for $\varepsilon \in R^*$ and $\varpi \in P(\Phi^\vee)$ we can define a character $\chi_{\varpi, \varepsilon} \in \Hom(\ZZ \Phi, R^*)$
 via the formula $\chi_{\varpi, \varepsilon}(\beta) = \varepsilon ^ {(\varpi, \beta)}$.

Every character $\chi \in \Hom(\ZZ \Phi, R^*)$ acts on the set of generators $\mathcal{X}_{\Phi, R}$ of the Steinberg group $\St(\Phi, R)$ via
\begin{equation} \chi \cdot x_\alpha(\xi) = x_\alpha(\chi(\alpha) \cdot \xi),\ \chi \in H,\ \alpha\in \Phi,\ \xi \in R. \end{equation}
It is not hard to check that the above formula gives a well-defined automorphism of $\St(\Phi, R)$ which we also denote by $\chi$.

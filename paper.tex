...
\begin{proof}
 ...
 %TODO: Write down this argument, at least a part of it?
\end{proof}
...
%To show that $f_m$ is an isomorphism we construct a map $g_m \colon G_{m+1} \to G_m$ in the opposite direction.
%We define it on generators $\mathcal{X}_{m+1}$ and then verify that it is well defined.
%Clearly, $g_m$ should be defined as the identity map on the subset of generators $\mathcal{X}_m \subseteq \mathcal{X}_{m+1}$.
%Now if $x_{\alpha}(\xi)$ is an element of $\mathcal{X}_{m+1}\setminus \mathcal{X}_m$
%there exist $\xi' \in B_m$ and $\alpha'\in \Phi$ such that $\xi = t\xi'$ and $\alpha-\alpha'\in\Phi$, 
% therefore we can set 
%\[g_m(x_\alpha(\xi)) = [x_{\alpha-\alpha'}(\xi'), x_{\alpha'}(N_{\alpha-\alpha',\alpha'} t)].\]
%It follows from~\eqref{item:i} that this definition does not depend on the choice of $\alpha'$.
...
\begin{equation}
\label{eq:verify-bmp1} [x_\alpha(\xi), S_{\alpha'}(at^{m+1})] = [x_\alpha(t\xi), x_{\alpha'}(at^m)] = S_{\alpha+\alpha'}(N_{\alpha,\alpha'}a\xi t^{m+1}).
\end{equation}
\end{lemma}
\begin{proof}
...
\item \label{case:1} First consider the case $d \leq 0$. There are two further subcases.
 \begin{enumerate}
  ...
  \end{multline}
  %What this really proves: $b_{m}(\alpha, |\xi|, -\gamma, 1)$ and $b_m(\alpha, |\xi|, \alpha+\beta+\gamma, m)$ implies $b_{m+1}(\alpha, |\xi|, \alpha+\beta, m+1 )$.
  \item Case $\alpha \perp \alpha'$. 
  As before, we set $\alpha' = \gamma$ and use~\eqref{eq:H1iii},\eqref{eq:bm} and~\eqref{eq:cm}:
  \begin{multline} \nonumber
   [x_\alpha(\xi), S_{\gamma}(bt^{m+1})] = [x_\alpha(\xi), [x_{\beta+\gamma}(b't^m), x_{-\beta}(t)]] = \\   
  = [[x_\alpha(\xi), x_{\beta+\gamma}(b't^m)], {}^{x_{\beta+\gamma}(b't^m)}\!x_{-\beta}(t)] = {}^{x_{\beta+\gamma}(b't^m)}\![x_{\alpha+\beta+\gamma}(b't^m\xi), x_{-\beta}(t)] = 1.
  \end{multline}
 \end{enumerate} 
...
\begin{enumerate}
 \item \label{case:2a} Case $\alpha\not\perp\alpha'$ is handled similarly to the Case~\eqref{case:1a} with the only
  ...
  %TODO: No real induction going on here...
  \begin{multline} \nonumber
...
\begin{align*}
{}^{S_\gamma(t^d)}S_\alpha(bt^{m+1}) = {}^{S_\gamma(t^d)}[x_{-\beta}(b't^{m+1}), x_{\alpha+\beta}(1)] & \text{ by~\cref{lem:bmp1}}\\
 = [S_{-\beta}(b't^{m+1}), {}^{S_\gamma(t^d)}x_{\alpha+\beta}(1)]                                     & \text{ by Case~\eqref{case:2a} since $\gamma\not\perp-\beta$}\\
 = [[x_{-\beta-\gamma}(b''t^{m+1-d}), S_\gamma(t^d)], {}^{S_\gamma(t^d)}x_{\alpha+\beta}(1)]          & \text{ by \eqref{lem:bmp1},\eqref{eq:defmpn} or~\cref{lem:bmp1} if $d=m+1$}\\
 = [x_{-\beta-\gamma}(b''t^{m+1-d)}), [S_\gamma(t^d), x_{\alpha+\beta}(1)]]                           & \text{ by~\eqref{eq:H1iii} and~\eqref{eq:bm} since $m+1-d \leq m$}\\
 = S_{\alpha}(b'''t^{m+1})                                                                            & \text{ by $(c_m)$,\eqref{lem:bmp1},\eqref{eq:defmpn} or~\cref{lem:bmp1} if $d=m+1$} \end{align*}
...
\begin{align*}
 ...
 = {}^{S_{-\beta}(t^d)}\![x_{\alpha+\beta}(b't^{m+1-d}), S_\gamma(at^d)] & \text{ by \eqref{lem:bmp1},\eqref{eq:defmpn} or~\cref{lem:bmp1} if $d=m+1$} \\
 = [S_{\alpha}(b't^{m+1}) x_{\alpha+\beta}(b't^{m+1-d}), S_\gamma(at^d)] & \text{ by~\cref{lem:bmp1} and $(b_d)$ if $d\leq m$ or Case~\eqref{case:2} if $d=m+1$ }\\
 = {}^{S_{\alpha}(b't^{m+1})}\!S_{\alpha+\beta+\gamma}(abt^{m+1}) [S_{\alpha}(b't^{m+1}), S_\gamma(at^d)] & \text{ by~\eqref{eq:H1ii} and by \eqref{lem:bmp1},\eqref{eq:defmpn} or~\cref{lem:bmp1} if $d=m+1$} \\

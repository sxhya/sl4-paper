...
\begin{rem}
 Our relative Steinberg symbols differs from the one defined in~\cite{Ste73} in that
  it takes values in $\St(\Phi, R, I)$ rather than in $\St(\Phi, R)$. %Elaborate on this...
\end{rem}

\subsection{Another presentation of linear Steinberg groups} \label{sec:another-presentation}
%TODO: Formulate why do we write what we write
Throughout this subsection $R$ denotes a commutative ring and $I$ its splitting ideal.
...
\begin{lemlist}
 ...
 \begin{equation}\nonumber x(v, wa) = x(va, w).\end{equation} %TODO: What is ''a``?
...
Let $u \in E(n, R)e_1$, $v \in I^n$ be such that $u^tv = 0$.
Recall from \S~4 of~\cite{LS17} that one can define the following elements of $\St(n, R, I)$:
...
Recall from~\cite{LS17} that it is possible to give a presentation of the relative Steinberg group $\St(n, R, I)$ 
formulated in terms of elements $F(u, v)$, $S(v, u)$.
More formally, \cite[Proposition 3.10]{LS17} asserts that $\St(n,\,R,\,I)$ admits presentation with the set of generators
\begin{multline*} \{F(u,\,v)\mid u\in E(n,\,R)e_1,\ v\in I^n,\ u^tv=0\}\, \cup \\ \cup \{S(u,\,v)\mid u\in I^n,\ v\in E(n,\,R)e_1,\ u^tv=0\}\end{multline*}
...
\end{align}
Here $g^*$ denotes the contragradient matrix, i.\,e. $g^* = {g^{-1}}^t$ and $e_i$ is the standard basis vector of $R^n$.

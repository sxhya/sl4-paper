...
\begin{tikzpicture}
\draw[fill=black] 
(0,0) circle [radius=.1] node [below] {$\alpha$} --
(1,0) circle [radius=.1] node [below] {$\beta$} --
(2,0) circle [radius=.1] node [below] {$\gamma$}
\end{tikzpicture}.
...
Now suppose  $0 \leq d \leq m+1$. Clearly \eqref{item:i} and \eqref{eq:defmpn} (in the case $1\leq d\leq m$) or~\cref{lem:bmp1} (in the case $d=0,m+1$) imply that
...
Using~\eqref{eq:H1ii} multiple times it is not hard to show that the map $j\colon \St(\Phi, B) \to G_\infty$ given by
 $j(x_\alpha(b)) = \prod x_\alpha(b_i)$, where $b = \sum b_i$ is the decomposition of $b$ into a finite sum of its homogeneous components $b_i\in B_i$, is a well-defined map.

...
\begin{equation}
 \xymatrix{ 1 \ar[r] & C(\Phi, R, I) \ar[r] & \St(\Phi, R, I) \ar[r]^{\ext{R}} & \St(\Phi, R) \ar[r]^{\pi_*} & \St(\Phi, R/I) \ar[r] & 1 }
\end{equation}

\begin{multline}
 \{ t, u \}'_\alpha = h_{\alpha}(u) ^ {h_{?}(t)} =  y_\alpha(t(u-1)) \cdot y_{-\alpha}\left(-t^{-1}(u^{-1}-1)\right)^{x_\alpha(-t)} \cdot y_\alpha\left(t(u-1)\right)^{w_\alpha(-t)} \cdot \\
\cdot y_\alpha(u-1) \cdot y_{-\alpha}\left(-(u^{-1}-1)\right)^{x_\alpha(-1)} \cdot y_\alpha(u-1)^{w_\alpha(-1)}.
\end{multline}
...
\begin{rem} Despite the fact that formula~\eqref{eq:sigmadef} determines the maps $\sigma(\varpi)$, $\sigma(-\varpi)$ uniquely, 
 we cannot a priori be sure that~\eqref{eq:sigmadef} actually {\it defines} them, i.\,e. gives well-defined maps between $N(\pm\varpi)$.
This is caused by the fact there is no direct description of the set of relations which hold in $N(\pm\varpi)$ between the generators $\XX(\pm\varpi)$, so we cannot verify that $\sigma(\pm\varpi)$ preserves these relations. \end{rem}
...
\begin{enumerate}
 ...
 \item $(a \cdot \ext{A[t]}(\gamma), b, \beta) \sim (a, b, j^*_+(\gamma)^b \cdot \beta)$, where $\gamma$ is an element of $\St(\Phi, A[t], m[t])$;
  %and $b$ acts on $j^*_+(\gamma)$ via the conjugation action of $\St(\Phi, R)$ on $\St(\Phi, A[t, t^{-1}], m[t, t^{-1}])$;
 ...
 \item $(a, b \cdot \{ t, a \}_\alpha, \beta) \sim (a, b, \{ t, a \}'_\alpha \cdot \beta) $, where $a \in (1+m)^*$
  and $\{t, a\}'_\alpha$ is a relative Steinberg symbol discussed in ???
\end{enumerate}

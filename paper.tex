...
Now assume that $I$ is an ideal of ring $R$ embedded as a subring into a larger ring $S$. 
...
\end{comment} 
 ...
 Now from~\eqref{item:xsmall-scalar}--\eqref{item:xsmall-commute} we obtain:
...
\begin{lemma} Suppose that $g = x_{hk}(\xi)$ is a generator of $\St(n, R)$ such that $m = d\phi(g)d^{-1} \in \E(n, R)$, then 
 \begin{equation*} g \cdot X_d(u, v) \cdot g^{-1} = X_d(mu, m^*v) \text{ and } g \cdot Y_d(v, u) \cdot g^{-1} = Y_d(mv, m^*u). \end{equation*}
\end{lemma}
\begin{proof}
Similar to~\cite[3.14]{Ka77}, cf. also with the proof of~\cite[Lemma~4.4d]{LS17}.
\begin{comment} % Do not remove
 ...
 By~\cite[3.12]{Ka77} we can write $g \cdot X_d(u, v) \cdot g^{-1}$ as follows:
 \begin{equation} \nonumber

...
%TODO: Add couple of further corollaries
...
\section{Rehmann--Soul{\'e} theorem}
The aim of this section is to prove a variant of Rehmann--Soul{\'e} theorem on presentation of Steinberg groups. 
...
In what follows we will be concerned with finding a compact presentation of the Steinberg group $\St(4, B)$. This presentation is analogous to the that~\cite[Lemma~3.3]{Tu83}, which is formulated for $\St(n, B)$, $n\geq 5$.
...
There are several reasons why we can't simply refer to the original results of U.~Rehmann and C.~Soul{\'e}~\cite{Re75} or~\cite{RS76}.
The first is that we want to prove Rehmann and Soul{\'e} prove that $G_m \to G_{m+1}$ is an isomorphism for $m\geq 2$, while we want to %establish this fact starting from $m = 1$.
...
%(it is assumed that $A=k$ is a field in~\cite{Re75} and that $A=\mathbb{Z}$ in~\cite{RS76}).

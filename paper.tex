...
Fir $m\geq 2$ denote by $G_m$ the group presented by generators $\mathcal{X}_m$ and the set of relations $\RR{m}$ consisting of the following
...
Now define the group $G_1$ presented by generators $\mathcal{X}_1$ and the set of relations $\RR{1}$ consisting of 
 the above families of relations $(a_1)$, $(b_1)$, $(c_1)$ with one extra family of relations
\begin{equation} \nonumber \tag{$d_1$}
[[x_{\alpha}(\xi t), x_{\beta}(\eta)],  x_{\gamma}(\zeta t)] = [[x_{\alpha}(\xi t), x_{\beta}(\eta t)],  x_{\gamma}(\zeta)], 
\end{equation}
where $\alpha, \beta, \gamma \in \Phi$ are such that $\alpha+\beta+\gamma \in \Phi$ and $\xi, \eta, \zeta \in A$.

The aim of this subsection is to demonstrate the following analogue of~\cite[Lemma~3.3]{Tu83}. 
\begin{lemma}\label{lem:tul3.3}
 Let $\Phi$ be a simply-laced root systems of rank $\geq 3$. 
 Then the natural inclusion of generators $\mathcal{X}_n \subseteq \mathcal{X}_{n+1}$ induces an isomorphism $G_n \to G_{n+1}$ for $n\geq 1$.
 Consequently, the group $\St(\Phi, B)$ is isomorphic to $G_1$.
\end{lemma}
...
%\begin{rem}
%There are two reasons why we can't simply refer to~\cite{Re75} or~\cite{RS76} for the proof of~\cref{lem:tul3.3}.

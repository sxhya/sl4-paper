...
\end{lemma}
\begin{proof}
The action of $S(R)$ on $S(D_{R, I})$ given by ${}^g h = S(\Delta)(g) \cdot h \cdot S(\Delta)(g)^{-1}$ induces an action of $S(R)$ on $S(R, I)$. With respect to this action the map $\mu \colon S(R, I) \to S(R)$ from~\eqref{LodayRelativization} is an $S(R)$-map.
...
In the special case $M = S(R, I)$ this construction produces a map $C \to C_{S}(R, I)$ which is natural in $(R, I)$ by~\cite[Proposition~3]{Lo78}. This map is an isomorphism by~\cite[Th{\'e}or{\`e}me~2]{Lo78}. \end{proof}

Define the unstable Quillen K-groups functors as $\K_{i}^{G, P}(R) = \pi_i(BG(R)^+_{P(R)})$, $i\geq 1$. The functors with lower numbers admit the following description.
...
\subsection{Elementary calculations in relative Steinberg groups}
\begin{comment}
Hall-Witt identity
...
\[  . \]
\end{comment}
Throughtout this section $\Phi$ is a simply laced root system, $A$ is a commutative ring, and $I, J$ are two its ideals. 

We denote by $\overline{\St}(\Phi, A, I)$ the kernel of the map $\St(\Phi, A) \to \St(\Phi, A/I)$.
This group coincides with the image in $\St(\Phi, A)$ of the relative group $\St(\Phi, A, I)$ defined in the previous subsection.

We define the following two families of elements of $\overline{\St}(\Phi, A, I)$:
\begin{itemize}
...
Writing the starting portion of the homology long exact sequence for the map $S(R) \to S(R/I)$ 
and using the isomorphisms of~\cref{lem:relativeH3} and~\cref{lem:lowerKgroups} we obtain a commutative diagram
...
In view of~\cref{lem:Panin-stability} the map $f'$ is precisely the canonical map $\GW_3^{[0]}(k[X]) \to \GW_3^{[0]}(k[X, X^{-1}])$.

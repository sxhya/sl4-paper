...
\end{lemma}
\begin{proof}
 It suffices to verify this property for the generators $g = z_\alpha(f, \xi)$ of the subgroup $\overline{G}^{\geq 0}_M$ ($f \in M[X]$, $\xi\in A[X]$).
 %Notice that in the case $\xi = 0$ the assertion immediately follows from~\eqref{T_leq0} since $t(x_\alpha(f)) = x_\alpha(f)$. 
 We verify the assertion by induction on the degree of $\xi$ in $X$.

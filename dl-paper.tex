...
Recall that a root subset $S \subseteq \Phi$ is called {\it closed} if $\alpha, \beta \in S$ and $\alpha+\beta\in \Phi$ imply $\alpha + \beta \in S$. 
Recall that a closed root subset $S$ is called {\it parabolic} (resp. {\it reductive}, resp. {\it special}) if $S \cup -S = \Phi$ (resp. $S = -S$, resp. $S \cap (-S) = \varnothing$).
...
Let $S \subseteq \Phi$ be a closed root subset (see~\cref{df:root-subsets}). We denote by $U(S, M)$ the subgroup of $\St(\Phi, A)$ generated by root subgroups $X_\alpha(M)$ corresponding to all $\alpha \in S$. We denote by $\Phi^+$ (resp. $\Phi^-$) the subsets of positive (resp. negative) roots of $\Phi$ with respect to some chosen order on $\Phi$.
...
\begin{itemize}
 \item Let $A$ be an arbitarary commutative ring and $M$ be an ideal of $A$. Starting from subsection~\ref{sec:Pa0-basic} we also assume that $A$ is local and $M$ is the maximal ideal of $A$.
...
We denote by $\St^h(\Phi, B)$ the Steinberg group in the homogeneous presentation, i.\,e. the group given by the set of all homogeneous Steinberg generators modulo the following set of relations which we call homogeneous Steinberg relations
...
\end{align}
\begin{rem}
Notice that in the above presentation we omitted the relations $[x_\alpha(a), x_\alpha(b)] = 1$ but it easy to see that they, in fact, follow from~\eqref{eq:H1iii},~$\text{R2}_{d,e}$ and $R3_{d,e}^\angle$: \[ ???. \]
\end{rem}

It is not hard to show that the map $\St^h(\Phi, B) \to \St(\Phi, B)$ induced by the obvious embedding of generators is an isomorphism. 
...
For $n\geq 1$ we define the ``truncated'' Steinberg group $\St^{\leq n}(\Phi, B)$ by means of the set $\mathcal{X}_{\leq n}^\Phi$ of homogeneous Steinberg generators of degree $\leq n$ and the set of homogeneous Steinberg relations $\mathcal{R}_{\leq n}^\Phi$ of degree $\leq n$.  We denote by $F(\mathcal{X}^\Phi_{\leq n})$ the free group on $\mathcal{X}^\Phi_{\leq n}$. 

Notice that $\lim\limits_{n \to +\infty} \St^{\leq n}(\Phi, B) = \St^h(\Phi, B) \cong \St(\Phi, B)$.
...
Let $M$ be an ideal of a local ring $A$. We denote by $P_{\alpha, M}(0)$ the subgroup of $\St(\Phi, A[X, X^{-1}])$ generated by the images of the following five types of elements of $\overline{\St}(\Phi, A[X], M[X])$ under the map $j_+ \colon \St(\Phi, A[X]) \to \St(\Phi, A[X, X^{-1}])$:
...
The assertion is obvious for generators of type 3 and 4.
...
\begin{lemma} \label{Pstar-large} Suppose that $(A, M)$ is a local pair. Then the subgroup $P_{\alpha, M}(*)$ contains the image of $K(A[X], M[X])$ in $\St(\Phi, A[X, X^{-1}])$. \end{lemma}

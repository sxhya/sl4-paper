...
\begin{equation}
 \GW_i^{[k]}(R) = \begin{array}{cl} \mathrm{KO}_i(R), & k = 0, \\ U_i(R), & k = 1, \\ \mathrm{KSp}_i(R), & k = 2, \\ {}_{-1}\!U_i(R), & k = 3. \end{array}
\end{equation}


\begin{lemma}\label{lem:bass-ft} Suppose that $2 \in R^*$, then for any $i\geq 1$, $k\in \ZZ/4\ZZ$ one has
...
For a commutative local ring $A$ with the maximal ideal $M$ we denote by $B = B_{A, M}$ the subring $A[X^{-1}] + M[X]$ of the ring $R = A[X, X^{-1}]$.
...
The following lemma summarizes facts about $\K_2(\Phi, -)$ in the field case.
\begin{lemma}
...
\begin{equation*} \xymatrix{
 C(\Phi, B, I) \ar[r] \ar[d] & \St(\Phi, B, I) \ar[r] \ar[d] & \St(\Phi, B) \ar[r] \ar[d] & \St(\Phi, k[t]) \ar[d] \\
 C(\Phi, R, I) \ar[r]        & \St(\Phi, R, I) \ar[r] \ar@{-->}[ur]& \St(\Phi, R) \ar[r]        & \St(\Phi, k[t, t^{-1}])
}\end{equation*}

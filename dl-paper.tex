...
We need one more technical definition. Denote by $Z_{\alpha, \beta}$ the subgroup $U(Z_+(\alpha) \setminus \{\alpha - \beta \}, M[X])$ of $\St(\Phi, A[X, X^{-1}])$.  Recall that this means that $Z_{\alpha, \beta}$ is generated by all root subgroups $X_\gamma(M[X])$, where $\gamma \in Z_+(\alpha) \setminus \{ \alpha - \beta \}$. It is clear that $Z_{\alpha,\beta} \subseteq j_+(Z_\alpha(A, M))$.
...
The remaining last factor $z_\delta(af, -X)$ can be rewritten as $x_{\delta}(af) \cdot c_{\delta}(-af, X)$ (cf.~\cref{rem:c}), which is a product of generators of type Z and K5. \end{proof}
...
It is clear that the specified conjugation centralizes $j_+(Z_\alpha(A, M))$ (and hence $Z_{\alpha, \beta}$). By~\eqref{Chevalley-CCF1} and~\cref{rem:recognition} 
$X_{-\beta}(M\cdot X)^{x_\alpha(aX^{-1})} \subseteq X_{-\beta}(M\cdot X) \cdot X_{\alpha-\beta}(M) \subseteq P_\alpha(0).$ Finally, from~\cref{image-K-a-b} we get that
$K(\alpha, \beta)^{x_\alpha(aX^{-1})} \subseteq \langle K(\alpha, \beta), Z_{\alpha, \beta} \rangle \subseteq P_\alpha(0)$.
\end{proof}

\begin{corollary} The image of $K(\alpha, \beta)$ under the automorphism of conjugation by $x_\alpha(aX^{-1})$ is contained in $P_\beta(0)$. \end{corollary}
\begin{proof} 

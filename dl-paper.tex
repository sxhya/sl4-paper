...
Let $g = z_\gamma(Xf, \xi)$ be a generator of type $1$ for some $\gamma \in \Phi \setminus \Psi$.
...
If, on the other hand, $\gamma$ is such that $\alpha - \gamma \in \Phi$, we obtain from~\cref{Zrels}\eqref{Z3} that
...
Now let us verify the assertion for the generators of types 2--4.
...
On the other hand, from~\eqref{Chevalley-CCF1} we get that
\begin{align*}
...
\end{align*}
Clearly, the expressions in the right-hand side are products of generators of type 2, 4 and 4, Z.

Now pick a root $\gamma \in \Phi \setminus \Psi$ such that $\alpha + \gamma \in \Phi$. Substituting $s = Xf$, $\eta = X$, $\xi = aX^{-1}$ and $\alpha = -\alpha - \gamma$ into~\eqref{Z5} we obtain that
\begin{multline*} z_{-\alpha}(X^2f, aX^{-1}) = x_{-\alpha-\gamma}(\epsilon Xf) \cdot x_{-\gamma}(-af) \cdot x_{\gamma}(aX^2 f) \cdot x_{-\alpha}(X^2f) \cdot \\
 \cdot z_{-\alpha-\gamma}(-\epsilon Xf, -\epsilon a) \cdot x_{\alpha+\gamma}(-\epsilon a^2 Xf) \cdot x_{\alpha}(- a^2 f) \cdot z_{-\gamma}(a f, -X), \end{multline*}
where $\epsilon = N_{\gamma,\alpha}$. 
It is clear that the the first 7 factors in the right-hand side are generators of type
1, Z, 1, 2, 1, 1, Z.
...
\begin{proof} It follows from~\cref{K-a-b} that $\widetilde{P}_\alpha(0) \cap K(A[X], M[X]) =  \widetilde{K}(\alpha, \beta) \cdot X_{-\beta}(M \cdot X).$ 
Since any subgroup $H \leq \St(\Phi, A[X], M[X])$ satisfies $H = \left(H \cap K(A[X], M[X])\right) \rtimes ev^*_{X=0}\left(H\right)$, we obtain from~\cref{rem:DS} that

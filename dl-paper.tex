...
The transitivity follows from~\cref{lem:action}, the first property of~\cref{prop43} and the following calculation, which we leave as an exercise to the reader:
...
\begin{proof}[Proof of~\cref{thm:main}]
 ...
 Now let $A$ be arbitrary commutative ring and $\Phi = \rD_\ell$. If $g \in \St(\Phi, A[X])$ is such that its image in $\St(\Phi, A[X, X\inv])$ is trivial, then so is its image in all localizations $\St(\Phi, A_M[X, X\inv])$, where $M$ ranges over the maximal ideals of $A$. By the just established local case the images of $g$ in all $\St(\Phi, A_M[X])$ are also trivial. Now by the local-global principle \cite[Theorem~2]{LS17} the element $g$ is trivial as well.
 ...
 Notice that $h_+ \in \overline{\St}(\Phi, A[X], XA[X])$, moreover, for every maximal ideal $M$ of $A$, the image of $h_+$ in $\St(\Phi, A_M[X], XA_M[X])$ is trivial by~\cref{thm:P1glueing}.

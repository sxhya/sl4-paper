...
But $c_{\pm \gamma}(f, X\xi) = x_{\pm \gamma}(f) \cdot z_{\pm \gamma}(-f, -X\xi)$ is a product of two generators of type 3. \end{proof}
...
\[\xymatrix{1 \ar[r] & P_\alpha(0) \ar[r] & P_\alpha(*) \ar[r] & M^+ \ar@<5pt>@{-->}[l] \ar[r] & 1}.\] \end{lemma}
\begin{proof} We need to verify that the conjugate by $x_{-\alpha}(mX)$ to every generator of $P_\alpha(0)$ listed in~Definition~\ref{defP0} still belongs to $P_\alpha(0)$.
...
\begin{proof} We need to verify that the conjugate by $x_\alpha(aX^{-1})$ to every generator of $P_\alpha(0)$ lies in $P_\alpha(0)$. 
First of all, consider the case $\alpha + \beta \in \Phi$ (only possible for the generator of the first type):
...
z_{\beta}(f, X\xi) ^ {x_{\alpha}(aX^{-1})} = & x_{\alpha} (af\xi) \cdot x_{\alpha-\beta} (N_{\beta,-\alpha}\cdot Xf\xi^2a) \cdot z_{\beta}(f, X\xi). \end{align}
...
Choose $\beta\in \Phi$ such that $\alpha+\beta \in \Phi$ and invoke~\eqref{Z5} of~\cref{Zrels} substituting $s = Xf$, $\eta = X$, $\xi = aX^{-1}$:
\begin{multline} \nonumber z_{-\alpha}(X^2f, aX^{-1}) = x_{-\alpha-\beta}(\epsilon Xf) \cdot x_{-\beta}(-af) \cdot x_{\beta}(X^2 f a) \cdot x_{-\alpha}(X^2f) \cdot \\
 \cdot z_{-\alpha-\beta}(-\epsilon Xf, -\epsilon a) \cdot x_{\alpha+\beta}(-\epsilon Xf a^2) \cdot x_{\alpha}(- f a^2) \cdot z_{-\beta}(f a, -X), \end{multline}
...
It remains to verify the assertion for the remaining generator $z_\alpha(f, gX)$ of type 2.
Substituting in~\eqref{Z5} of~\cref{Crels} $s := -\epsilon f$, $\xi = -\epsilon g$, $\eta=X$ we obtain that
\begin{multline} \nonumber z_{\alpha+\beta}(-\epsilon fX, -\epsilon \xi) = x_\alpha( -f) x_{-\beta}(- f \xi) \cdot x_{\beta}( X^2f \xi) \cdot x_{\alpha+\beta}(-X\epsilon f) \cdot \\
 \cdot z_\alpha(f,  X\xi) \cdot x_{-\alpha}(X^2f\xi^2) \cdot x_{-\alpha-\beta}(\epsilon Xf \xi^2) \cdot z_{-\beta}(f\xi, -X), \end{multline}
where $\epsilon = N_{\alpha,\beta}$. 
We can express $z_{\alpha}(f, X\xi)$ through other terms and obtain that
\begin{multline} \nonumber z_\alpha(f, X\xi) = x_{\alpha+\beta}(\epsilon Xf) \cdot x_{\beta}(-X^2 f \xi) \cdot x_{-\beta}(f \xi) \cdot x_\alpha(f) \cdot \\ 
 \cdot z_{\alpha+\beta}(-\epsilon X f, -\epsilon \xi) \cdot z_{-\beta}(-f\xi, -X) \cdot x_{-\alpha-\beta}(-\epsilon X f \xi^2) \cdot x_{-\alpha}(-X^2 f\xi^2) \cdot \end{multline}

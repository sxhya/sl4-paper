...
\begin{df}\label{def:Kab} Denote by $\widetilde{K}(\alpha, \beta)$ the subgroup of $\St(\Phi, A[X], M[X])$ generated by the following five families of elements (below $\xi \in A[X]$, $f\in M[X]$):
 \begin{enumerate}
...
Denote by $K(\alpha, \beta)$ the image of $\widetilde{K}(\alpha, \beta)$ in $\St(\Phi, A[X, X\inv])$. \end{df}
...
Consequently, $K(\alpha, \beta)$ coincides with the intersection $\mathrm{Im}(K(A[X], M[X]) \to \St(\Phi, A[X, X\inv])) \cap P_\alpha(0) \cap P_\beta(0)$. In particular, $K(\alpha, \beta)$ does not depend on the choice of the root $\delta$. \end{prop}
\begin{proof} 
For $i=1..5$ denote by $\mathcal{G}_i$ the set of all generators of type $i$ from~\cref{def:Kab}.
By~\cref{thm:Stepanov} every element of $\St(\Phi, A[X], XM[X])$ can be presented as a product of generators $\mathcal{G}_1$ and generators $\mathcal{G}' = \{ x_{\gamma}(Xf) \mid \gamma \in \Psi\}$. In turn, by~\cref{Kgen-strong} one can express $g$ as a product of these generators and generators $\mathcal{G}_5$. %We are going to rewrite this product so that the generators of $\widetilde{K}(\alpha, \beta)$ accumulate on the left-hand side of this expression, while the elements of the root subgroups $X_{-\alpha}(MX)$ and $X_{-\beta}(MX)$ accumulate on the right-hand side.

By~\cref{thm:Stepanov} $\widetilde{K}(\alpha, \beta)$ contains the normal subgroup $G_0 = \St(\Phi, A[X], X^2M[X])$, so it suffices to obtain the required presentation of $g$ modulo $G_0$. By~\cref{Zrels} and~\cref{Crels} the elements of $\mathcal{G}'$ commute with the elements of $\mathcal{G}_1$ and $\mathcal{G}_5$ modulo $G_0$, therefore we can rewrite $g$ as $g_1 \cdot g_2$, where $g_1 \in \langle \mathcal{G}_1, \mathcal{G}_5, G_0 \rangle \subseteq \widetilde{K}(\alpha, \beta)$ and $g_2 \in \langle \mathcal{G}' \rangle$.

Now let us study the structure of the subgroup $\langle \mathcal{G}' \rangle$. It is clear that  $x_{\gamma_1}(mX)$ and $x_{\gamma_2}(m'X)$ commute modulo $G_0$ whenever $\gamma_1 \neq - \gamma_2$, $\gamma_1, \gamma_2 \in \Psi$. On the other hand, for $\gamma \in \Psi$ any commutator $c_{\gamma}(mX, m'X)$ is congruent to some generator of $\mathcal{G}_5$  modulo $G_0$ (cf. the proof of~\cref{Kgen-strong}) and, therefore, commutes with the elements of $\mathcal{G}'$ modulo $G_0$. 

Now pick each factor $x_{\gamma}(Xf)$, $\gamma \in \{ -\alpha, -\beta \}$ appearing in the presentation for $g_2$, decompose it as $x_{\gamma}(X^2f') \cdot x_{\gamma}(m'X)$ for some $f', m'$ and then move the second factor to the rightmost position within $g_2$ conjugating all factors along the way. By the previous paragraph only elements of  $\mathcal{G}_5$ may appear modulo $G_0$ after the simplification of these conjugates.

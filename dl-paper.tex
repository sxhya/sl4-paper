...
\begin{rem} \label{rem:palpha} From the two preceding lemmas it follows that the value of $p_\alpha$ on an element $g\in K(A[X], M[X])$ can be computed as follows. Express $g$ as a product of elements $z_{\beta}(Xf, \xi)$ for $\beta \in \Phi$ and $c_\delta(f, X\xi)$ for some fixed $\delta \perp \alpha$. Now $p_\alpha(g)$ coincides with the sum of constant terms of the polynomials $f$ corresponding to the factors of the form $z_{-\alpha}(Xf, \xi)$ occuring in this expression. \end{rem}
...
\begin{proof}
 ...
 \begin{df} Denote by $Z_{\alpha, \beta}$ the subgroup of $\St(\Phi, A[X, X^{-1}])$ generated by elements $z_\gamma(f, X\xi)$, where $\gamma \in Z_+(\alpha) \setminus \{ \alpha - \beta \}$.
 \end{df}
  
\begin{lemma} \label{image-K-a-b} The image of $K(\alpha, \beta)$ under $S_{\alpha}(a, -)$ is contained in the subgroup of $\St(\Phi, A[X, X^{-1}])$  generated by $K(\alpha, \beta)$ and $Z_{\alpha, \beta}$.
\end{lemma}
\begin{proof} 
To simplify notation we call the generators of the subgroup $Z$ ``generators of type 6``, despite the fact that these elements are not always contained in $K(\alpha, \beta)$ (or even $K(A[X], M[X])$).

By~\eqref{eq:Zconj1}, \eqref{eq:Zconj2} the conjugate to each generator $z_\gamma(Xf, \xi)$ of type 1 by $x_\alpha(aX^{-1})$ is a product of generators of type 6, 6, 1.
By~\eqref{eq:Zconj3} the conjugate to the generator $x_{-\alpha}(X^2f)$ is a product of generators of type 1, 6, 1, 2, 1, 1, 6, 6.
The conjugation by $x_\alpha(aX^{-1})$ fixes both generators of type 3 and transforms $x_{-\beta}(X^2f)$ into a products of generators of type 2, 1. As for the remaining generators of type 5, \cref{Crels} shows that the corresponding conjugate can be written as a product of generators of type 6, 6, 5. \end{proof}  

\begin{corollary} The image of $K(\alpha, \beta)$ under $S(\alpha, -)$ is contained in $P_\beta(0)$. \end{corollary}
\begin{proof} Follows from the above lemma and the fact that the generators of $Z$ are generators of type 2 for $P_\beta(0)$ in the sense of~\cref{defP0}.
...
\begin{lemma} \label{lem:SRsharp} For $g \in K(A[X], M[X]) \cap P_\alpha(m) \cap P_\beta(m')$ holds \[S_\alpha(a, g)\cdot  h^{-1}_\alpha(1 + am) \cdot x_{\alpha-\beta}(-N_{\alpha,-\beta}\cdot m'a) \in P_\beta\left(\tfrac{m'}{1 + am}\right). \] \end{lemma}
\begin{proof}
 By~\cref{K-a-b} $g$ can be presented as $ g_0 \cdot x_{-\alpha}(mX) \cdot x_{-\beta}(m'X)$ for some $g_0\in K(\alpha, \beta)$, therefore from~\eqref{eq:Smult} and~\cref{lem:Tulenbayev-formula} we obtain that
...
\begin{proof} By~\eqref{Z2}--\eqref{Z4} of~\cref{Zrels} the conjugate to the generator $g=z_\gamma(Xf, \xi)$ of type 1 by $x_{\beta-\alpha}(b)$ either coincides with $g$ or is a product of elements of type 4, 1, 1. 
 Now if $g = x_\epsilon(Xf)$, $\epsilon \in \{\pm \alpha, \pm \beta\}$ is a generator of type 2 or 3, the conjugation by $x_{\beta-\alpha}(a)$ either fixes $g$ or transforms it into a product of either two elements of type 2 or two elements of type 3. 
 ...
 Finally, from~\cref{rem:palpha} we obtain that $z_{\alpha-\beta}(Xf, b) \in K(A[X], M[X]) \cap P_\alpha(0) \cap P_{\beta}(0) = K(\alpha, \beta)$. \end{proof} 
...
\begin{proof} By~\cref{image-K-a-b} and~\cref{conj-K-a-b} it remains to show that the conjugate to each generator $g = z_\gamma(f, X\xi)$, $\gamma \in Z_+(\alpha) \setminus \{ \alpha - \beta \}$ of $Z_{\alpha, \beta}$ by $x_{\beta-\alpha}(b)$ belongs to $P_\alpha(0)$.
 
 Throughout this proof we understand generator types according to~\cref{defP0}.
 It is clear that $g$ itself is a generator of type 2, therefore it remains to consider the case $\gamma\not \perp \beta - \alpha$. 
 
 In the case $\alpha - \beta - \gamma \in \Phi$ we obtain from \eqref{Z2} of~\cref{Zrels} that
\[ z_{\gamma}(f, X\xi) ^ {x_{\beta-\alpha}(b)} = x_{\beta-\alpha} (- bXf\xi) \cdot x_{\gamma + \beta -\alpha} (N_{\gamma, \beta -\alpha}\cdot bf)     \cdot z_{\gamma}(f, X\xi)\ \text{if}\ \alpha - \beta - \gamma \in \Phi. \]
 Notice that $\langle \gamma + \beta - \alpha, \alpha \rangle = \langle \gamma , \alpha \rangle + \langle \beta, 
 \alpha \rangle - \langle \alpha, \alpha \rangle \geq 1 + 1 - 2 = 0$, 
 therefore the expression in the right-hand side is a product of generators of type 1, 2, 2 or 1, 3, 2. {\bf TODO: } Double check that both possibilities actually occur.

Now suppose that the other alternative holds, namely that $\alpha - \beta + \gamma \in \Phi$.
Since $\langle \beta - \gamma - \alpha , \alpha \rangle = \langle \beta , \alpha \rangle - \langle \gamma , \alpha \rangle - \langle \alpha, \alpha \rangle \leq 1 - 1 - 2 = -2$
we obtain that $\beta - \gamma - \alpha = -\alpha$. Thus, by \eqref{Z3} of~\cref{Zrels}
...
\begin{lemma} \label{lem:SRobtuse}
 For $g \in K(A[X], M[X]) \cap P_\alpha(m) \cap P_\beta(m')$ the element
 \[ x_{\beta - \alpha}(b) \cdot S_\alpha(a, g) \cdot h_{\alpha}^{-1}(1+am)\cdot h_{\beta - \alpha}((1 + \epsilon abm')^{-1})\cdot x_{\beta - \alpha}(-b(1 + \epsilon abm')) \]
 belongs to $P_\alpha\left(\tfrac{m - \epsilon bm'}{1+am}\right)$, where $\epsilon = N_{\alpha, -\beta}$.
\end{lemma}
\begin{proof}
Set $c = 1 + \epsilon a b m'$, $s_0 = \langle -a, m \rangle$, $s_1 = \langle -\epsilon am', -bc^{-1} \rangle$.
...
\end{multline}
Denote by $h_0$ the product of all terms except the first and the last one in the above formula. Clearly, $h_0$ is a product of generators of $P_\alpha(0)$ of type 1, 2, 6, 6. Now by~\cref{K-a-b} we can write $g = g_0 \cdot x_{-\beta}(m'X) \cdot x_{-\alpha}(mX)$ for $g_0 \in K(\alpha, \beta)$.
The required assertion now follows from \eqref{eq:Smult}, \eqref{eq:SRobtuse-computation} and the preceding lemma:
\begin{multline} \nonumber

...
Throughout this section $\Phi$ is a simply-laced irreducible root system and $R$ is a local ring with maximal ideal $M$.
We denote by $B$ the ring $M[t] + R[t^{-1}]$ considered as a graded subring of $R[t, t^{-1}]$.
Denote by $\overline{G}^{\geq 0}_M$ the image of the map $\St(\Phi, R[t], M[t]) \to \St(\Phi, R[t, t^{-1}])$ and by
 $G^{\leq 0}$ the group $\St(\Phi, R[t^{-1}])$.

\[ \xymatrix{ \overline{\St}(\Phi, R, M) \ar[r]^{j_+i_+} \ar[d]^{i^-} & \overline{G}_M^{\geq 0} \ar@{^{(}->}[d] \\
              G^{\leq 0} \ar[r]^{j_-} & \St(\Phi, R[t, t^{-1}]), } \] 
...
The following lemma summarizes basic properties of structure constants used in our computations below.
\begin{lemma} Suppose $\Phi$ is simply laced and $\alpha, \beta \in \Phi$ are such that $\alpha+\beta = \gamma \in \Phi$.
\begin{enumerate}
 \item $N_{\alpha, \beta} = -N_{\beta,\alpha}$;
 \item $N_{\alpha, \beta} = - N_{-\alpha, -\beta}$;
 \item $N_{\alpha, \beta} = N_{\beta, \gamma} = N_{\gamma, \alpha}.$
\end{enumerate}
\end{lemma}
\begin{proof} See~\cite[\S~14]{VP}. \end{proof}

Recall that $z_\alpha(s, \xi) := x_\alpha(s)^{x_{-\alpha}(\xi)}$.
\begin{lemma}\label{Zrels} Let $\Phi$ be a simply laced root system and $\alpha,\beta\in \Phi$ be a pair of roots. The elements $z_\alpha(s, \xi)$ satisfy the following relations (everywhere below $\xi, \eta\in R$, $s\in I$):
\begin{enumerate} 
...
\item\label{Z4} $z_{\gamma}(s, \xi) ^ {x_{-\alpha}(\eta)} = z_{\gamma}(s, \xi)\ \text{if}\ \gamma\perp\alpha$.
\item\label{Z5} $z_{\alpha+\beta}(st, \xi) = x_{-\beta}(-s\xi) \cdot x_\alpha(N_{\alpha,\beta}s) \cdot x_{-\alpha}(N_{\alpha,\beta}st^2\xi^2) \cdot x_{-\alpha-\beta}(-st\xi^2) \cdot z_{-\beta}(-s\xi, -t) \cdot x_{\beta}(-st^2\xi)  \cdot x_{\alpha+\beta}(-st) \cdot z_\alpha(N_{\alpha,\beta} s, -N_{\alpha, \beta}t\xi).$
\end{enumerate} \end{lemma}
\begin{proof}
The first four assertions are contained in~\cite[Lemma~9]{S15}. For the proof of the last formula set $\gamma = \alpha + \beta$ and compute:
\begin{multline} z_{\gamma}(N_{\alpha,\beta}st, \xi) = 
[x_\alpha(s)^{x_{-\gamma}(\xi)}, x_\beta(t)^{x_{-\gamma}(\xi)}] = [x_{-\beta}(N_{\alpha, -\gamma} s\xi) \cdot x_\alpha(s),\ x_{-\alpha}(N_{\beta, -\gamma}t\xi) \cdot x_\beta(t)] = \\ = x_{-\beta}(N_{\alpha, -\gamma} s\xi) \cdot x_\alpha(s) \cdot \left( x_{-\beta}(N_{\alpha, -\gamma} s\xi) \cdot x_\alpha(s)\right)^{x_{-\alpha}(-N_{\beta, -\gamma}t\xi) \cdot x_\beta(-t)} = \\
= x_{-\beta}(N_{\alpha, -\gamma} s\xi) \cdot x_\alpha(s) \cdot z_{-\beta}(N_{\alpha, -\gamma} s\xi, -t)^{x_{-\alpha}(-N_{\beta, -\gamma}t\xi)} \cdot z_\alpha(s, -N_{\beta, -\gamma}t\xi)^ {x_\beta(-t)} = \\
= x_{-\beta}(N_{\alpha, -\gamma} s\xi) \cdot x_\alpha(s) \cdot x_{-\alpha}(-N_{\alpha, -\gamma}N_{\beta, -\gamma}st^2\xi^2) \cdot x_{-\gamma}(-N_{-\beta, -\alpha} N_{\alpha, -\gamma} N_{\beta, -\gamma}st\xi^2) \cdot z_{-\beta}(N_{\alpha, -\gamma} s\xi, -t) \cdot \\ \cdot x_{\beta}(-N_{\beta,-\gamma}st^2\xi)  \cdot x_{\gamma}(-N_{\alpha, \beta}st) \cdot z_\alpha(s, -N_{\beta, -\gamma}t\xi) =\\ = x_{-\beta}(-N_{\alpha, \beta} s\xi) \cdot x_\alpha(s) \cdot x_{-\alpha}(st^2\xi^2) \cdot x_{-\gamma}(-N_{\alpha, \beta} st\xi^2) \cdot z_{-\beta}(-N_{\alpha, \beta} s\xi, -t) \cdot x_{\beta}(-N_{\alpha, \beta}st^2\xi)  \cdot x_{\gamma}(-N_{\alpha, \beta}st) \cdot z_\alpha(s, -N_{\alpha, \beta}t\xi). \qedhere
\end{multline}  
In the last equality we invoked the standard properties of structure constants (see e.\,g. formulas in Vavilov-Plotkin \S~14 which imply in our case that $N_{\alpha, \beta} = N_{\beta,-\gamma} = N_{-\gamma, \alpha} = -N_{\alpha, -\gamma}$).
\end{proof}

Using a variant of Hall-Witt identity: $[x, [ y, z]] = [[ x^{-1}, y^{-1}], z] ^ {x^{-1}y^{-1}} \cdot  [[ z^{-1}, x], y^{-1}] ^ {z^{-1}y^{-1}}$ we compute
\begin{multline}
...
\begin{lemma} Denote by $C$ the mutual commutator subgroup $\left[\St(\Phi, XA[X]), \overline{\St}(\Phi, A, M)\right]$.
Then one has \[\overline{\St}(\Phi, A[X], M[X]) = \overline{\St}(\Phi, A, M) \cdot \overline{\St}(\Phi, A[X], XM[X]) \cdot C.\] 
Consequently, the composite of the two factors in the right-hand side of the above formula coincides with the subgroup $I$ defined as 
 the intersection $\overline{\St}(\Phi, A[X], M[X]) \cap \overline{\St}(\Phi, A[X], XA[X])$ (or alternatively, as the kernel of the evaluation map $\overline{\St}(\Phi, A[X], M[X]) \to \overline{\St}(\Phi, A, M)$).
\end{lemma}
\begin{proof}
 Take $g \in \overline{\St}(\Phi, A[X], M[X])$ and write it as $g = \prod_i z_{\alpha_i}(f_i(X), \xi_i(X))$ for some $f_i(X) = f_i(0) + Xf_i'(X) \in M[X]$, $\xi_i(X) = \xi_i(0) + X\xi_i'(X) \in A[X]$.
 It is clear that modulo $\overline{\St}(\Phi, A[X], XM[X])$ the element $g$ is congruent to $g_1(X) = \prod_i z_{\alpha_i}(f_i(0), \xi_i(X)).$ 
 
 Now each factor $z_{\alpha_i}(f_i(0), \xi_i(X))$ can written as $z_{\alpha_i}(f_i(0), \xi_i(0))^{x_{-\alpha_i}(X\xi'_i(X))} = [x_{-\alpha_i}(X\xi'_i(X)), z_{\alpha_i}(f_i(0), \xi_i(0))] \cdot z_{\alpha_i}(f_i(0), \xi_i(0))$.
 Using the fact that the subgroup $\left[\St(\Phi, XA[X]), \overline{\St}(\Phi, A, M)\right]$ is normalized by $\St(\Phi, A, M)$ (this follows e.\,g. from the formula $[g, h]^{h_1} = [h_1^{-1}, g][g, h_1^{-1}h]$) we conclude that $g_1(X)$ is congruent to $g(0) = g_1(0)$ modulo $C$, which implies the first assertion.
 
 Denote by $H$ the composite of $C$ and $\overline{\St}(\Phi, A[X], XM[X])$. It is clear that $H \subseteq I$. The reverse inclusion follows from the above argument and the fact that for $g \in I$ one has $g(0)=1$. \qedhere \end{proof}

 \begin{lemma}
   If $R$ is a local ring with maximal ideal $M$ then $\overline{\St}(\Phi, R, M) = U(\Phi^+, M) \cdot M(\Phi, R, M) \cdot U(\Phi^-, M)$.
 \end{lemma}
 \begin{proof}
  See Theorem~2.4 of~M. Stein's paper on surjective stability in dimension 0.
 \end{proof}

\begin{corollary} For $R$, $M$ as above holds
 $\left[\St(\Phi, XR[X]), \overline{\St}(\Phi, R, M)\right] \subseteq \St(\Phi, XM[X]) \cdot \left[\St(\Phi, XR[X]), \St(\Phi, M)\right]$.
\end{corollary}
\begin{proof}
 Notice first that the generators of $M(\Phi, R, M)$ normalize both $\St(\Phi, XA[X])$ and $\St(\Phi, M)$ (see eq:conj-h-x).
 Now for $g \in \St(\Phi, XR[X])$, $h \in M(\Phi, R, M)$, $u^+ \in U(\Phi^+, M)$, $u^- \in U(\Phi^-, M)$ holds:
 \[ [g, h u^+ u^-] = [g, h] \cdot [{}^{h}\!g, {}^{h}\!(u^+u^-)] \in \St(\Phi, XM[X]) \cdot [\St(\Phi, XR[X]), \St(\Phi, M)]. \qedhere\]
\end{proof}

\begin{corollary} For $R$ and $M$ as above the subgroup $I$ is generated as a group by its subgroup $\St(\Phi, R[X], XM[X])$ and
 the elements $[x_\alpha(m), x_{-\alpha}(X\xi)]$, $m \in M$, $\xi \in R[X]$, $\alpha \in \Phi$. 
\end{corollary}
\begin{proof}
By the above lemmata one has $I \subseteq \St(\Phi, R[X], XM[X]) \cdot [\St(\Phi, XR[X]), \St(\Phi, M)].$
It is clear that modulo $\St(\Phi, R[X], XM[X])$ the commutator subgroup $[\St(\Phi, XR[X]), \St(\Phi, M)]$ is generated
 by all $[x_\alpha(m), x_{-\alpha}(X\xi)]^g$, $m \in M$, $\xi \in R[X]$, where $g \in \St(\Phi, M + XR[X])$.
To verify the assertion, therefore, it remains to show that the commutators $[[x_\alpha(m), x_{-\alpha}(X\xi)], g]$ belong to $\St(\Phi, R[X], XM[X])$.
Moreover, since the latter subgroup is normal it suffices to prove this inclusion in the special case when $g$ is a generator of $\St(\Phi, R[X])$,
i.\,e. $g = x_\beta(\xi)$, $\eta \in R[X]$. Without loss of generality we may assume $\alpha \neq \pm \beta$.

Set $c_{\alpha,\beta}(m,\xi,\eta) = [[x_{\alpha}(m), x_{-\alpha}(X\xi)], x_\beta(\eta)]$.
If $\alpha + \beta \not\in \Phi$, $\alpha - \beta \not \in \Phi$ then, obviously, one has $c_{\alpha,\beta}(m,\xi,\eta) = 1$.
Now suppose $\alpha + \beta \in \Phi$, $\alpha - \beta \in \Phi$. We have:
\begin{multline} \nonumber c_{\alpha,\beta}(m,\xi,\eta) = [x_{\alpha}(m), x_{-\alpha}(X\xi)] [x_{-\alpha}(X\xi), x_{\alpha}(m)x_{\alpha+\beta}(N_{\beta,\alpha} m\eta)] = \\
 = {}^{x_\alpha(m)}\![x_{-\alpha}(X\xi), x_{\alpha + \beta}(N_{\beta, \alpha}m\eta)] = x_{\beta}(N_{\beta,\alpha} N_{-\alpha, \alpha+\beta} mX\xi\eta) x_{\alpha+\beta}(N_{\alpha,\beta}N_{\beta,\alpha}N_{-\alpha, \alpha+\beta}m^2X\xi\eta). 
\end{multline}
It is clear that the expression in the right-hand side belongs to $\St(\Phi, R[X], XM[X])$. In the case $\alpha - \beta \in \Phi$, $\alpha + \beta \not \in \Phi$ the computation is similar.
{\bf TODO:} we have to consider more cases if we want to make the result valid for nonsimply-laced root systems.
\end{proof}
...
Denote by $P_\alpha(0)$ the subgroup of $\St(\Phi, R[X, X^{-1}])$ generated by the images of the following elements of $\St(\Phi, R[X])$ under the
 map induced by the embedding $R[X] \subseteq R[X, X^{-1}]$:
\begin{enumerate}
 \item $z_{\beta}(Xf, g), \beta \in \Phi \text{ such that }\alpha + \beta \in \Phi\text{ or } \alpha - \beta \in \Phi;$
 \item $z_{\beta}(f, Xg),\ \beta \in \Phi \text{ such that }\alpha - \beta \in \Phi \cup \{0\};$
 \item $z_{\gamma}(f, g),\ \alpha \perp \gamma;$
 \item $x_{-\alpha}(X^2f),$
 \item $[x_{\beta}(m), x_{-\beta}(Xg)],\ \beta \in \Phi \text{ such that }\alpha + \beta \in \Phi$.
\end{enumerate}
In the above formulae and below we assume that $f \in M[X]$ and $g \in R[X]$. \end{df}

Denote by $P_\alpha(*)$ the subgroup of $\St(\Phi, R[X, X^{-1}])$ generated by $P_\alpha(0)$ and the elements $x_{-\alpha}(mX)$, $m \in M$. The following lemma shows that this subgroup is sufficiently large.
\begin{lemma} The subgroup $P_\alpha(*)$ contains the image of the subgroup $\St(\Phi, R[X], XM[X])$ in $\St(\Phi, R[X, X^{-1}])$. \end{lemma}
\begin{proof}
Clearly $P_\alpha(*)$ contains the elements $x_\beta(Xf)$ for all $\beta \in \Phi$ and $z_\beta(Xf, g)$ for $\beta \in \Phi \setminus \{\pm \alpha\}$, $f \in M[X]$, $g\in R[X]$.
The assertion now easily follows from Stepanov's theorem. This can also be verified in a straightforward manner without Stepanov theorem.

Now let us verify the assertion for the generators $c_\beta(m, \xi) = [x_\beta(m), x_{-\beta}(X\xi)] = x_\beta(m) \cdot z_{\beta}(-m, -X\xi)$.
It is clear that $c_\beta(m, \xi)$ are contained in $P_\alpha(*)$ in the cases $\beta \perp \alpha$, $\alpha - \beta \in \Phi \cup \{0\}$.
There are problems: 1) for the generator $c_{-\alpha}(m, \xi)$, 2) for the generator $c_\beta(m, \xi)$, $\alpha + \beta \in \Phi$.

\begin{comment}
Let us draw the matrix for $z_\alpha(m, X\xi)$:
\[ \left(\begin{array}{cc} 1 & 0 \\ -X\xi & 1 \end{array} \right) \cdot \left(\begin{array}{cc} 1 & m \\ 0 & 1 \end{array} \right) \cdot \left(\begin{array}{cc} 1 & 0 \\ X\xi & 1 \end{array} \right) = 
 \left(\begin{array}{cc} 1 & m \\ -X\xi & 1 - mX\xi \end{array} \right) \cdot \left(\begin{array}{cc} 1 & 0 \\ X\xi & 1 \end{array} \right) = \left(\begin{array}{cc} 1 + mX\xi & m \\ -mX^2\xi^2 & 1 - mX\xi \end{array} \right) \]
Now let us draw the matrix for $c_\alpha(m, \xi)$:
\[ \left(\begin{array}{cc} 1 & m \\ 0 & 1 \end{array} \right) \left(\begin{array}{cc} 1 + mX\xi & -m \\ mX^2\xi^2 & 1 - mX\xi \end{array} \right) = 
 \left(\begin{array}{cc} 1 + mX\xi + m^2X^2\xi^2 & -m^2X\xi \\ mX^2\xi^2 & 1 - mX\xi \end{array} \right) \] 
\end{comment}
\end{proof}

\begin{comment}
Useful consequence: $z_{-\alpha}(X^2f(X), g(X)) \in P_\alpha(0)$. Proof: $z_{-\alpha}(X^2f(X), g(X)) = 
x_{-\beta}(-Xg) \cdot x_{-\alpha-\beta}(\epsilon X) \cdot x_{\alpha+\beta}(\epsilon X^3f^2g^2) \cdot x_{\alpha}(-X^2fg^2) \cdot 
z_{-\beta}(-Xg, -Xf) \cdot x_{\beta}(-X^3f^2g)  \cdot x_{-\alpha}(-X^2f) \cdot z_{-\alpha-\beta}(\epsilon X, -\epsilon Xfg)$,
where $\epsilon = N_{-\alpha-\beta,\beta}$. This is a product of generators of types: 1,1,2,2,1,1,4,1.
\end{comment}
   
\begin{lemma}\label{P0_normal} The subgroup $P_\alpha(0)$ is normal in $P_\alpha(*)$. In particular, there is an exact sequence split by the map $m \mapsto x_{-\alpha}(mX)$:
\[\xymatrix{1 \ar[r] & P_\alpha(0) \ar[r] & P_\alpha(*) \ar[r] & M^+ \ar@<5pt>@{-->}[l] \ar[r] & 1}.\]
\end{lemma}
\begin{proof} We need to verify that the conjugate by $x_{-\alpha}(mX)$ to every generator of $P_\alpha(0)$ listed in~Definition~\ref{defP0} still belongs to $P_\alpha(0)$. The assertion is obvious for the generators of types (3) and (4).
...
the latter expression, clearly, is a product of generators of types $4, 1, 1$.
\end{proof}

For $m \in M$ denote by $P_\alpha(m)$ the coset $P_\alpha(0) \cdot x_{-\alpha}(mX)$.
From the above lemma it follows that $P_\alpha(*)$ coincides with the union of all $P_\alpha(m)$, $m\in M$. 
The latter fact allows to make the following definition. 

\begin{df}
Define the map $S_\alpha(a, -) \colon P_\alpha(*) \to \St(\Phi, R[t, t^{-1}])$ on each coset $P_\alpha(m)$, $m \in M$ via the following formula:
...
\begin{lemma}\label{P0_conj} The subgroup $P_\alpha(0)$ is stable under conjugation by $x_\alpha(aX^{-1})$ for arbitrary $a\in R$. \end{lemma}
\begin{proof} As before, we need to verify that the conjugate of every generator of $P_\alpha(0)$ lies in $P_\alpha(0)$. First of all, consider the case $\alpha + \beta \in \Phi$ (only possible for the generator of the first type):
...
Let us verify the assertion for the generator of type (4). Choose $\beta\in \Phi$ such that $\alpha+\beta \in \Phi$ and invoke Lemma~\ref{Zrels}:
\[z_{-\alpha}(X^2f, aX^{-1}) = x_{-\beta}(-a) \cdot x_{-\alpha-\beta}(\epsilon X) \cdot x_{\alpha+\beta}(\epsilon Xf^2a^2) \cdot x_{\alpha}(-a^2f) \cdot z_{-\beta}(-a, -Xf) \cdot x_{\beta}(-aX^2f^2)  \cdot x_{-\alpha}(-X^2f) \cdot z_{-\alpha-\beta}(\epsilon X, -\epsilon af), \] where $\epsilon = N_{-\alpha-\beta,\beta}$. It is clear that the latter expression is a product of generators of types: 2, 1, 1, 2, 2, 1, 1, 1, respectively.

It remains to verify the assertion for the generator $z_\alpha(f, gX)$.
\begin{comment}
\[ z_{\alpha+\beta}(st, \xi) = x_{-\beta}(-s\xi) \cdot x_\alpha(\epsilon s) \cdot x_{-\alpha}(\epsilon st^2\xi^2) \cdot x_{-\alpha-\beta}(-st\xi^2) \cdot z_{-\beta}(-s\xi, -t) \cdot x_{\beta}(-st^2\xi)  \cdot x_{\alpha+\beta}(-st) \cdot z_\alpha(\epsilon s, -\epsilon t\xi) \]  

\[ z_{\alpha+\beta}(- s t, \xi) = x_{-\beta}(-\epsilon s\xi) \cdot x_\alpha(s) \cdot x_{-\alpha}(s t^2\xi^2) \cdot x_{-\alpha-\beta}(s t\xi^2) \cdot z_{-\beta}(-\epsilon s\xi, \epsilon t)) \cdot x_{\beta}(-\epsilon st^2\xi)  \cdot x_{\alpha+\beta}(s t) \cdot z_\alpha( s, t\xi) \]  

\[ z_\alpha(-s, t\xi) = z_{\alpha+\beta}(st, \xi) x_{-\beta}(-\epsilon s\xi) \cdot x_\alpha(s) \cdot x_{-\alpha}(s t^2\xi^2) \cdot x_{-\alpha-\beta}(s t\xi^2) \cdot z_{-\beta}(-\epsilon s\xi, \epsilon t)) \cdot x_{\beta}(-\epsilon st^2\xi)  \cdot x_{\alpha+\beta}(s t) \]  

\[ z_\alpha(s, t\xi) = z_{\alpha+\beta}(-st, \xi) x_{-\beta}(\epsilon s\xi) \cdot x_\alpha(-s) \cdot x_{-\alpha}(-s t^2\xi^2) \cdot x_{-\alpha-\beta}(-s t\xi^2) \cdot z_{-\beta}(\epsilon s\xi, \epsilon t)) \cdot x_{\beta}(\epsilon st^2\xi)  \cdot x_{\alpha+\beta}(- s t) \]  
\end{comment}
Substituting $s := \epsilon f$, $t = -\epsilon g$ and $\xi = X$ in the fifth equality of Lemma~\ref{Zrels} and expressing the rightmost term through others we obtain (here $\beta$ is some other root such that $\alpha + \beta \in \Phi$): \[ z_\alpha(f, gX) = z_{\alpha+\beta}(-fg, X) x_{-\beta}(\epsilon fX) \cdot x_\alpha(-f) \cdot x_{-\alpha}(-f g^2X^2) \cdot x_{-\alpha-\beta}(-f gX^2) \cdot z_{-\beta}(\epsilon fX, \epsilon g) \cdot x_{\beta}(\epsilon fg^2X)  \cdot x_{\alpha+\beta}(- f g),\ \epsilon = N_{\alpha,\beta}. \]  
The factors in the right hand side have types: 2, 1, 2, 4, 1, 1, 1, 2. 
It is clear, that conjugation by $x_{\alpha}(aX^{-1})$ acts trivially on the third factor $x_\alpha(-f)$, 
...
\begin{multline} h = {}^{x_\alpha(aX^{-1})}\!g_0 \cdot \left(x_{\alpha}(aX^{-1}) \cdot x_{-\alpha}(mX)\right) \cdot x_{\alpha}\left(-\frac{aX^{-1}}{1+am}\right) \cdot \{X, 1+am\} \cdot h_\alpha^{-1}(1 + am) = \\
...
\begin{lemma} If $\alpha - \beta \in \Phi$ then for all $g \in \St(\Phi, R[X], XM[X])$ holds $S_\alpha(a, g) \in P(\beta, 0)$. \end{lemma}
\begin{proof}

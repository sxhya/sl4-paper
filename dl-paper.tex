...
This action satisfies the following additional property. If $v = (g, [h], u) \in \overline{V}$ is such that $\overline{p}(v) = \overline{1} = G_M^0$ then for any $h_1 \in \St(\Phi, A[X\inv])$ holds $j_B(h_1) \cdot v = (h_1 \cdot g, [h \cdot h_1^{-1}], u)$ and, moreover, $\overline{p}(j_B(h_1) \cdot v) = \overline{1}$. Here $j_B$ denotes the obvious map $\St(\Phi, A[X\inv]) \to \St(\Phi, B)$.
\end{prop}
\begin{proof}
...
We retain notation of the previous subsection and assume additionally that the map $j_R \colon \St(\Phi, B) \to \St(\Phi, R)$ is injective. This assumption allows us to consider $\St(\Phi, B)$ as a subgroup of $\St(\Phi, R)$.
%Denote by $I$ the ideal $M[X, X\inv]$ of $B$ (it is also an ideal of $R$).
Notice that by~\cref{lem:lemma32} the subgroup $\overline{G}^{\geq 0}_M$ is contained in $\St(\Phi, B)$.

%We denote by $t$ the map $\St(\Phi, R, I) \to \St(\Phi, B)$ from.
\begin{lemma} For $v\in \overline{V}$ and any $g \in \overline{G}^{\geq 0}_M$ holds $\overline{p}(g \cdot v) = g \cdot \overline{p}(v)$. \end{lemma}
\begin{proof} First of all, notice that the assertion of the lemma can be verified directly in the case $g = z_\alpha(f, a)$ for all $a\in A$, $f\in M[X]$. Using~\cref{SR:additivity} and the fact that $x_\beta(X^2f) \in P_\alpha(0)$ for all $\beta \in \Phi$ it is not hard to show that the assertion also holds for $g = z_\alpha(X^2f, aX^{-1})$.

It suffices to verify the assertion of the lemma for the generators $g = z_\alpha(f, \xi)$ of $\overline{G}_M^{\geq 0}$ for all $f \in M[X]$, $\xi\in A[X]$, $\alpha \in \Phi$. We accomplish this by 
induction on the degree of $\xi$ in $X$. The base of induction is clear.

Suppose that the assertion holds for all $g = z_\alpha(f, \xi)$ for which $\xi$ has degree $\leq n$.
...
\end{multline*}
From the first part of the proof and inductive assumption we obtain that the assertion of the lemma holds for all the factors in the right-hand side and, therefore, also holds holds for $g = z_\alpha(f, X\xi)$. It is easy to deduce from this that the assertion also holds for $g = z_\alpha(f, X\xi + a) = x_\alpha(-a) \cdot z_\alpha(f, X\xi) \cdot x_\alpha(a)$. 
\end{proof}
...
\end{theorem}
\begin{proof}
 Since $j_- = j_R j_B$, where $j_B$ is as in~\cref{prop43}, it suffices to show that $j_B$ is injective.
 Set $v_0 = (1, [1], 1) \in \overline{V}$ and suppose $g \in \Ker(j_B)$.
 By~\cref{prop43} holds $v_0 = j_B(g) \cdot v_0 = (g, [g^{-1}], u)$ therefore $g \in G_M^0$ and consequently $g = 1$.
 
 Now suppose $g_+ \in \St(\Phi, A[X])$ and $g_- \in \St(\Phi, A[X\inv])$ are such that $j_-(g_-) = j_+(g_-)$. From~\cref{field-injectivity} it follows that the image $j_+(g_+) = j_-(g_-)$ in $\St(\Phi, k[X, X\inv])$ belongs to $\St(\Phi, k)$ and therefore coincides with the image in $\St(\Phi, k)$ of some $g_0 \in \St(\Phi, A)$. 
 Set $g_+' = g_+g_0^{-1}$ and $g_-' = g_-g_0^{-1}$.
 %Clearly, $g_+' \in \overline{\St}(\Phi, A[X], M[X])$. Set $v = () $
\end{proof}

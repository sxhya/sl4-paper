...
Let us briefly remind the reader definitions and basic properties pertaining to relative Steinberg groups.
...
\end{equation}
It is not hard to show that $C(\Phi, R, I) = 0$ in the case when $I$ is a splitting ideal of $R$, i.\,e. when the canonical map $R \to R/I$ splits, see~\cite[Lemma~8]{S15}. In order to better understand $C(\Phi, R, I)$  we would like to continue the sequence~\eqref{eq:relative-St} to the left. Thus we need to work with unstable $\K_3$-functors for which we choose the definition a la Quillen (cf.~\cite{Pa89})

\begin{df} Suppose that $\St(\Phi, R)$ is perfect and super-perfect (i.\,e. the homology groups $\HH_i(\St(\Phi, R), \ZZ)$ vanish for $i=1,2$.) Denote by $X$ the result of application of Quillen's $+$-construction to the discrete classifying space $B\G(\Phi, R)$ with respect to the elementary subgroup $\E(\Phi, R)$. Now define $\K_3(\Phi, R)$ as the third homotopy group $\pi_3(X)$. \end{df}
%TODO: Give reference on the definition of the plus-construction?

\begin{prop} The group $\K_3(\Phi, R)$ is isomorphic to $\HH_3(\St(\Phi, R), \ZZ)$. In particular, it does not depend on the isogeny form of $\G(\Phi, R)$ used to define it. Moreover, for a ring epimorphism $R \to R/I$ there is a functorial exact sequence
...
\begin{lemma}
 Let $F$ be a field of characteristic $\neq 2$. Then for $\ell \geq 7$ the canonical map $\K_3(\rD_\ell, F[X]) \to \K_3(\rD_\ell, F[X^{\pm 1}])$ is surjective.
\end{lemma}
\begin{proof}

...
\end{lemma}
\begin{proof}
 We have to show that $[T_\alpha(aX^{-1}), T_\beta(b)](g, h, u) = (g, h, u)$ for $\beta\in Z(\alpha)$, $a, b\in A$.
 \begin{multline}
  T_\beta(b) T_\alpha(aX^{-1}) T_\beta(-b) T_\alpha(-aX^{-1})(g, h, u) = \\
  = T_\beta(b) T_\alpha(aX^{-1}) \left(x_\beta(-b) x_\alpha(-aX^{-1}) \cdot g, h \cdot x_{\alpha}\left(\tfrac{aX^{-1}}{1-am}\right) \cdot x_\beta(b), u(1-am)\right)
 \end{multline}
\end{proof}

...
It is not hard to verify that tha map $[u, x] \mapsto (u, x - \langle\langle u \rangle\rangle)$ defines an isomorphism of $P(k)$ and $\K_1^\mathrm{MW}(k)$.

\begin{comment}
Under additonal assumption $\mathrm{char}(k)\neq 2$ the assertion of the lemma in the symplectic case can be obtained as a chain of isomorphisms
\[ \K_2(\rC_\ell, k[X, X\inv]) \cong \GW_2^2(\Phi, k[X, X\inv]) \cong \GW_2^2(k) \oplus \GW_1^1(k) \cong \K_2^\mathrm{MW}(k) \oplus \K_1^\mathrm{MW}(k). \]
Here the first 
corollary of~\cite[Lemma~4.1.1]{AF17} and \cref{bass-ft} below. 
\end{comment}
\end{proof}

...
Notice that $p(T_\alpha(a + Xf) \cdot (g, h, u)) = x_\alpha(Xf) \cdot {}^{x_\alpha(a)}p(g, h, u) \in \overline{G}_M^{\geq 0},$ therefore
 the right-hand side of~\eqref{T_leq0} lies in $V$. Finally, the fact that~\eqref{T_leq0} gives rise to a well-defined map $\overline{V} \to \overline{V}$ follows
  from the following identity:
\[T_\alpha(a + Xf) \cdot ((g, h, u) \cdot h_0) = (T_\alpha(a + Xf) \cdot (g, h, u)) \cdot {}^{x_{\alpha}(a)}\!h_0.\]  

It is easy to see that the expression in the right-hand side of~\eqref{T_leq0} does not depend on the choice of the representative for $h$.
...
\begin{proof}
 ...
 Notice that we can not naively invoke~\eqref{T_1} for the second time because the value of the function $p$ on the triple $(x_\alpha(aX^{-1})\cdot g ,\ h',\ u \cdot (1 + am))$ does not belong to $P_\alpha(*)$.
...
\begin{equation*} \ldots = \left( [x_\beta(b), x_{\alpha}(aX^{-1})] \cdot g, [h'' \cdot x_{\alpha}\left(-a(1-am)X^{-1}\right) \cdot x_{\beta}(-b_0)], u \right) = \left(g, [h], u\right), \end{equation*}
...
\begin{proof} We need to show that for $a\in A$, $f \in M[X]$ and $\alpha, \beta \in \Phi$ forming an obtuse angle one has 
 \[ [T_\beta(Xf),\ T_\alpha(aX^{-1})] (g, [h], u) = \left(x_{\alpha+\beta}(N_{\beta,\alpha} \cdot af) \cdot g, [h], u\right) = T_{\alpha+\beta}(N_{\beta, \alpha} \cdot af) (g, [h], u). \]
...
From~\cref{K-a-b-cor} and the proof of the first part of~\cref{lem:orbit-action} it follows that \[p(g, h, u) \in j_+\left(K(A[X], M[X])\right) \cap P_\alpha(m) \cap P_{\beta}(m')\text{ for some }m,m' \in M.\]

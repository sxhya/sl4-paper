...
Denote by $V$ the subset of $\St(\Phi, A[X, X^{-1}]) \times C \times (1 + M)^*$ consisting of those triples $(g, h \overline{G}^0_M, u)$ for which $\pi(g, h, u) = g \cdot j_-(h) \cdot \{ X, u \} \in \overline{G}_M^{\geq 0}$. The correctness of this definition is clear from the above commutative diagram.

The next step of the proof is to define for $\alpha \in \Phi$, $a \in A$ a  function $T_\alpha(aX^{-1}) \colon V \to V$. This function is defined for those triples $(g, h, u)$ for which $\pi(g, h, u) \in P_\alpha(*) \subseteq \overline{G}_M^{\geq 0}$. If $\pi(g, h, u) \in P_\alpha(m)$ for some $m \in M$, then $T_\alpha(aX^{-1})$ is defined via the following identity:

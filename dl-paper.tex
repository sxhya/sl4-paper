...
\begin{proof}
 ...
 \begin{df} Denote by $Z$ the subgroup of $\St(\Phi, A[X, X^{-1}])$ generated by elements $z_\gamma(f, X\xi)$, where $\gamma \in Z_+(\alpha) \setminus \{ \alpha - \beta \}$.
 \end{df}
  
\begin{lemma} \label{image-K-a-b} The image of $K(\alpha, \beta)$ under $S_{\alpha}(a, -)$ is contained in the subgroup of $\St(\Phi, A[X, X^{-1}])$  generated by $K(\alpha, \beta)$ and $Z$.
\end{lemma}
\begin{proof} 
...
\begin{lemma} For $g \in K(A[X], M[X])$ there exist $m, m' \in M$ such that \[S_\alpha(a, g)\cdot  h^{-1}_\alpha(1 + am) \cdot x_{\alpha-\beta}(-N_{\alpha,-\beta}\cdot m'a) \in P\left(\beta, \frac{m'}{1 + am}\right). \] \end{lemma}
\begin{proof}
...
\begin{proof}
 By~\eqref{Z2}--\eqref{Z4} of~\cref{Zrels} the conjugate to the generator $g=z_\gamma(Xf, \xi)$ of type 1 by $x_{\beta-\alpha}(b)$ either coincides with $g$ or is a product of elements of type 4, 1, 1. 
...
\begin{lemma} 
 For $g \in K(\alpha, \beta)$ and $a, b\in A$ the conjugate to
 $S_\alpha(a, g)$ by $x_{\beta-\alpha}(b)$ belongs to $P_\alpha(0)$.
\end{lemma}
\begin{proof}
 By~\cref{image-K-a-b} and~\cref{conj-K-a-b} it remains to show that the conjugate to each generator $g = z_\gamma(f, X\xi)$, $\gamma \in Z_+(\alpha) \setminus \{ \alpha - \beta \}$ of $Z$ by $x_{\beta-\alpha}(b)$ belongs to $P_\alpha(0)$.
...
\begin{lemma}
 For $g \in K(A[X], M[X]) \cap P_\alpha(m) \cap P_\beta(m')$ holds 
 \[ x_{\beta - \alpha}(b) \cdot S_\alpha(a, g) \cdot h_{\alpha}^{-1}(1+am)\cdot h_{\beta - \alpha}((1 + N_{\alpha,-\beta} \cdot abm')^{-1})\cdot x_{\beta - \alpha}(-b(1 + N_{\alpha,-\beta} \cdot abm')) \in P\left(\alpha, \frac{m - bm'}{1+am}\right). \]
\end{lemma}
\begin{proof}
Set $\epsilon = N_{\alpha, -\beta}$, $c = 1 + \epsilon a b m'$, $s_0 = \langle -a, m \rangle$, $s_1 = \langle -\epsilon am', -bc^{-1} \rangle$.
By~\cref{K-a-b} we can write $g = g_0 \cdot $ for some $m, m' \in M$ hence by ??? we obtain that
 \begin{multline} \nonumber 

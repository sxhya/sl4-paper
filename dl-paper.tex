...
\begin{rem} \label{rem:recognition}
  From the above definition it follows that an element $z_\gamma(Xf, \xi)$ is a generator of type 1 provided $\gamma\neq\pm\alpha$ and $\gamma \not \perp \alpha$. Since $x_\gamma(Xf) = z_\gamma(Xf, 0)$, it follows that $x_{\gamma}(Xf)$ is also a generator of type 1. In particular, if $\beta$ is such that $\alpha + \beta \in \Phi$ then $x_{\pm(\alpha+\beta)}(Xf)$ is a generator of type 1 and $x_{\alpha+\beta}(f)$ is a generator of type 2.
\end{rem}
...
\end{prop} 
\begin{proof}
It is clear that $K(A[X], M[X]) \supseteq \widetilde{K}(\alpha, \beta) \cdot X_{-\alpha}(M \cdot X) \cdot X_{-\beta}(M \cdot X)$. Let us prove the reverse inclusion. Fix an element $g\in K(A[X], M[X])$. We need to show that $g$ lies in the product of subgroups in the right-hand side.
...
\end{corollary}
\begin{proof} It is clear that $\widetilde{K}(\alpha, \beta) \subseteq \widetilde{P}_{\alpha, M}(0) \cap \widetilde{P}_{\beta, M}(0) \cap K(A[X], M[X])$. 
Let us prove the reverse inclusion. Let $g$ be an element of the intersection of subgroups in the right-hand side.
...
We need one more technical definition. We denote by $Z_{\alpha, \beta}$ the subgroup of $\St(\Phi, A[X, X^{-1}])$ generated by elements $x_\gamma(f)$, where $\gamma \in Z_+(\alpha) \setminus \{ \alpha - \beta \}$ and $f \in M[X]$. To simplify notation we call the generators of the subgroup $Z_{\alpha,\beta}$ ``generators of type Z``. 
...
\begin{proof} We need to verify that the conjugate by $x_\alpha(aX^{-1})$ of every generator from~\cref{def:Kab} lies in the specified subgroup. 

Let $z_\gamma(Xf, \xi)$ be a generator of type $1$ for some $\gamma \in \Phi \setminus \Psi$.
...
By the choice of $\gamma$ both $\alpha + \gamma$ and $\alpha - \gamma$ lie in $Z_+(\alpha) \setminus \{\alpha - \beta\}$, therefore in both cases the expressions in the right-hand side are products of generators of type Z, Z, 1.

A similar computation using \eqref{C1}--\eqref{C3} of \cref{Crels} shows that the conjugate of a generator of type 5 is either the generator itself or a product of generators of type Z, Z, 5.

Let us verify the assertion for the generators of types 2--4.
...
\end{align*}
and the expressions in the right-hand side are products of generators of type 2, 4 and 4, Z.

It remains to verify the assertion for the generators $x_{-\alpha}(X^2f) $ of type 2.
...
The first 7 factors in the right-hand side are generators of type 1, Z, 1, 2, 1, 1, Z.
The remaining last factor $z_\delta(af, -X)$ can be rewritten as $x_{\delta}(af) \cdot c_{\delta}(-af, X)$ (cf.~\cref{rem:c}) and therefore is a product of generators of type Z and 5. \end{proof}
...
\begin{proof} Since any subgroup $H \leq \St(\Phi, A[X], M[X])$ satisfies $H = \left(H \cap K(A[X], M[X])\right) \rtimes ev^*_{X=0}\left(H\right)$, we obtain from~\cref{rem:DS} that
$\widetilde{P}_\alpha(0) =  \left(\widetilde{K}(\alpha, \beta) \cdot X_{-\beta}(M \cdot X)\right) \rtimes Z_\alpha(A, M).$
 
Thus, in view of~\cref{image-K-a-b} it remains to show that $X_{-\beta}(MX)^{x_\alpha(aX^{-1})} \subseteq P_\alpha(0)$ and $Z_{\alpha, \beta}^{x_\alpha(aX^{-1})} \subseteq P_\alpha(0)$.
\end{proof}

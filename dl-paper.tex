...
\begin{lemma}\label{superfluous-relations}
 For every simply-laced root system $\Phi$ of rank $\geq 3$ and every $n \geq 1$ one can exclude from the presentation of $\St^{\leq n}(\Phi, B)$ all the relators $\text{R3}_{d,e}^\bot$ for which $\max(0,d) + \max(0,e) > 1$.
\end{lemma}
\begin{proof}
...
To see that $\text{R3}^\bot_{d,e}$ is superfluous one must iteratively apply one of the above observations.
Notice that the value $\max(0,d,e)$ decreases strictly whenever $e > 1$. 
Finally, by the first observation $\text{R3}^\bot_{1,1}$ follows from $\text{R3}^\bot_{0,0}$.
\end{proof}

\begin{lemma} For $\Phi=\rA_{\geq 4}, \rD_{\geq 5}, \rE_{6,7,8}$ and $n \geq 1$ the map $i_\Phi\colon \St^{\leq n}(\Phi, B) \to \St^{\leq n+1}(\Phi, B)$ induced by the natural embedding of generators, is an isomorphism. In particular, the natural map $\St^{\leq 1}(\Phi, B) \to \St(\Phi, B)$ is an isomorphism. \end{lemma}
\begin{proof}
 The special case $\Phi=\rA_{\geq 4}$ of the assertion is due to Tulenbayev, see~\cite[Lemma~3.3]{Tu83}. Thus, it remains to deduce from it the assertion in the cases $\Phi=\rD_\ell,\rE_\ell$.

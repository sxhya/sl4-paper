...
\subsection{Relative Steinberg groups} \label{sec:quillen}
In this subsection we recall the definitions and basic facts pertaining to relative Steinberg groups as defined by J.-L.~Loday in~\cite{Lo78},
 where a theory of relative central extensions is developed and then applied to the usual (stable) Steinberg group $\St(R)$ from algebraic K-theory (see e.\,g.~\cite[Ch. III \S~5]{Kbook}).
The main goal of this subsection is to show that Loday's result can be applied to the unstable Steinberg groups, and that
 the resulting relative groups enjoy the same good properties as their stable counterparts.

%Some of the results of this subsection have been already mentioned in~\cite{S15} (cf. e.\,g. Corollaries 3--4).
%Nevertheless, we chose to reproduce them with greater amount of detail for the reason that~\cref{lem:prop41} depends on the unstable exact sequence of.

First of all, let us recall the definition of the category of (commutative) pairs $\catname{Pairs}$.
The objects of $\catname{Pairs}$ are pairs $(R, I)$, in which $R$ is a commutative ring and $I$ its ideal.
By definition, a morphism $f \colon (R, I) \to (R', I')$ in the category of pairs corresponds to a ring map $f \colon R \to R'$ such that $f(I) \subseteq I'$.
Notice that the mapping $(R, I) \mapsto (R \to R/I)$ defines a functor from $\catname{Pairs}$ the morphism category $\catname{CRings}^\rightarrow$.

There is an obvious fully faithful embedding $\catname{CRings} \to \catname{Pairs}$ sending $R$ to $(R, R)$.
For a given functor $S \colon \catname{CRings} \to \catname{Groups}$ a {\it relativization} of $S$ is any functor $\widetilde{S} \colon \catname{Pairs} \to \catname{Groups}$
 extending $S$ in the obvious sense. Relativization of a functor is not unique.

Recall that {\it the double ring} $D_{R, I}$ of a pair $(R, I)$ is, by definition, the pullback ring $R \times_{R/I} R$.
In other words, it is the ring consisting of pairs of elements of $R$ congruent modulo $I$.
Denote by $p_0, p_1, \Delta$ the two obvious projections and the diagonal map $\xymatrix{D_{R,I} \ar@<1pt>[r] \ar@<-2.5pt>[r] & \ar@<-4pt>[l] R}$.

Let $S \colon \catname{CRings} \to \catname{Groups}$ be a functor.
Set $G_i = \Ker(S(p_i))$ and define {\it Loday's relativization} $S(R, I)$ as $ G_0 / [G_0, G_1]$. The map $S(p_1)$ induces a natural transformation $S(R, I) \to S(R)$. We denote this map by $\mu = \mu_{R,I}$ and its kernel by $C_S(R, I)$.
\begin{equation} \label{LodayRelativization}
 \xymatrix{ 1 \ar[r] & C_S(R, I) \ar[r] & S(R, I) \ar^{\mu}[r] & S(R) \ar[r] & S(R/I) \ar[r] & 1}
\end{equation}

\begin{df}
By definition, the {\it relative Steinberg group} $\St(\Phi, R, I)$ is the result of application of Loday's relativization to the functor $\St(\Phi, -)$. Notice that $\St(\Phi, R, I)$ is not a subgroup of $\St(\Phi, R)$ but rather its central extension by the abelian group $C_{\St(\Phi, -)}(R, I)$.
For shortness we rename the latter group to $C(\Phi, R, I)$.
\end{df}

Our next goal is to obtain a more concrete description of the group $C(\Phi, R, I)$.
We need to recall some additional notation and terminology.
...
Now if $\nu \colon N \twoheadrightarrow Q$ is a surjective map of groups, a {\it relative central extension of $\nu$} is, by definition,
a crossed module $(M, N, \mu)$ such that the cokernel of $\mu$ is $\nu$.
The mentioned groups and morphisms fit into the following diagram.
\begin{equation} \label{RelativeCentralExtension}
 \xymatrix{ 1 \ar[r] & L \ar[r]^{\ker(\mu)} & M \ar^{\mu}[r] & N \ar^{\nu}[r] & Q \ar[r] & 1} \end{equation}

A morphism $(M, \mu) \to (M', \mu')$ between two relative central extensions of $\nu$ is, by definition, an $N$-group homomorphism $f\colon M \to M'$ such that $\mu' f = \mu$. Denote by $Ext(Q, N, L)$ the set of isomorphism classes of relative central extensions of $\nu$ by an abelian group $L$.
It turns out that there is a well-defined bijection $\xi \colon Ext(Q, N, L) \to \HH^3(Q, N; L)$ which is called {\it characteristic class} (see~\cite[Th{\'e}or{\`e}me~1]{Lo78}).
 
 
For the rest of this subsection $G, P, S$ denote three group-valued functors on the category of commutative rings satisfying the following assumptions:
\begin{enumerate} [label=(A\arabic*)]
 ...
 \item \label{req:uce} There is a natural transformation $\varphi \colon S \to P$ such that $ \varphi_R \colon S(R) \to P(R)$ is a universal central extension for all $R$.
 %This means that $\varphi_R$ is a certain surjective and that its kernel is contained in the center of $S(R)$.
 \item \label{req:coeq} For every pair $(R, I)$ the coequalizer of the pair of maps $S(p_0), S(p_1) \colon S(D_{R, I}) \to S(R)$ is precisely the map $S(R) \to S(R/I)$.
\end{enumerate}
\begin{example}
 The obvious triple of functors satisfying the above assumptions is as follows.
 Take the infinite general linear group functor $\mathrm{GL}(-)$ as $G$, its infinite elementary subfunctor $E(-)$ as $P$
  and the infinite Steinberg group functor $\St(-)$ as $S$. %TODO: Why these 4 propeties hold? What is a relative central extension.
\end{example}

\begin{lemma}
 Suppose $(G, P, S)$ is a triple satisfying properties \text{A1}--\text{A4}.
 The map $\mu \colon S(R, I) \to S(R)$ is the universal relative central extension of $\nu \colon S(R) \to S(R/I)$, i.\,e. the initial object of the category of relative central extensions.
 
 There is a natural isomorphism between the kernel $C_S(R, I)$ of $\mu$ and the third relative homology group $\HH_3(S(R), S(R/I), \ZZ)$.
\end{lemma}
\begin{proof}
From~\ref{req:left-exact} and $\varphi(G_i) \subseteq \Ker(G(p_i))$ we conclude that $G_0 \cap G_1 \subseteq \Ker(\varphi \colon S(D_{R, I}) \to G(D_{R, I}))$ therefore $G_0 \cap G_1$ is a central subgroup of $S(D_{R, I})$. From~\ref{req:uce} and~\cite[Ch.~III, Theorem~5.4]{Kbook} we conclude that $\HH_1(S(R), \ZZ) = \HH_2(S(R), \ZZ) = 0$ for all rings $R$.

Using~\cite[Proposition~6]{Lo78} we obtain that the extension $S(R, I) \to S(R)$ is a universal relative central extension of the coequalizer $\mathrm{coeq}(d_0, d_1)$, which coincides with $S(R) \to S(R/I)$ by~\ref{req:coeq}. 

Now set $C = \HH_3(S(R/I), S(R), \ZZ)$, $L = C_S(R, I)$.
Since 
Recall from the proof of \cite[Th{\'e}or{\`e}me~2]{Lo78} that there is a map $\alpha \colon C \to L.$
Under t

of~. To see that this isomorphism is natural in $(R, I)$ notice that the map $ $
\end{proof}
...
\end{prop}
\begin{proof}
 The first claim follows from~\ref{req:subfunc} and~\ref{req:uce} using the standard properties of the plus-construction, see \cite[\S~IV.1]{Kbook}   (cf. Exercises~1.8--1.9 ibid.). %provided one verifies that all the intermediate isomorphisms are natural in $R$.

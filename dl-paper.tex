...
\begin{abstract} We prove the $\mathbb{P}^1$-glueing theorem (aka Horrocks theorem) for the unstable even-dimensional orthogonal Steinberg groups. The $\mathbb{P}^1$-glueing theorem for Steinberg groups is one of the principal ingredients needed in the proof of the $\K_2$-analogue of Serre's problem, positive solution of which currently is only known in the linear case. \end{abstract}
\maketitle
...
Before we proceed with the proof of~\cref{thm:P1glueing} let us briefly recall the relevant notation. As before, $A$ denotes an arbitary local ring with maximal ideal $M$ and $I = M[X, X\inv]$ is an ideal of both $R=A[X, X\inv]$ and its subring $B = A[X\inv] + M[X]$.
...
\begin{proof} Since $j_- = j_R j_B^-$, where $j_B^-$ is as in~\cref{prop43}, it suffices to show that $j_B^-$ is injective.
 ...
 Notice that the homomorphism $j_+i_+ = j_-i_-$ is split by the evaluation homomorphism $ev_{X=1}^*$ and therefore is injective. Thus, we conclude that $g = 1$.
 
 Now suppose that $g_+ \in \St(\Phi, A[X])$ and $g_- \in \St(\Phi, A[X\inv])$ are such that $j_B^-(g_-) = j_+(g_+)$.
 ...
 \[ [j_+(h_+)^{-1}, h_-, 1] = j_+(h_+)^{-1} \cdot [1, h_-, 1] = j_+(h_+)^{-1} j_B^-(h_-) \cdot [1, 1, 1] = [1, 1, 1], \]
  therefore, by the definition of~$\overline{V}_T$, $h_- = i_-(\gamma_1)$ for some $\gamma_1 \in G_M^0$. 

Denote by $T$ the isomorphism $\St(\Phi, A[X]) \to \St(\Phi, A[X\inv])$ induced by the obvious isomorphism of polynomial rings sending $X$ to $X^{-1}$.
Repeating the above argument (now with $T(h_+)$ and $T^{-1}(h_-)$ playing the role of $h_-$ and $h_+$, respectively) we obtain that $h_+ =i_+(\gamma_2)$ for some $\gamma_2 \in G_M^0$. Since $j_+i_+ = j_- i_-$ is injective, we conclude that $\gamma_1 = \gamma_2$.
Thus, we have shown that $g_+$ and $g_-$ are the images of $\gamma_1 \cdot g_0 \in \St(\Phi, A)$ under $i_+$ and $i_-$, respectively.
 \end{proof}

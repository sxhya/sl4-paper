...
\subsection{Lemma on the presentation of Steinberg groups}
...
We consider $A[X, X\inv]$ as a $\mathbb{Z}$-graded ring in which $X^{-1}$ has degree $1$.
We denote by $B = B(A, I)$ the subring $A[X^{-1}] + I[X] \subseteq A[X, X\inv]$ with the induced grading.
As an $A$-module $B$ decomposes as $\oplus_{d\in\mathbb Z}B_d$ where $B_d=I \cdot X^d$ for $d>0$, and $B_d=A \cdot X^d$ for $d\leq0$.
Obviously, $B = A[X^{-1}]$ in the case $I=0$ and $B = A[X, X^{-1}]$ in the case $I=A$.
...
Whenever the coefficient $\xi$ of a Steinberg generator $g = x_\alpha(\xi)$ of $\St(\Phi, B)$ is a homogeneous element of $B$, i.\,e. $\xi \in B_d$ for some $d \in \mathbb{Z}$,
 we call the corresponding generator $g$ homogeneous of degree $d$.
...
For $n\geq 1$ we define ``truncated'' Steinberg groups $\St^{\leq n}(\Phi, B)$ by the set of homogeneous Steinberg generators of degree $\leq n$
modulo relations $\text{R}1_{d}$, $\text{R3}_{d, e}$ for all $d,e \leq n$ and $\text{R2}_{d,e}$ for those $d,e \leq n$ for which $d+e \leq n$.

It will be convenient for us to distinguish between the cases when depending on whether the roots $\alpha, \beta$ appearing in $\text{R3}_{d,e}$ are orthogonal to each other or form a sharp angle.
In the former case we denote such relator by $\text{R3}^\bot_{d,e}$, and in the latter case by $\text{R3}^\angle_{d,e}$.

The following lemma is inspired by the proof of the main result in ???.
Basically, it says that most of the relators $\text{R3}^\bot_{d,e}$ in the presentation of $\St^{\leq n}(\Phi, B)$ are superflous.
\begin{lemma}\label{superfluous-relations}
 
\end{lemma}
\begin{proof}
We will use the following commutator identities (cf Rehmann-Soule).
\begin{align} \label{eq:H1ii}  [ab, c] = {}^a[b, c] \cdot [a,c];&\\ %= [a,[b,c]]\cdot [b,c] \cdot [a,c];&\\
 \label{eq:H1iii} [a,c]   = 1    \text{ implies } [a, [b,c]] = [[a,b],{}^bc].& \end{align}

In the case $0 < d \leq e \leq n$ the assertion follows from the following computation:
...
In the case $d \leq 0 \leq e \leq n$
\begin{align*} 
 [x_\alpha(\xi), x_{\gamma}(bt^{e})] = [x_\alpha(\xi), [x_{\beta+\gamma}(b't^{e-1}), x_{-\beta}(t)]] & \text{ by~$\text{R2}_{e-1,1}$ } \\
 = [[x_\alpha(\xi), x_{\beta+\gamma}(b't^{e-1})], {}^{x_{\beta+\gamma}(b't^{e-1})}\!x_{-\beta}(t)] & \text{ by \eqref{eq:H1iii} and $\text{R3}^\angle_{d,1}$ } \\
 = {}^{x_{\beta+\gamma}(b't^{e-1})}\![x_{\alpha+\beta+\gamma}(b't^{e-1}\xi), x_{-\beta}(t)] & \text{ by $\text{R2}_{d,e-1}$ and $\text{R3}^\angle_{e-1,e-1}$  } \\
 = 1 & \text{ by $\text{R3}^\bot_{d+e-1,1}$. \qedhere} \end{align*}
\end{proof}
...
\end{multlined}
\end{gather*}
The expressions in the right-hand sides of the above formulae are clearly symmetric under the action of the transposition $(a, \alpha) \leftrightarrow (b, \beta)$, which implies the desired relation.
\end{proof}

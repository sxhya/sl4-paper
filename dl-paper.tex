...
To be able to write an explicit formula for this action we need two major ingredients. The first one is a certain presentation of $G$ involving much fewer generations and relation, see~\cref{lemma33}. The other ingredients are certain subgroups $P_\alpha(0)$, $P_\alpha(*)$ of $\St(\Phi, A[X, X\inv])$, which are modeled after the nameless groups from~\cite[Lemma~3.4]{Tu83}. Their precise definition and immediate properties are given in section~\ref{sec:Pa0-basic}, while the facts about them, which are needed to verify that the action of $G$ satisfies Steinberg relations, are placed in~\cref{sec:Pa0-Steinberg}.
...
\subsection{The subgroups \texorpdfstring{$P_\alpha(0)$}{P(0)}, \texorpdfstring{$P_\alpha(*)$}{P(*)} and their properties.} \label{sec:Pa0-basic}
...
\subsection{Construction of a torsor for \texorpdfstring{$\St(\Phi, B)$}{St(B)}.} \label{sec:V-construction}
...
The above isomorphism allows us to treat $\overline{V}$ and $\overline{V}_T$ like the same object, for which we can interchangeably use either of the two notations,
...
\subsection{Proof of the \texorpdfstring{$\mathbb{P}^1$}{P\textonesuperior}-glueing theorem.} \label{sec:P1glueing}

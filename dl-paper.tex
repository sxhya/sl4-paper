...
\end{lemma}
\begin{proof} 
It suffices to verify the assertion for the generators $g = z_\alpha(f, \xi)$ of the subgroup $\overline{G}^{\geq 0}_M$ for all $f \in M[X]$, $\xi\in A[X]$. We accomplished this by induction on the degree of $\xi$ in $X$.
It is obvious, that the assertion holds in the case $\xi = a \in A$. 

Suppose that the assertion is true for all $g_1 = z_\alpha(f, \xi)$ with $\xi$ of degree $\leq n$.
Substituting $s := fX$, $\eta := X^{-1}$, $\xi := X\xi$ in~\eqref{Z5} we obtain the following equality in $\St(\Phi, A[X, X\inv], M[X, X\inv])$:
\begin{multline}
 z_{\alpha+\beta}(f, X\xi) = x_\alpha(\epsilon Xf) \cdot x_{-\beta}(-faX^2) \cdot x_{\beta}(af) \cdot x_{\alpha+\beta}(f) \cdot \\ \cdot z_\alpha(-\epsilon Xf, -\epsilon \xi) \cdot x_{-\alpha}(-\epsilon \xi^2fX) \cdot x_{-\alpha-\beta}(- f \xi^2X^2) \cdot z_{-\beta}(X^2f\xi, -X^{-1})
\end{multline}
It is not hard to verify the assertion for all the factors in the right hand side of the above expression
 (except for the fifth factor, for which it is precisely the inductive assumption). It remains to see that
  $t(z_\alpha(f, X\xi + a)) = x_\alpha(-a) \cdot t(z_\alpha(f, X\xi)) \cdot x_\alpha(a)$.
\end{proof}

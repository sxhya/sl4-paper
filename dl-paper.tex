...
The constants $N_{\alpha \beta i j}$ appearing in the right-hand side of \eqref{rel:CCF} are integers equal to $\pm 1,\pm 2,\pm 3$, they are called the {\it structure constants} of the Chevalley group $G_{sc}(\Phi, R)$. There is a degree of freedom in the choice of signs of the structure constants. Several different methods have been proposed for choosing signs of these constants, see e.\,g.~\cite{V00}, \cite[\S~9]{VP}.
...
Let $R=[x_\alpha(at^d),\ x_\gamma(bt^e)] = 1$ be a relation of type $\text{R3}^\bot_{d,e}$ for some $\alpha\perp\gamma$.
...
\begin{lemma} For $\Phi=\rA_{\geq 4}, \rD_{\geq 5}, \rE_{6,7,8}$ and $n \geq 1$ the map $i_n\colon \St^{\leq n}(\Phi, B) \to \St^{\leq n+1}(\Phi, B)$ induced by the natural embedding of generators, is an isomorphism. In particular, the obvious map $\St^{\leq 1}(\Phi, B) \to \St(\Phi, B)$ is an isomorphism. \end{lemma}
\begin{proof}
 The special case $\Phi=\rA_{\geq 4}$ of the assertion has been proved by Tulenbayev (see~\cite[Lemma~3.3]{Tu83}) 
 and therefore can be used to deduce the assertion in the cases $\Phi=\rD_\ell,\rE_\ell$.

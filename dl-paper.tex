...
Let $g = z_\beta(Xf, \xi)$ be a generator of type 1 such that $\alpha - \beta \in \Phi$. By~\cref{Zrels}(2) we get that \begin{equation} z_{\beta}(Xf, \xi) ^ {x_{-\alpha}(mX)} =  x_{-\alpha} (- mX^2f\xi) \cdot x_{\beta-\alpha} (N_{\beta, -\alpha}\cdot mX^2f) \cdot z_{\beta}(Xf, \xi). \label{eq3-1} \end{equation}
...
Now consider the case when $g = z_\beta(f, X\xi)$ is a generator of type $2$. Again by~\cref{Zrels}(2)
\begin{equation}
...
Now let $g = z_{\beta}(Xf, \xi)$ be a generator of type 1 in the case $\alpha + \beta \in \Phi$. By~\cref{Zrels}(3) 
...
\begin{df} 
Define the following two subgroups of $\overline{\St}(\Phi, R, M[X, X^{-1}])$:
\[ P_{\alpha}(0) = j_+(\widetilde{P}_{\alpha, M}(0)), \ P_{\alpha}(*) = j_+(\widetilde{P}_{\alpha, M}(*)), \]
where $j_+ \colon \overline{\St}(\Phi, A[X], M[X]) \to \overline{\St}(\Phi, R, M[X, X^{-1}]$ is the obvious embedding.
...
It follows immediately from the definition of $S_\alpha(a, -)$ that for $g_1 \in P_\alpha(m)$, $g_2 \in P_\alpha(*)$ one has
...
\begin{proof} First of all, notice that the assertion of the lemma can be verified directly in the case $p_1 = z_\alpha(f, a)$ for all $a\in A$, $f\in M[X]$. Using~\eqref{eq:Smult} and the fact that $x_\beta(X^2f) \in P_\alpha(0)$ for all $\beta \in \Phi$ it is not hard to show that the assertion also holds for $p_1 = z_\alpha(X^2f, aX^{-1})$.

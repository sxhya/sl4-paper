...
\begin{itemize}
 \item $A$ is an arbitarary commutative ring and $M$ is an ideal of $A$. 
 In Section~\ref{sec:presentation} there are no restrictions on $(A, M)$, but in later sections the pair $(A, M)$ is assumed to be local.
 \item $R$ denotes the Laurent polynomial ring $A[X, X^{-1}]$. It will be convenient for us to set $t = X^{-1}$, so also $R = A[t, t^{-1}]$.
 ...
 \item $I$ denotes the ideal $M[t, t^{-1}]$ of $R$. $I$ can also be considered as an ideal of $B$.
\end{itemize}

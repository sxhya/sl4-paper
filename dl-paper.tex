...
\begin{proof}
 ...
 It remains to notice that in~\eqref{eq:Zconj1}--\eqref{eq:Zconj3} after evaluating at $X=0$ the only terms that remain 
...
Our next goal is to determine what happens with the generators of $K(A[X], M[X])$ under the map $S_\alpha(a, -)$.
At first we could look at what happens with $K(A[X], M[X]) \cap P_\alpha(0)$ under $S_\alpha(a, -)$.
We notice the following elements:
\begin{itemize}
 \item $x_\alpha(c)$, $c \in A$;
 \item $x_{\alpha+\beta}(d)$, $\alpha+\beta \in \Phi$
\end{itemize}

\[ x_\alpha(c), , x_{\alpha-\beta}(d) \]

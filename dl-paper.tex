...
\begin{multline*} p\left(x_\beta(-b) \cdot x_\alpha (-a X^{-1})\cdot  g,\ h',\ u (1 - am) \right) = \\ =
 x_\beta(-b) \cdot S_\alpha(-a, p(g,h,u)) \cdot h_{\alpha}^{-1}(1-am)\cdot h_{\beta}(c^{-1}) \cdot x_{\beta}(bc) \in P_\alpha(\tfrac{m+\epsilon bm'}{1-am}). \end{multline*}
Thus, we can continue~\eqref{R2_0_1-computation} using~\eqref{T_1}--\eqref{T_leq0}:
\begin{equation} \label{R2_0_1-computation2} \ldots = \left([x_\beta(b),\ x_\alpha(aX^{-1})]\cdot g,\ [h''],\ uc\right) = T_{\alpha+\beta}(-\epsilon abX^{-1})(g,\ [h],\ u), \end{equation} where the last equality is obtained via~\eqref{eq:conj-h-x} as follows:
 \begin{multline} \nonumber
...
\end{enumerate}
\end{prop}
\begin{proof} By~\cref{superfluous-relations} and Lemmas~\ref{R1_d}--\ref{R2_0_1} the action of $\St^{\leq 1}(\Phi, B)$ on $\overline{V}$ given by~\eqref{T_1} and~\eqref{T_leq0} is well-defined. On the other hand, the group $\St^{\leq 1}(\Phi, B)$ is isomorphic to $\St(\Phi, B)$ by~\cref{lemma33} (set $t = X^{-1}$).

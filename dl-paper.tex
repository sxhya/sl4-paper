...
\begin{rem} \label{rem:recognition}
  From the above definition it follows that an element $z_\gamma(f, \xi)$ is a generator of type 1 provided $X \divides f$, $\gamma\neq\pm\alpha$ and $\gamma$ is not orthogonal to $\alpha$.
  Since $x_\gamma(f) = z_\gamma(f, 0)$, it follows that under the same assumptions $x_{\gamma}(f)$ is also a generator of type 1.
  In particular, if $\beta, \alpha - \beta \in \Phi$ then $x_{\pm(\alpha-\beta)}(Xf)$ is a generator of type 1.
  
  Similarly, if $\gamma - \alpha \in \Phi$ and $X \divides \xi$ then $z_\gamma(f, \xi)$ is a generator of type 2.
  In particular, if $\beta, \alpha + \beta \in \Phi$ then $x_{\alpha+\beta}(f)$ is a generator of type 2.
\end{rem}
...
\end{lemma} \begin{proof} Assume $g_2\in P_\alpha(m')$ for some $m' \in M$, so that $g_1 \cdot g_2 \in P_\alpha(m + m')$. The assertion now follows from the definition of $S_\alpha(a, -)$:
...
Let $\beta$ be such that $\alpha - \beta \in \Phi$ and let $g = z_{\beta}(Xf, \xi)$ be a generator of type $1$. From~\cref{Zrels}~\eqref{Z3} we obtain that
\begin{equation} z_{\beta}(Xf, \xi) ^ {x_{\alpha}(aX^{-1})} = x_{\alpha} (af\xi) \cdot x_{\alpha-\beta} (N_{\beta,-\alpha}\cdot af\xi^2) \cdot z_{\beta}(Xf, \xi), \label{eq:Zconj2} \end{equation}
and the expression in the right-hand side is a product of generators of type 5, 2, 1.
Now let $g = z_{\beta}(f, X\xi)$ be a generator of type 2 (for $\beta$ as above). Again by~\cref{Zrels}~\ref{Z3}
\begin{equation} z_{\beta}(f, X\xi) ^ {x_{\alpha}(aX^{-1})} = x_{\alpha} (af\xi) \cdot x_{\alpha-\beta} (N_{\beta,-\alpha}\cdot aXf\xi^2) \cdot z_{\beta}(f, X\xi).
\end{equation}
and the expression in the right-hand side are products of generators of type 5, 1, 2.

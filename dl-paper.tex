...
\begin{df}\label{def:Kab} Denote by $\widetilde{K}(\alpha, \beta)$ the subgroup of $\overline{\St}(\Phi, A[X], M[X])$ generated by the following five families of elements parametrized by $\xi \in A[X]$, $f\in M[X]$:
 \begin{enumerate}
...
It is clear from the definition that $K(\alpha, \beta) \subseteq P_\alpha(0) \cap P_\beta(0) \cap j_+(K(A[X], M[X]))$. The reverse inclusion follows from the just proved decomposition and the fact that for $g\in j_+(K(A[X], M[X]))$ one has $p_{\alpha}(g) = m$ and $p_\beta(g) = m'$ (cf.~\cref{rem:palpha}). \end{proof}
...
If $\langle \beta, \gamma \rangle$ is $0$ or $1$, then $g$ is a generator of type 2 or 3 for $P_\beta(0)$ in the sense of~\cref{defP0}. Finally, $g = z_\beta(f, X\xi)$ also belongs to $P_\beta(0)$ (this follows e.\,g. from Remarks~\ref{Pstar-char}--\ref{rem:palpha}). \end{proof}

...
\begin{lemma} \label{lem:field-case}
 Under certain assumptions the map $\St(\Phi, k[t]) \to \St(\Phi, k[t, t^{-1}])$ is injective.
\end{lemma}
\begin{proof}
 %TODO: Add references on Rehmann and Hurrelbrink...
\end{proof}
...
Let $a$ be an element of $\Ker(h)$. Since $i$ is injective by~\cref{lem:field-case}, the element $a$ also lies in $\Ker(\pi_B)$ and hence comes from some $b \in \St(\Phi, B, I)$ via $\mu_B$. Since $g(b) \in \Ker(\mu_R)$ there exists some $c \in C(\Phi, R, I)$ such that $j_R(c) = g(b)$. By our assumption, $f$ is surjective, therefore
...
\begin{proof}
 ...
 From~\cref{lem:bass-ft,lem:summary} we obtain that $\GW_3^{[0]}(k[X, X^{-1}]) \cong \GW_3^{[0]}(k)$.

...
\begin{proof}
Clearly, $P_\alpha(*)$ contains the elements $x_\beta(Xf)$ for all $\beta \in \Phi$ and $z_\beta(Xf, \xi)$ for $\beta \in \Phi \setminus \{\pm \alpha\}$, $f \in M[X]$, $\xi\in A[X]$. Invoking~\eqref{Z5} of~\cref{Zrels} with $s = Xf$, $\eta = 1$ we conclude from this that $z_{\pm\alpha}(Xf, \xi)$ also belong to $P_{\alpha}(*)$. Thus, we have shown that $P_\alpha(*)$ contains $\St(\Phi, A[X], M[X])$.
...
But $c_{\pm \gamma}(f, X\xi) = x_{\pm \gamma}(f) \cdot z_{\pm \gamma}(-f, -X\xi)$ is a product of two generators of type 3.
\begin{comment}

...
\begin{rem} \label{Pstar-char} The above lemma allows us to characterize $P_\alpha(*)$ as follows: it consists of images under $j_+$ of those elements 
 $g \in \overline{\St}(\Phi, A[X], M[X])$ for which $ev_{X=0}^*(g)$ lies in the subgroup $C_\alpha(A, M) := \Img(\overline{\St}(Z_0(\alpha), A, M) \to \overline{\St}(\Phi, A, M)) \ltimes \hat{U}(Z_+(\alpha), M)$ 
...
\begin{proof}
 ...
 Set $h_0 = ev_{X=0}(g_1))^{-1}$, then, clearly, $g_1 \cdot i_+(h_0) \in K(A[X], M[X])$ and 
 ...
 By definition, $h_1 h_2^{-1} = i_-(h_0)$, for $h_0 \in \overline{\St}(\Phi, A, M)$, moreover, $p(g, h_1, u) \cdot p(g, h_2, u)^{-1} = j_- i_-(h_0) = j_+ i_+(h_0) \in P_\alpha(*)$,
 Clearly, $ev_{X=1}(j_+ i_+ (h_0)) = ev_{X=1}(i_+(h_0)) = ev_{X=0}(i_+(h_0)) = h_0$.
 On the other hand, by~\cref{Pstar-char} there exists $g_1 \in \overline{\St}(\Phi, A[X], M[X])$ such that $ev_{X=0}(g_1) \in C_\alpha(A, M)$
  and $j_+(g_1) = j_+ i_+ (h_0)$.
 Now we have: $h_0 = ev_{X=1}(j_+(g_1)) = ev_{X=1}(g_1)$
 
 %Invoking injective stability for $\K_1(\Phi, -)$ and surjective stability $\K_2(\Phi, -)$ we can find $h_0 \in \overline{\St}(Z_0(\alpha), A, M) \ltimes U(Z_+(\alpha), M)$ such that $\pi(h_0) = \pi(j_-(h_1 h_2^{-1}))$.
\end{proof}

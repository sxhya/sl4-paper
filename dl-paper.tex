...
Let $G \colon \catname{CRings} \to \catname{Groups}$ be a functor preserving binary pullbacks and 
 $P$ its subfunctor such that for all $R$ the group $P(R)$ is a perfect normal subgroup of $G(R)$.
...
Let $S \colon \catname{CRings} \to \catname{Groups}$ be another group functor equipped with a natural transformation
 $\varphi \colon S \to P$ such that for all $R$ the group $S(R)$ is a universal central extension of $P(R)$. Then the following holds:
\begin{proplist}
...
\end{prop}
\begin{proof}
 The first claim follows from the solution of Exercises~1.8--1.9 of \cite[Chapter~IV]{Kbook}. %provided one verifies that all the intermediate isomorphisms are natural in $R$.
 ...
 Since $\varphi(G_i) \subseteq \Ker(G(p_i))$ and $G$ preserves pullbacks we conclude that $G_0 \cap G_1 \subseteq \Ker(\varphi) \colon S(D_{R, I}) \to G(D_{R, I}))$
  and hence $G_0 \cap G_1$ is a central subgroup of $S(D_{R, I})$.
 Now invoking~\cite[Proposition~6]{Lo78} we conclude that the extension $S(R, I) \to S(R)$ is a universal
  relative central extension of the coequalizer $\mathrm{coeq}(d_0, d_1)$.

...
We let $h_0 \in G_M^0$ act on $V$ on the right by $(g, h, u) \cdot h_0 = (g, h \cdot i_-(h_0), u)$. This action is well defined since $G^0_M \subset \overline{G}^{\geq 0}_M$. Whenever two triples $v_1, v_2 \in V$ lie in the same orbit we use the notation $v_1 \sim v_2$ and say that they are congruent. The same notation $h_1 \cong h_2$ is also used for the elements of $\St(\Phi, A[X\inv])$ that are right-adjacent with respect to $G^0_M$.

We denote by $\overline{V}$ the set of orbits of this action.
We use the notation $(g, [h], u)$ for the elements of $\overline{V}$.
We denote by $\overline{p}$ the function $\overline{V} \to \overline{G}^{\geq 0}_M/G_M^0$ mapping each orbit $(x, [h], u)$ to the coset $p(x, h, u)G_M^0$.
...
If $p(g, h, u)$ belongs to $P_\alpha(m)$ for some $m \in M$, then we define $t_\alpha(aX^{-1})$ by the following identity:

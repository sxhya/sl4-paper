...
Let $M$ be an ideal of a local ring $A$. We denote by $P_{\alpha, M}(0)$ the subgroup of $\St(\Phi, A[X, X^{-1}])$ generated by the images of the following five families of elements of $\overline{\St}(\Phi, A[X], M[X])$ under the map $j_+ \colon \St(\Phi, A[X]) \to \St(\Phi, A[X, X^{-1}])$:
\begin{enumerate} 
...
Notice that in the above definition we use letter $f$ (resp. $\xi$) to denote an element of $M[X]$ (resp. $A[X]$).
We adhere to this convention until the end of this section.
Notice also that by~\eqref{Zdef} one has $z_\beta(f, 0) = x_\beta(f)$.

We denote by $P_{\alpha, M}(*)$ the subgroup of $\St(\Phi, A[X, X^{-1}])$ generated by $P_{\alpha, M}(0)$ and the elements $x_{-\alpha}(mX)$, $m \in M$.
\end{df}
...
\begin{lemma}\label{P0_normal} The subgroup $P_{\alpha, M}(0)$ is normal in $P_{\alpha, M}(*)$. Moreover, there is a short exact sequence of groups, which is split by the map $m \mapsto x_{-\alpha}(mX)$ (we denote by $(M, +)$ the additive group of the ideal $M$):
...
Thus, the elements $c_{\beta}(f, X\xi)$ lie in $P_{\alpha, M}(0)$ for all $\beta \in Z(\alpha)$.

It is also easy to check that $P_{\alpha, M}(0)$ contains the image of $Z_\alpha(A, M)$ under the natural embedding of $\overline{\St}(\Phi, A, M) \hookrightarrow \St(\Phi, A[X, X\inv])$.
...
Then the subgroup $P_{\alpha, M}(*)$ contains the image in $\St(\Phi, A[X, X^{-1}])$ of the subgroup $K(A[X], M[X]) \leq \overline{\St}(\Phi, A[X], M[X])$ defined in the beginning of~\cref{sec:computationOfK}. \end{lemma}
\begin{proof} Clearly, $P_\alpha(*)$ contains the elements $x_\beta(Xf)$ for all $\beta \in \Phi$ and $z_\beta(Xf, \xi)$ for $\beta \in \Phi \setminus \{\pm \alpha\}$, $f \in M[X]$, $\xi\in A[X]$, therefore by~\cref{thm:Stepanov} $P_\alpha(*)$ contains all of $j_+(\overline{\St}(\Phi, A[X], XM[X]))$. The required assertion now follows from~\cref{Kgen-strong} and~\cref{rem:c-DS}. \end{proof}

\begin{rem} \label{Pstar-char} The above lemma allows us to characterize $P_{\alpha, M}(*)$ as follows: it coincides with the image under $j_+$ of the subgroup 
of $\overline{\St}(\Phi, A[X], M[X])$ consisting of those elements $g$ for which $ev_{X=0}^*(g)$ lies in the subgroup $Z_\alpha(A, M)$. \end{rem} 
...
From~\cref{P0_normal} it follows that $P_\alpha(*)$ coincides with the union of all $P_\alpha(m)$, $m\in M$, which allows us to give the following definition. 

...
Let us also briefly recall Loday's procedure for relativization of $S$, that is,
 extending $S$ from the category of rings to the category of pairs.
...
\begin{proof}
 ...
 Now invoking~\cite[Proposition~6]{Lo78} we conclude that the extension $S(R, I) \to S(R)$ is a universal relative central extension of the coequalizer $\mathrm{coeq}(d_0, d_1)$, 
   which coincides with $S(R) \to S(R/I)$ by~\eqref{req:coeq}. The kernel of $\mu$ is isomorphic to $\HH_3(S(R), S(R/I), \ZZ)$ by~\cite[Th{\'e}or{\`e}me~2]{Lo78}.
 
 The third claim now follows from the starting portion of the homology long exact sequence of the pair $(S(R/I), S(R))$ and the first two assertions.
\end{proof}

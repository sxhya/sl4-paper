...
\begin{enumerate}
 ...
 \end{multline}
\item {\it Case $\beta = -\alpha$, $f_1(X) = Xf_1'(X)$}.
Suffices to prove the assertion under the assumption $f_2(X)=0$.
First consider the case $f_1(X) = mX$, where $m\in M$. For any $\alpha \in \Phi$ one can choose $\gamma$ such that $\langle \alpha, \gamma \rangle = -1$

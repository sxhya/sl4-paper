...
%TODO: Give the definition of $D_{R, I}$ and of 
...
\begin{enumerate}
 ...
 \item The map $\mu \colon S(R, I) \to S(R)$ is a universal relative central extension of 
    $S(R) \to S(R/I)$, whose kernel is naturally isomorphic to the third relative homology group $\HH_3(S(R), S(R/I), \ZZ)$.
 \item There is an exact sequence which is natural in $(R, I)$:
...
In the above definition we could have replaced the orthogonal group with a different isogeneous form of the orthogonal group, e.\,g. 
 we could have chosen $G_n = \mathrm{SO}_{2n}$ or $G_n = \mathrm{Spin}_{2n}$ and then defined $P_n$ as its elementary subfunctor.
Of course, this would change the functors $\K_i(G_n, P_n, -)$ for $i=1,2$ however, as the proof of~\cref{characterization} suggests
 it would not change the functors $\K_i(G_n, P_n, -)$ for $i\geq 3$.

Now let $G_n, P_n$ be again as in~\cref{ex:orth}. 
Let us formulate a stability result for orthogonal Quillen K-groups (see~\cite[Theorem~9.4]{Pa89}).
\begin{theorem}[Panin] \label{lem:Panin-stability}
 ...
 Then the canonical map $\K_i(G_n, P_n, R) \to \K_i(G_{n+1}, P_{n+1}, R)$ is an epimorphism for $n \geq b$ 
 and an isomorphism for $\ell \geq b + 1$, where $b = \mathrm{max}(2i, a+i-1)$. \end{theorem}
 
Of course, in the limit, the groups $\K_i(\mathrm{O}_\infty, \mathrm{EO}_\infty, R) = KO_i(R)$ coincide with Karoubi's hermitian groups ${}_{-1}\!L_i(R)$ defined in~\cite{Ka80}.
The modern version of hermitian K-theory is the theory of higher Grothendieck--Witt groups of schemes.
We refer the reader to~\cite[\S~2]{AF17} or~\cite[\S~2]{FRS12} for a more detailed introduction to it.
For our purposes it would be sufficient to think of $\GW_i^{[k]}(R)$ as a shorthand for the following:
\begin{equation}

...
\begin{corollary} \label{Kgen-strong} For $A$ and $M$ as in~\cref{thmStein} and arbitrary fixed root $\gamma\in \Phi$ the subgroup $K(A[X], M[X])$ is generated as a group by
 $\overline{\St}(\Phi, A[X], XM[X])$ and the elements $c_{\pm \gamma}(m, X\eta)$, where $m \in M$, $\eta \in A[X]$. \end{corollary}
\begin{proof} Choose some ordering on $\Phi$ with respect to which $\gamma$ becomes the maximal positive root. 
 Invoking~\eqref{C4} of~\cref{Crels} with $\xi = 1$, $s = m$, $t = X\eta$ we obtain that modulo 
 $\overline{\St}(\Phi, A[X], XM[X])$ the element $c_{\alpha + \beta}(m, X\eta)$ is equivalent to 
 $c_{\alpha}(-\epsilon m, -\epsilon X \eta)^{-1}$, $\epsilon = N_{\alpha, \beta}$. 
 Arguing by induction on the height of $\alpha$ we conclude that $c_\alpha(m, X\eta)$ for all $\alpha \in \Phi^+$
 belong to the subgroup generated by $\overline{\St}(\Phi, A[X], XA[X])$ and $c_\gamma(m, X\eta)$.
 Similar argument also works for the negative roots, and the required assertion now follows. \end{proof}

\subsection{The subgroups $P_\alpha(0)$, $P_\alpha(*)$ and their properties.} 
Throughout this subsection $\Phi$ is a simply-laced root system of rank $\geq 3$ and $\alpha$ denotes some fixed root of $\Phi$.

Consider the following root subsets of $\Phi$ (cf.~\cite[Proposition~5.7]{St71}):
\begin{equation} \nonumber \begin{align}
   Z_+(\alpha) & = \{ \beta \in \Phi \mid \langle \alpha, \beta \rangle > 0 \}, \\
   Z_0(\alpha) & = \{ \beta \in \Phi \mid \langle \alpha, \beta \rangle = 0 \}, \\
   Z(\alpha)   & = Z_0(\alpha) \sqcup Z_+(\alpha). \end{align} \end{equation}
...
Notice that the subgroup $Z_\alpha(A, A)$ coincides with $Z(\hat{U}_\alpha)$ defined in~\cite[984]{St71}.

\begin{rem}\label{Z-DS} Notice that our assumptions on the rank of $\Phi$ guarantee that $Z_0(\alpha)$ is nonempty, 
hence $Z_\alpha(A, M)$ contains relative Dennis--Stein symbols $\langle a, m \rangle$ for $a\in A$, $m\in M$.
...
It is also clear that $P_\alpha(0)$ contains the elements $c_{\pm \beta}(f, X\xi)$ for all $\beta \in Z(\alpha)$
...
\begin{rem} \label{rem:palpha} From the two preceding lemmas it follows that the value of $p_\alpha$ on an element $g\in K(A[X], M[X])$ can be computed via the following procedure. 
...
\begin{df} Denote by $K(\alpha, \beta)$ the subgroup of $\St(\Phi, A[X, X^{-1}])$ generated by the following elements:
 \begin{enumerate}
  ...
  \item $c_{\pm \delta}(f, X\xi)$ for some fixed root $\delta \in \Phi \setminus \Psi$. \end{enumerate} \end{df}
...
\begin{lemma} Let $\Phi$ be any root system of rank $\geq 3$, $\alpha \in \Phi$ and $(A, M)$ be a local pair.
 Then the partial function $t_\alpha(aX^{-1}) \colon V \not \to V$ descends to a well-defined total function 
...
\begin{proof}
 ...
 $p(g, h \cdot i_-(h_0), u) = j_+(g_1) \cdot j_-(i_-(h_0)) = j_+(g_1 \cdot i_+(h_0)) \in j_+(K(A[X], M[X])) \subseteq P_\alpha(*)$,
  which shows that $t_\alpha(aX^{-1})$ is defined on $(g, h \cdot i_+(h_0), u)$, while the latter triple also lies in the same orbit as $(g, h, u)$.
 ...
 By~\cite[Theorem~2.5]{Ste73} the latter subgroup is generated by relative Steinberg symbols $\{ a, 1 + m \}_\beta$, where $\beta$ is any fixed long root of $\Phi$,
 It is not hard to check using e.\,g. the table on p.~986 of~\cite{St71} that $Z_0(\alpha)$ always contains a long root, therefore
  $\overline{\K_2}(\Phi, A, M)$ is contained in $Z_\alpha(A, M)$ by~\cref{Z-DS} hence $h_0 \in Z_\alpha(A, M)$.
 Thus, we have shown that $i_-(h_0)$ is centralized by $X_\alpha(A[X^{-1}])$, which allows us to conclude that
 \begin{multline} \nonumber

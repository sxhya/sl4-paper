...
We start this section by recalling basic notation and facts pertaining to Grothendieck--Witt groups.
...
For our purposes it suffices to restrict attention to the affine case, in which  the Grothendieck--Witt groups $\GW_i^{[k]}(R)$ for $i \geq 1,\ [k] \in \ZZ/4\ZZ$ can be considered simply as a shorthand for the following groups:
...
Here the groups $\mathrm{KO}_i(R)$ are the stable analogues of the groups $\KO_i(2n, R)$ considered in the previous chapter. Similarly, one can also define the symplectic $K$-groups $\mathrm{KSp}_i(R)$. The groups ${}_{\pm 1}\!U_i(R)$ are defined in~\cite{Ka80} and we will not use their definition directly.

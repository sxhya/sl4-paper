...
\item\label{Z5} $z_{\alpha+\beta}(st, \xi) = x_{-\beta}(-s\xi) \cdot x_\alpha(N_{\alpha,\beta}s) \cdot x_{-\alpha}(N_{\alpha,\beta}st^2\xi^2) \cdot x_{-\alpha-\beta}(-st\xi^2) \cdot z_{-\beta}(-s\xi, -t) \cdot x_{\beta}(-st^2\xi)  \cdot x_{\alpha+\beta}(-st) \cdot z_\alpha(N_{\alpha,\beta} s, -N_{\alpha, \beta}t\xi)$ for any $\alpha, \beta$ such that $\alpha+\beta\in\Phi$.
\end{enumerate} \end{lemma}
\begin{proof}
The first four assertions are contained in~\cite[Lemma~9]{S15}. For the proof of the last formula set $\gamma = \alpha + \beta$.
Now direct computation using~\cref{lem:structure-constants} shows that:
\begin{multline} \nonumber z_{\gamma}(N_{\alpha,\beta}st, \xi) = 
...
The elements $c_\alpha(s, t)$ satisfy the following relations for all $s\in I,\ t\in J,\ \xi\in A$.
 \begin{enumerate}
  ...
  \item \label{C4} $c_{\alpha+\beta}(s, t\xi) = [x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(N_{\alpha,\beta}t)} \cdot c_{\alpha}(N_{\alpha, \beta}s\xi, -N_{\alpha,\beta}t)^{-1} \cdot x_{-\beta}(-st\xi^2)$ if $\alpha+\beta\in\Phi$.
 \end{enumerate}
\end{lemma}
\begin{proof}
...
Now suppose $\alpha+\beta \in \Phi$. Using~\eqref{HW-variant} we obtain:
\begin{multline} \nonumber
...
Finally, relation~\eqref{C4} can be verified by the following computation involving identities~\eqref{HW-variant} and $[x, yz]^y = [y^{-1}, x] \cdot [x, z]$:
\begin{multline} \nonumber
 [x_{\alpha+\beta}(s), x_{-\alpha-\beta}(t\xi)] = [x_{\alpha+\beta}(s), [x_{-\alpha}(N_{-\alpha,-\beta}t), x_{-\beta}(\xi)]] = \\
  = \left([[ x_{\alpha+\beta}(-s), x_{-\alpha}(-N_{-\alpha,-\beta}t)], x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s)} \cdot  [[ x_{-\beta}(-\xi), x_{\alpha+\beta}(s)], x_{-\alpha}(N_{\alpha,\beta}t)] ^ {x_{-\beta}(-\xi)}\right)^{x_{-\alpha}(N_{\alpha,\beta}t)} = \\ =
  [ x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(N_{\alpha,\beta}t)} \cdot  [ x_{\alpha}(N_{\alpha,\beta}s\xi), x_{-\alpha}(N_{\alpha,\beta}t)] ^ {x_{-\beta}(-\xi)x_{-\alpha}(N_{\alpha,\beta}t)}  = \\ = [x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(N_{\alpha,\beta}t)} \cdot  [ x_{\alpha}(N_{\alpha,\beta}s\xi), x_{-\alpha}(N_{\alpha,\beta}t) x_{-\alpha-\beta}(t\xi)] ^ {x_{-\alpha}(N_{\alpha,\beta}t)} = \\ =
  [x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(N_{\alpha,\beta}t)} \cdot  [x_{-\alpha}(-N_{\alpha,\beta}t), x_{\alpha}(N_{\alpha,\beta}s\xi)] \cdot [x_{\alpha}(N_{\alpha,\beta}s\xi), x_{-\alpha-\beta}(t\xi)] = \\
  [x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(N_{\alpha,\beta}t)} \cdot c_{\alpha}(N_{\alpha, \beta}s\xi, -N_{\alpha,\beta}t)^{-1} \cdot x_{-\beta}(-st\xi^2). \qedhere
\end{multline}

...
\item\label{Z5} If $\alpha+\beta\in\Phi$, then the following relation holds (in which $\epsilon = N_{\alpha,\beta}$):
\begin{multline} \nonumber z_{\alpha+\beta}(st, \xi) = x_{-\beta}(-s\xi) \cdot x_\alpha(\epsilon s) \cdot x_{-\alpha}(\epsilon st^2\xi^2) \cdot x_{-\alpha-\beta}(-st\xi^2) \cdot z_{-\beta}(-s\xi, -t)\cdot \\ \cdot x_{\beta}(-st^2\xi)  \cdot x_{\alpha+\beta}(-st) \cdot z_\alpha(\epsilon s, -\epsilon t\xi).\end{multline}
\end{enumerate} \end{lemma}
\begin{proof}
The first four assertions are contained in~\cite[Lemma~9]{S15}.
To prove the remaining assertion use direct computation (in which $\epsilon = N_{}$):
\begin{multline} \nonumber
  z_{\alpha+\beta}(s\eta, \xi) = 
\end{multline}


For the proof of the last formula set $\gamma = \alpha + \beta$.
...
The elements $c_\alpha(s, t)$ satisfy the following relations for all $s\in I,\ t\in J,\ \xi\in A$.
 \begin{enumerate}
  ...
  \item \label{C4} If $\alpha+\beta\in\Phi$ then the following relation holds (in which $\epsilon = N_{\alpha,\beta}$):
...
Finally, relation~\eqref{C4} can be verified by the direct computation involving identities~\eqref{HW-variant} and $[x, yz]^y = [y^{-1}, x] \cdot [x, z]$ (everywhere below $\epsilon = N_{\alpha, \beta}$):
\begin{multline} \nonumber [x_{\alpha+\beta}(s), x_{-\alpha-\beta}(t\xi)] = [x_{\alpha+\beta}(s), [x_{-\alpha}(-\epsilon t), x_{-\beta}(\xi)]] = \\ = \left([[ x_{\alpha+\beta}(-s), x_{-\alpha}(\epsilon t)], x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s)} \cdot  [[ x_{-\beta}(-\xi), x_{\alpha+\beta}(s)], x_{-\alpha}(\epsilon t)] ^ {x_{-\beta}(-\xi)}\right)^{x_{-\alpha}(\epsilon t)} = \\ = [ x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(\epsilon t)} \cdot  [ x_{\alpha}(\epsilon s\xi), x_{-\alpha}(\epsilon t)] ^ {x_{-\beta}(-\xi)x_{-\alpha}(\epsilon t)}  = \\ = [x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(\epsilon t)} \cdot  [ x_{\alpha}(\epsilon s\xi), x_{-\alpha}(\epsilon t) x_{-\alpha-\beta}(t\xi)] ^ {x_{-\alpha}(\epsilon t)} = \\ = [x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(\epsilon t)} \cdot  [x_{-\alpha}(-\epsilon t), x_{\alpha}(\epsilon s\xi)] \cdot [x_{\alpha}(\epsilon s\xi), x_{-\alpha-\beta}(t\xi)] = \\ = [x_{\beta}(-st), x_{-\beta}(\xi)] ^ {x_{\alpha+\beta}(-s) x_{-\alpha}(\epsilon t)} \cdot c_{\alpha}(\epsilon s\xi, -\epsilon t)^{-1} \cdot x_{-\beta}(-st\xi^2). \qedhere
\end{multline}

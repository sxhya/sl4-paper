...
For $n\geq 1$ we define ``truncated'' Steinberg groups $\St^{\leq n}(\Phi, B)$ by the set of generators $x_\alpha(a)$, $\alpha\in \Phi$, $a\in B_d$, $d\leq n$, and relations 
\begin{itemize}
 \item $\text{R}1_{d}$ for $d\leq n$;
 \item $\text{R2}_{d,e}$ for $d,e \leq n$ such that $d+e \leq n$;
 \item $\text{R3}^\angle_{d, e}$ for $d, e\leq n$;
 \item $\text{R3}^\bot_{d, 0}$ for $d \leq n$.
 \end{itemize}

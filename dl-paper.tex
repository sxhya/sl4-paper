...
\[ p(x_\alpha(aX^{-1})\cdot g, h', u(1 + am)) = S_\alpha(p(g, h, u) \cdot h^{-1}_\alpha(1 + am) \cdot x_{\alpha-\beta}(-\epsilon am') \in P_\beta(\tfrac{m'}{1+am}), \]
...
As before, we may assume that \[p(g, h, u) \in j_+\left(K(A[X], M[X])\right) \cap P_\alpha(m) \cap P_{\alpha + \beta}(m')\ttxt{ for some $m, m' \in M\].

...
\begin{rem} \label{Pstar-char} The above lemma allows us to characterize $P_\alpha(*)$ as follows: it consists of images under $j_+$ of those elements 
 $g \in \overline{\St}(\Phi, A[X], M[X])$ for which $ev_{X=0}^*(g)$ lies in the subgroup $Z_\alpha(A, M) := \Img(\overline{\St}(Z_0(\alpha), A, M) \to \overline{\St}(\Phi, A, M)) \ltimes \hat{U}(Z_+(\alpha), M)$ 
 (i.\,e. the subgroup generated by elements $x_{\beta}(m),\ \beta \in Z_+(\alpha)$ and $z_{\gamma}(m, \zeta),\ \gamma \in Z_0(\alpha),$ where $m \in M$, $\zeta \in A$ 
 and $Z_+(\alpha)$, $Z_0(\alpha)$ are as in~\cite[Proposition~5.7]{St71}). \end{rem} 
...
Denote by $\overline{G}_M^0$ the image of the left arrow $i_-$ in $G^{\leq 0}$. 
...
We let $\overline{G}_M^0$ act on $V$ on the right by $(g, h, u) \cdot h_0 = (g, h \cdot h_0, u)$, $h_0 \in \overline{G}_M^0$.
It is clear that this action is well defined. Denote by $\overline{V}$ the quotient set $V / \overline{G}_M^0$.
...
\begin{proof}
 ...
 Fix $c = [g, h, u] \in \overline{V}$. Since $p(g, h, u) \in \overline{G}^{\geq 0}_M$ there exists $g_1 \in \overline{\St}(\Phi, A[X], M[X])$ such that $j_+(g_1) = p(g, h, u).$
 ...
 $p(g, h \cdot i_-(h_0), u) = j_+(g_1) \cdot j_-(i_-(h_0)) = j_+(g_1 \cdot i_+(h_0)) \in j_+(K(A[X], M[X])) \subseteq P_\alpha(*)$,
  which shows that $(g, h \cdot i_+(h_0), u)$ is the representative of $c$ on which $t_\alpha(aX^{-1})$ is defined.
  
 Next, let us show that the value of $T_\alpha(aX^{-1})$ does not depend on the choice of a representative.
 Let $(g, h_1, u), (g, h_2, u)$ be a pair of representatives of the same $c \in \overline{V}$ for which both $p(g, h_1, u)$ and $p(g, h_2, u)$ lie in $P_\alpha(*)$.
 By definition, $h_1^{-1} h_2 = i_-(h_0)$, for some $h_0 \in \overline{\St}(\Phi, A, M)$, moreover, 
 ...
 the projections of $g_0$, $g_1$ and $h_0$ in $G(\Phi, A[X, X^{-1}])$ are equal, therefore $g_0 \cdot h_0^{-1} \in \overline{\K_2}(\Phi, A, M)$. 
 By~\cite[Theorem~2.5]{Ste73} the latter subgroup is generated by relative Steinberg symbols $\{ a, m \}_\beta$, where $\beta$ is any fixed long root of $\Phi$.
 It is not hard to check using e.\,g. the table on p.~986 of~\cite{St71} that $Z_0(\alpha)$ always contains a long root, therefore
  $\overline{\K_2}(\Phi, A, M)$ is contained in $Z_\alpha(A, M)$ by ??? hence $h_0 \in Z_\alpha(A, M)$.
 Thus, we have shown that $i_-(h_0)$ is centralized by $X_\alpha(A[X^{-1}])$ therefore
 \begin{multline} \nonumber
  T_\alpha(aX^{-1})[g, h_1, u] = \left[ x_\alpha(aX^{-1})\cdot g ,\ h_1 \cdot x_\alpha\left(-\tfrac{aX^{-1}}{1 + am}\right),\ u \cdot (1 + am)\right] = \\
  = \left[ x_\alpha(aX^{-1})\cdot g ,\ h_1 i_-(h_0) \cdot x_\alpha\left(-\tfrac{aX^{-1}}{1 + am}\right),\ u \cdot (1 + am)\right] = T_\alpha(aX^{-1})[g, h_2, u]. \qedhere

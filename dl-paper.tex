...
For our computations we will need two explicit elements of $\K_2(\Phi, R)$. Recall that Steinberg symbols are defined for any $s, t \in R^*$ by the identity
\begin{equation} \label{eq:steinberg} \{ s, t \}_\alpha = h_\alpha(s) \cdot h_\alpha(t) \cdot h_\alpha(st)^{-1}. \end{equation}

Our convention for Dennis--Stein symbols follows~\cite{DS73} (and {\it not} III.5.11 of ~\cite{Kbook}).
Recall that Dennis--Stein symbols are defined for arbitrary $r, s\in R$ satisfying $1 + rs \in R^*$ as follows:
\begin{equation} \label{eq:dennis-stein}  \langle r,s \rangle _ \alpha = x_{-\alpha}\left(\tfrac{- s}{1 + rs}\right) \cdot x_{\alpha}(r) \cdot x_{-\alpha}(s) \cdot x_{\alpha}\left(\tfrac{- r}{1+rs}\right) \cdot h_{\alpha}(1 + rs)^{-1}. \end{equation} 
...
We denote by $B = B(A, I)$ the subring $A[t^{-1}] + I[t] \subseteq A[t, t\inv]$ with the induced grading.
As an $A$-module $B$ decomposes as $\oplus_{d\in\mathbb Z}B_d$ where $B_d=I \cdot t^d$ for $d<0$, and $B_d=A \cdot t^d$ for $d\leq0$.
Obviously, $B = A[t]$ in the case $I=0$ and $B = A[t, t\inv]$ in the case $I=A$.
...
\end{align}
By the degree of a relation we mean the maximum of the degrees of generators that appear in it.
For example, the degree of $\text{R2}_{d,e}$ is $\max(d,e,d+e)$ while the degree of $\text{R3}^\bot_{d,e}$ or $\text{R3}^\angle_{d,e}$ is $\max(d,e)$.
...
The following lemma asserts that most of the relators $\text{R3}^\bot_{d,e}$ in this presentation of $\St^{\leq n}(\Phi, B)$ can be omitted.
\begin{lemma}\label{superfluous-relations}
 For every simply-laced root system $\Phi$ of rank $\geq 3$ and every $n \geq 1$ one can exclude from the presentation of $\St^{\leq n}(\Phi, B)$ the relators $\text{R3}_{d,e}^\bot$ for which $\max(0,d) + \max(0,e) > 1$.
\end{lemma}
\begin{proof}
...
Without loss of generality we may assume $e \geq d$ and $e > 0$. Let $\alpha$ and $\gamma$ be a pair of orthogonal roots of $\Phi$.
We can find $\beta \in \Phi$ forming an obtuse angle with both $\alpha$ and $\gamma$ (see e.\,g.~\cite[Lemma~3.1.2]{RS76}).

We start with the following two observations inspired by the proof of the main result of~\cite{RS76}.
\begin{enumerate}
\item In the case $0 < d \leq e \leq n$ the relation $\text{R3}^\bot_{d,e}$ is a consequence of $\text{R3}^\bot_{0,e-d}$:
\begin{align*} x_{\alpha+\beta+\gamma}(abt^e) = [x_{\alpha+\beta}(at^{e-d}), [x_{-\beta}(t^d), x_{\beta+\gamma}(b)] ] & \text{ by $\text{R2}_{d,0}$, $\text{R2}_{d, e-d}$} \\ 
 = [[x_{\alpha+\beta}(at^{e-d}), x_{-\beta}(t^d)], {}^{x_{-\beta}(t^d)} x_{\beta+\gamma}(b)] & \text{ by~\eqref{eq:H1iii}, $\text{R3}^\bot_{0,e-d}$} \\ 
 = {}^{x_{-\beta}(t^d)} [x_{\alpha}(at^e), x_{\beta+\gamma}(b)] & \text{ by $\text{R2}_{e-d,d}$, $\text{R3}^\angle_{d,e}$} \\ 
 = {}^{x_{-\beta}(t^d)} [x_{\alpha + \beta}(at^{e-d}), x_{\gamma}(bt^d)] & \text{ by $\text{R2}_{e,0}$, $\text{R2}_{e-d,d}$} \\ 
 = [x_{\alpha}(at^e) \cdot x_{\alpha + \beta}(at^{e-d}), x_{\gamma}(bt^d)] & \text{ by $\text{R2}_{e-d,d}$, $\text{R3}^\angle_{d,d}$} \\ 
 = {}^{x_\alpha(at^e)}x_{\alpha+\beta+\gamma}(abt^e) \cdot [x_\alpha(at^e), x_\gamma(bt^d)] & \text{ by~\eqref{eq:H1ii}, $\text{R2}_{e-d,d}$} \\
 = x_{\alpha+\beta+\gamma}(abt^e) \cdot [x_\alpha(at^e), x_\gamma(bt^d)] & \text{ by $\text{R3}^\angle_{e,e}$}. \end{align*}

\item In the case $d \leq 0 \leq e \leq n$ the relation $\text{R3}^\bot_{d,e}$ is a consequence of $\text{R3}^\bot_{1,d+e-1}$:
\begin{align*} [x_\alpha(at^d), x_{\gamma}(bt^{e})] = [x_\alpha(at^d), [x_{\beta+\gamma}(b't^{e-1}), x_{-\beta}(t)]] & \text{ by~$\text{R2}_{e-1,1}$ } \\
 = [[x_\alpha(at^d), x_{\beta+\gamma}(b't^{e-1})], {}^{x_{\beta+\gamma}(b't^{e-1})}\!x_{-\beta}(t)] & \text{ by \eqref{eq:H1iii} and $\text{R3}^\angle_{d,1}$ } \\
 = {}^{x_{\beta+\gamma}(b't^{e-1})}\![x_{\alpha+\beta+\gamma}(ab't^{e-1}t^d), x_{-\beta}(t)] & \text{ by $\text{R2}_{d,e-1}$ and $\text{R3}^\angle_{e-1,e-1}$  } \\
 = 1 & \text{ by $\text{R3}^\bot_{d+e-1,1}$.} \end{align*} \end{enumerate}                                                           
Together the above observation show that if the degree of the relator $\text{R3}^\bot_{d,e}$ is $\geq 2$ then it is a consequence of some relator $\text{R3}^\bot_{d',e'}$ of strictly smaller degree.
Finally, by the first observation, $\text{R3}^\bot_{1,1}$ is a consequence of $\text{R3}^\bot_{0,0}$.
\end{proof}
...
\begin{lemma} The operators $T_\alpha$ satisfy $\text{R3}_{\leq 0, 1}$. \end{lemma}
...
\begin{lemma} The operators $T_\alpha$ satisfy $\text{R2}_{\leq -1, 1}$. \end{lemma}
...
\begin{prop} The operators $T_\alpha(aX^{-1})$ satisfy all relations $\text{R2}_{0,1}$ and relations $\text{R3}_{1,1}^{\angle}$ in the special case when $\alpha, \beta$ form a sharp angle. \end{prop}
\begin{proof} 
\begin{comment}

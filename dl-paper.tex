...
The constants $N_{\alpha \beta i j}$ appearing in the right-hand side of \eqref{rel:CCF} are integers equal to $\pm 1,\pm 2,\pm 3$, they are called the {\it structure constants} of the Chevalley group $G_{sc}(\Phi, R)$. There is a degree of freedom in the choice of these constants, for example, in the literature there are several methods of choosing signs of these constants, see e.\,g.~\cite{V00}, \cite[\S~9]{VP}.

In this paper we will be mostly interested in the case when the Dynkin diagram of $\Phi$ is simply-laced, i.\,e. does not contain double bonds, in which case Steinberg relations further simplify to the following relations:
\begin{align}
...
\end{align}
In the above formulae $a, b \in R$ and the integers $N_{\alpha, \beta, 1, 1} = N_{\alpha, \beta} = \pm 1$ are the structure constants of the Lie algebra of type $\Phi$. Although there is still some degree of freedom in their choice, these constants always must satisfy certain relations, provided by the following lemma (cf.~\cite[\S~14]{VP}).
...
In our computations we use two families of explicit elements of $\K_2(\Phi, R)$ called {\it Steinberg and Dennis--Stein symbols}. Notice that our treatment of symbols follows~\cite{DS73}. Recall that Steinberg symbols are defined for arbitrary $s, t \in R^\times$ as follows:

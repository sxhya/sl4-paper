...
The isomorphism between the sets $\overline{V}$ and $\overline{V}_T$ is given by the following two maps:
...
It is easy to see that these two maps are mutually inverse. This isomorphism allows us to treat these two sets as more or less the same object. In certain situations, however, it is preferable to work with one of them rather than the other. For example, specifying the action of $\St(\Phi, B)$ in terms of $\overline{V}$ leads to much shorter calculations in Lemmas~\ref{R3_leq0_1}--\ref{R2_0_1}, while the statements of~\cref{sec:P1glueing} look more natural when stated for~$\overline{V}_T$. 
...
the operators $T_\alpha$ defined above specify a well-defined action of $\St(\Phi, B)$ on $\overline{V} \cong \overline{V}$. 

This action satisfies the following additional property. For any $h_1 \in \St(\Phi, A[X\inv])$ holds $j_B^-(h_1) \cdot [1, h, u] = [1, h_1 h, u]$, where $j_B^-$ denotes the map $\St(\Phi, A[X\inv]) \to \St(\Phi, B)$ (here we identify $\overline{V}$ with $\overline{V}_T$ using the isomorphism~\eqref{eq:VVT}).
\end{prop}
\begin{proof}
...
We retain notation of the previous subsection and assume additionally that the map $j_R \colon \St(\Phi, B) \to \St(\Phi, R)$ is injective. 
This assumption allows us to consider $\St(\Phi, B)$ as a subgroup of $\St(\Phi, R)$.
By~\cref{lem:lemma32} $\St(\Phi, B)$ contains $\overline{G}^{\geq 0}_M$.
...
\end{multline*}
From the first part of the proof and inductive assumption we obtain that the assertion of the lemma holds for all the factors in the right-hand side and, therefore, also holds holds for $p_1 = z_\alpha(f, X\xi)$. It is easy to deduce from this that the assertion also holds for $p_1 = z_\alpha(f, X\xi + a) = x_\alpha(-a) \cdot z_\alpha(f, X\xi) \cdot x_\alpha(a)$. 
\end{proof}
...
\begin{proof} Since $j_- = j_R j_B^-$, where $j_B^-$ is as in~\cref{prop43}, it suffices to show that $j_B$ is injective.
 ...
 Set $h_+ = g_+g_0^{-1}$ and $h_- = g_-g_0^{-1}$, $v_0 = (1, [j_-(h_-)], 1)$.
 It is clear that $h_+ \in \overline{\St}(\Phi, A[X], M[X])$, $h_- \in \overline{\St}(\Phi, A[X\inv], M[X\inv])$ and that $j_-(h_-) = j_+(h_+)$.
 From~\cref{prop43} and~\cref{lem:action} we obtain that
 \[ [j_-(h_+), h_-, 1] = j_B^+(h_+) \cdot [1, h_-, 1] = j_B^-(h_-) j_B^+(h_+) \cdot [1, 1, 1] = [1, 1, 1], \]
 from which it follows that $h_-$ (and hence $h_+$) come from some element of $G_M^0$.
 \end{proof}

...
\section{Proof of the first part of the injectivity theorem}
...
We will also need the following property of relative Steinberg groups which is a special case of {\it ``Tulenbayev lifting property''} discussed in~\cite{LS17}.
...
\subsection{Construction of a model for $\St(\Phi, M[X] + A[X^{-1}])$}
...
We denote by $B$ the ring $M[X] + A[X^{-1}]$ considered as a graded subring of $A[X, X^{-1}]$. The notation of the present subsection is consistent with~\cref{sec:presentation} if we set $t = X^{-1}$, $I = M$.
...
Denote by $\overline{V}$ the set of orbits of this action.
Whenever two triples $(g, h_1, u)$ and $(g, h_2, u)$ lie in the same orbit we use the notation $(g, h_1, u) \sim (g, h_2, u)$ and say that these triples are congruent.
We also use the notation $(g, [h], u)$ for the elements of $\overline{V}$.
We denote by $\overline{p}$ the function $\overline{V} \to \St(\Phi, A[X, X\inv])/G_M^0$ mapping each orbit $(x, [h], u)$ to the coset $p(x, h, u)G_M^0$.
...
\begin{proof}
 ...
 $p(g, h \cdot i_-(h_0), u) = j_+(g_1) \cdot j_-(i_-(h_0)) = j_+(g_1 \cdot i_+(h_0)) \in j_+(K(A[X], M[X])) \subseteq P_\alpha(*)$,
  which shows that $t_\alpha(aX^{-1})$ is defined on the representative $(g, h \cdot i_+(h_0), u)$ congruent to $(g, h, u)$.
...
We can assume that $p(g, h, u) \in j_+\left(K(A[X], M[X]) \cap P_\alpha(m) \cap P_{\beta}(m')\right)$ for some $m,m' \in M$.
...
\end{multline}
The expressions in the right-hand sides of the above formulae are clearly symmetric under the action of the transposition $(a, \alpha) \leftrightarrow (b, \beta)$, which implies that the desired relation holds. 
\end{proof}
...
This action satisfies the following additional property. If $v = (g, [h], u) \in \overline{V}$ is such that $\overline{p}(v) = \overline{1} = G_M^0$ then for any $g_1 \in \St(\Phi, A[X\inv])$ holds $\overline{p}(j_-(g_1) \cdot v) = \overline{1}$.
\end{prop}
\begin{proof}
...
We need to verify the following property: for $g_1 \in \overline{G}_M^{\geq 0}$ and $v\in \overline{V}$ one has $\overline{p}(t(g_1) \cdot v) = g_1 \cdot \overline{p}(v)$. It suffices to verify this for $g = z_\alpha(f, \xi)$, $f \in M[X]$, $\xi\in A[X]$.

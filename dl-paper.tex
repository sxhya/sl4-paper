...
%TODO: Do we need any specific conditions on \Phi? Specify them.
...
induced by the ring homomorphism of evaluation at $0$. Clearly, this kernel, denote it by $K$, contains the subgroup $\overline{\St}(\Phi, A[X], XM[X])$, however, apriori it may be larger,
 so our goal will be to describe the explicit extra elements sufficient to generate it.

Obviously, $ev_{X=0}^*$ admits a section hence we can consider $\overline{\St}(\Phi, A, M)$ as a subgroup of $\overline{\St}(\Phi, A[X], M[X])$,
 moreover, the following decomposition holds:
\begin{equation} \label{relZero-decomp} \overline{\St}(\Phi, A[X], M[X]) = \overline{\St}(\Phi, A, M) \cdot K.\end{equation}
\begin{lemma} 
The following decomposition holds
 \[ K = \overline{\St}(\Phi, A[X], XM[X]) \cdot \left[\St(\Phi, XA[X]),\ \overline{\St}(\Phi, A, M)\right].\] 
\end{lemma}
\begin{proof}
 Fix $g \in K$ and write it as $g(X) = \prod_i z_{\alpha_i}(f_i(X), \xi_i(X))$ for some $f_i(X) = f_i(0) + Xf_i'(X) \in M[X]$, $\xi_i(X) = \xi_i(0) + X\xi_i'(X) \in A[X]$.
 ...
 %Denote by $H$ the composite of $C$ and $\overline{\St}(\Phi, A[X], XM[X])$. It is clear that $H \subseteq I$. The reverse inclusion follows from the above argument and the fact that for $g \in I$ one has $g(0)=1$. \qedhere 
\end{proof}
 \begin{lemma} If $A$ is a local ring with maximal ideal $M$ then \[\overline{\St}(\Phi, A, M) = U(\Phi^+, M) \cdot M(\Phi, A, M) \cdot U(\Phi^-, M).\] \end{lemma}
 \begin{proof} See~\cite[Theorem~2.4]{Ste73}. \end{proof}

\begin{corollary} For $A$, $M$ as above holds
 \[\left[\St(\Phi, XA[X]), \overline{\St}(\Phi, A, M)\right] \subseteq \St(\Phi, XM[X]) \cdot \left[\St(\Phi, XA[X]), \St(\Phi, M)\right].\]
\end{corollary}
\begin{proof}
 Notice first that the generators of $M(\Phi, A, M)$ normalize both $\St(\Phi, XA[X])$ and $\St(\Phi, M)$, see~\eqref{eq:conj-h-x}.
 Now for $g \in \St(\Phi, XR[X])$, $h \in M(\Phi, A, M)$, $u^+ \in U(\Phi^+, M)$, $u^- \in U(\Phi^-, M)$ holds:
 \[ [g, h u^+ u^-] = [g, h] \cdot [{}^{h}\!g, {}^{h}\!(u^+u^-)] \in \St(\Phi, XM[X]) \cdot [\St(\Phi, XA[X]), \St(\Phi, M)]. \qedhere\]
\end{proof}

\begin{corollary} For $A$ and $M$ as above the subgroup $K$ is generated as a group by its subgroup $\St(\Phi, A[X], XM[X])$ and
 the elements $[x_\alpha(m), x_{-\alpha}(X\xi)]$, $m \in M$, $\xi \in A[X]$, $\alpha \in \Phi$. 
\end{corollary}
\begin{proof}
By the above lemmata one has $K \subseteq \St(\Phi, A[X], XM[X]) \cdot [\St(\Phi, XA[X]), \St(\Phi, M)].$
It is clear that modulo $\St(\Phi, A[X], XM[X])$ the commutator subgroup $[\St(\Phi, XA[X]), \St(\Phi, M)]$ is generated
 by all $[x_\alpha(m), x_{-\alpha}(X\xi)]^g$, $m \in M$, $\xi \in A[X]$, where $g \in \St(\Phi, M + XA[X])$.
To verify the assertion, therefore, it remains to show that the commutators $[[x_\alpha(m), x_{-\alpha}(X\xi)], g]$ belong to $\St(\Phi, A[X], XM[X])$.
Moreover, since the latter subgroup is normal it suffices to prove this inclusion in the special case when $g$ is a generator of $\St(\Phi, A[X])$,
i.\,e. $g = x_\beta(\xi)$, $\eta \in A[X]$. Without loss of generality we may assume $\alpha \neq \pm \beta$.
\end{proof}

...
We consider $A[X, X\inv]$ as a graded ring in which $X^{-1}$ has degree $1$.
We denote by $B = B(A, I)$ the subring $A[X^{-1}] + I[X] \subseteq A[X, X\inv]$ with the induced grading.
As an $A$-module $B$ decomposes as $\oplus_{d\in\mathbb Z}B_d$ where $B_d=I \cdot X^d$ for $d<0$, and $B_d=A \cdot X^d$ for $d\geq0$.

We need to introduce notation for certain relations of $\St(\Phi, B)$.
\begin{align}
&\,\,x_{\alpha}(a)x_{\alpha}(b) =  x_{\alpha}(a+b),& a,b\in B_d, \tag{R$1_d$} \\
&\,[x_{\alpha}(a),\,x_{\beta}(b)]= x_{\alpha+\beta}(N_{\alpha\beta} \cdot ab),& a\in B_d,\ b\in B_e, \tag{R$2_{d,e}$} \\
&[x_{\alpha}(a),\,x_{\gamma}(b)]= 1,& \alpha \perp \beta,\ a\in B_d,\ b\in B_e, \tag{R$3^\bot_{d,e}$} \\
&[x_{\alpha}(a),\,x_{\beta}(b)]= 1,& \alpha - \beta \in \Phi,\ a\in B_d,\ b\in B_e. \tag{R$3^\angle_{d,e}$}
\end{align}
...
\begin{align*} 
 ...
 = [[x_\alpha(\xi), x_{\beta+\gamma}(b't^{e-1})], {}^{x_{\beta+\gamma}(b't^{e-1})}\!x_{-\beta}(t)] & \text{ by \eqref{eq:H1iii} and $\text{R3}^\angle_{0,1}$} \\
 = {}^{x_{\beta+\gamma}(b't^{e-1})}\![x_{\alpha+\beta+\gamma}(b't^{e-1}\xi), x_{-\beta}(t)] & \text{ by $\text{R2}_{0,e-1}$ and $\text{R3}^\angle_{e-1,e-1}$} \\
 = 1 & \text{ by $\text{R3}^\bot_{e-1,1}$.}
\end{align*}

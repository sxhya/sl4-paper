...
\begin{prop} The operators $T_\alpha(aX^{-1})$ satisfy all relations $\text{R2}_{0,1}$ and relations $\text{R3}_{1,1}$ in the special case when $\alpha, \beta$ form a sharp angle. \end{prop}
\begin{proof} 
\begin{comment}
...
\end{comment}
Let us verify $\text{R2}_1$. Let $\alpha, \beta$ be a pair of roots such that $\alpha + \beta \in \Phi$.
...
\end{multlined}
\end{gather*}
The expressions in the right-hand sides of the above formulae are clearly symmetric under the action of the transposition $(a, \alpha) \leftrightarrow (b, \beta)$,
 which implies the relation $\text{R3}_{1,1}$.
\end{proof}
...
\begin{lemma} For any simply laced root system $\Phi$ of rank $\geq 3$, a local ring $A$ and any $1 \leq m \leq n$ the relation $\text{R3}_{n,m}^\perp$ follows from $\text{R1}_n$, $\text{R2}_{n,0}$, $\text{R2}_{m,0}$ and $\text{R2}_{n-m,m}$ and $\text{R3}_{n-m, 0}^\perp$. \end{lemma}
\begin{proof}
...
Now if $c \in M$ we can express it as a difference $c = \epsilon_1 - \epsilon_2$ of two invertible elements. The required assertion now follows from $\text{R1}_{n}$, the standard commutator identity $[ab, c] = {}^a[b, c] [a, c]$ and the special case considered above.
\end{proof}

\begin{comment}
\begin{corollary}
  $T_\alpha$ satisfy the relations $\text{R3}_{1,1}$ in the remaining case when $\alpha$ is perpendicular to $\beta$.
\end{corollary}

...
The solution of the famous Serre problem on projective modules by D.~Quillen and A.~Suslin was one of the brilliant achievements of mathematics of the 70's, which bolstered the development of modern algebraic K-theory. In its weakest form Serre problem asks whether any projective module over a polynomial ring $R = k[x_1,\ldots, x_n]$ over a field $k$ is free. We refer the reader to the textbook~\cite{Lam10} for a comprehensive account on the problem, its history and the subsequent affirmative solution.

After the original Serre problem had been solved, numerous analogous statements have drawn the attention of specialists. For example, A.~Suslin formulated and solved the so-called $K_1$-analogue of Serre's problem. This result asserts that the functor $\K_{1,n}(R) = \mathrm{GL}_{n}(R)/\E_n(R)$ has trivial values on polynomial rings $k[x_1, \ldots x_n]$ over fields for all $n \geq 3$ see~\cite[Corollary~7.11]{Su77}. Suslin's results were subsequently generalized to $\K_1$-functors modeled on other linear groups (see the definition below). For example, for even-dimensional orthogonal groups the corresponding result was obtained by A.~Suslin and V. Kopeiko in~\cite{Su82}, while for more general types of Chevalley groups of rank $\geq 2$ this is a result of E.~Abe, see~\cite{Abe83}. Recently, A.~Stavrova has obtained probably the most general result in this direction: she solved the analogue of Serre problem for the functor $\K_1^G$ modeled on arbitary isotropic reductive group scheme $G/k$ of isotropic rank $\geq 2$ (see~\cite[Theorem~1.2]{St-poly}).
...
One might ask whether any analogue of Serre's problem also exists for the functor $\K_2$ and it turns out that it is indeed so. More precisely, in~\cite{Tu83} M. Tulenbaev demonstrates an ``early stability theorem'' from which the isomorphism $\K_2(\rA_\ell, k[x_1, \ldots x_n]) \cong \K_2(\rA_\ell, k) = \K^M_2(k)$ immediately follows for $\ell \geq 4$. While a number of new results on the $\K_1$-analogue of Serre's problem have appeared in the literature since~\cite{Su77} (see e.\,g.~\cite{Abe83, St-poly}), little progress has been made for the $\K_2$-analogue since~\cite{Tu83}. Currently the conjecture still remains open for all $\Phi$ different from $\rA_\ell$, $\ell \geq 4$.

In~\cite{LS17} the authors have shown that Steinberg groups $\St(\Phi, R)$ satisfy Quillen--Suslin local-global principle provided $\Phi$ is simply-laced and has rank $\geq 3$. This result is one of the ingredients needed in the proof of the $\K_2$-analogue of Serre problem. The aim of the present article is to make yet another step towards the solution of the problem. Our main result is the following theorem, which is the orthogonal analogue of~\cite[Theorem~5.1]{Tu83} and the $\K_2$-analogue of~\cite[Theorem~6.8]{Su82} (cf. also with~\cite[Theorem~VI.5.2]{Lam10} and~\cite[Theorem~1.1]{St-poly}).
\begin{theorem}[Horrocks theorem for orthogonal $\K_2$]\label{thm:main} Suppose $A$ is a commutative ring in which $2$ is invertible. Then for any $\ell \geq 7$ the following commutative square is a pullback square in which all arrows are injective:
\begin{equation} \nonumber
  \xymatrix{ \St(\rD_\ell, A) \ar[r] \ar[d] & \St(\rD_\ell, A[X]) \ar[d] \\ \St(\rD_\ell, A[X\inv]) \ar[r] & \St(\rD_\ell, A[X, X\inv]).}  
\end{equation} 
Moreover, the same assertion remains true if one replaces the functor $\St(\rD_\ell, -)$ with $\K_2(\rD_\ell, -)$. \end{theorem}

Our proof of~\cref{thm:main} goes as follows. After invoking the local-global principle~\cite[Theorem~2]{LS17} the proof reduces to the special case when $A$ is local. Now if $M$ is the maximal ideal of $A$, the proof of the injectivity of $\St(\rD_\ell, A[X]) \to \St(\rD_\ell, A[X, X\inv])$ comes down to proving injectivity of the following two maps
\[ \xymatrix{ \St(\rD_\ell, A[X]) \ar[r] & \St(\rD_\ell, A[X\inv] + M[X]) \ar[r] & \St(\rD_\ell, A[X, X\inv].) }\]
The injectivity of the second map is obtained in~\cref{thm41}. This is the only place in our proof which invokes the assumption that $2$ is invertible. The proof of~\cref{thm41} depends on certain computations with Grothendieck--Witt groups and also on the stability theorem for the higher orthogonal K-groups. The injectivity of the first map is obtained in~\cref{thm:P1glueing}, which is a direct generalization of~\cite[Proposition~4.3]{Tu83}. %In the proof of~\cref{thm:P1glueing} we also write explicitly many of the details left implicit in Tulenbaev's paper.

\subsection{Acknowledgements} Theorem 2 is obtained by the first-named author who acknowledges the financial support of RSCF grant No ???.
Theorem 3 is obtained by the second-named author who acknowledges the financial support of JSC Gazpromneft and RFBR grant No 18-31-20044
...
\[ H(\Phi, k) = \left\{\begin{array}{ll} \K_1^\mathrm{MW}(k)& \text{if $\Phi$ is symplectic,}\\ \K_1(k) \cong k^* & \text{otherwise.}  \end{array}\right. \]  \end{lemma}
...
Recall that this theory, developed by M.~Schlichting, is a modern broad generalization of the classical hermitian K-theory of rings (see e.\,g.~\cite{Ka80}). We refer the reader to~\cite[\S~2]{FRS12} and~\cite[\S~2]{AF17} for an introduction to Grothendieck--Witt groups.
...
\begin{proof}[Proof of~\cref{thm:main}]
 The assertion of the theorem follows from~\cref{thm41} and~\cref{thm:P1glueing} in the special case when $A$ is a local ring.

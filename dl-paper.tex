...
Notice that a priori $K(\alpha, \beta)$ depends on a choice of $\delta$, however as the next proposition shows, there is no such dependence.
...
Then for any $\alpha \in \Phi$ the partial function $t_\alpha(aX^{-1}) \colon V \not \to V$ gives rise to a well-defined total orbit function $T_\alpha(aX^{-1}) \colon \overline{V} \to \overline{V}$. \end{lemma}
\begin{proof}
...
The following proposition is directly inspired by~\cite[Proposition~4.3]{Tu83}.
\begin{prop} The operators $T_\alpha(aX^{-1})$ satisfy all relations $\text{R1}_1, $ $\text{R2}_1$ and relations $\text{R3}_{1,1}$ in the special case when $\alpha, \beta$ form a sharp angle.
\end{prop}
\begin{proof}
...
Let us verify $\text{R2}_1$. Let $\alpha, \beta$ be a pair of roots such that $\alpha + \beta \in \Phi$. Notice that we could assume from the start that $p(g, h, u) \in j_+(K(A[X], M[X]) \cap P_\alpha(m) \cap P_{\alpha + \beta}(m'))$ for some $m,m' \in M$ (cf. the first part of the proof of~\cref{lem:orbit-action}). Now for $a, b \in A$ set $\epsilon = N_{\alpha, \beta}$ and $c = 1 - \epsilon abm'$.
Applying~\cref{SR:obtuse} to a pair of roots $\alpha, \alpha+\beta$ we obtain that 
\begin{multline} \nonumber
[T_\beta(b), T_\alpha(aX^{-1})] \left(g, h, u \right) = \\
 = T_\beta(b) T_\alpha(aX^{-1}) \left( x_\beta(-b) \cdot x_\alpha (-a X^{-1}) \cdot g, h \cdot x_\alpha\left(\tfrac{aX^{-1}}{1-am}\right) , u (1 - am) \right) \sim \\ \sim T_\beta(b) \cdot T_\alpha(aX^{-1}) \left( x_\beta(-b) \cdot x_\alpha (-a X^{-1}) g, h \cdot x_\alpha\left(\tfrac{a X^{-1}}{1-am}\right) \cdot h_{\alpha}^{-1}(1-am)\cdot h_{\beta}(c^{-1})\cdot x_{\beta}(bc), u (1 - am) \right) = \\ = \left([x_\beta(b), x_\alpha(aX^{-1})] g, h', u c\right) \sim T_{\alpha+\beta}(-\epsilon abX^{-1})(g, h, u),\end{multline} 
 where the last equivalence is obtained as follows:
 \begin{multline} \nonumber
 h' = h\cdot x_\alpha\left(\tfrac{a X^{-1}}{1-am}\right) \cdot h_{\alpha}^{-1}(1-am)\cdot h_{\beta}(c^{-1})\cdot x_{\beta}(bc)\cdot x_\alpha\left(-\tfrac{a(1 - am)X^{-1}}{c}\right) \sim \\ \sim  h\cdot x_\alpha\left(\tfrac{a X^{-1}}{1-am}\right) \cdot h_{\alpha}^{-1}(1-am)\cdot h_{\beta}(c^{-1})\cdot x_{\alpha + \beta}\left(\epsilon ab(1-am)X^{-1}\right)\cdot x_\alpha\left(-\tfrac{a(1 - am)X^{-1}}{c}\right) = \\ = h\cdot x_\alpha\left(\tfrac{a X^{-1}}{1-am}\right) \cdot x_{\alpha + \beta}\left(\tfrac{\epsilon \cdot abX^{-1}}{c}\right)\cdot x_\alpha\left(-\tfrac{aX^{-1}}{1 - am}\right) \cdot h_{\alpha}^{-1}(1-am)\cdot h_{\beta}(c^{-1}) \sim \\ \sim h\cdot x_{\alpha + \beta}\left(\tfrac{\epsilon   abX^{-1}}{1 - \epsilon a b m'}\right).
\end{multline}

Let us prove $\text{R3}_{1,1}$ in the special case when $\alpha$ and $\beta$ form a sharp angle. As before, we can assume $p(g, h, u) \in j_+(K(A[X], M[X]) \cap P_\alpha(m) \cap P_{\beta}(m'))$ for some $m, m' \in M$. Set $\epsilon = N_{\alpha,-\beta}$.
Applying~\cref{SR:sharp} we obtain that
...
\end{multline}
The expressions in the right-hand sides of~\eqref{eq:sharp-formula1}--\eqref{eq:sharp-formula2} are clearly symmetric under transposition $(a, \alpha) \leftrightarrow (b, \beta)$, which implies $\text{R3}_{1,1}$.
\end{proof}

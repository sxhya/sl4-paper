...
We denote by $U(\Phi^+, A)$ (resp. $U(\Phi^-, A)$) the subgroup of $\St(\Phi, R)$ generated by root subgroups $X_\alpha(A)$ corresponding to all positive (resp. negative) roots of $\Phi$.
...
Recall that a closed subset $S \subseteq \Phi$ is called {\it parabolic} if $S \cup -S = \Phi$.
Its {\it special part} $\Sigma_S$, by definition, consists of all $\alpha \in S$ such that $-\alpha \not\in S$.
...
\end{lemma}
\begin{proof}
First of all, notice that the following equalities hold for elements $x,y,z$ of any group:
\begin{align}
...
\begin{proof} From~\eqref{eq:conj-h-x} we obtain that $\overline{H}(\Phi, A, M)$ normalizes both $\St(\Phi, XA[X])$ and $\St(\Phi, M)$ and, moreover, that $[\overline{H}(\Phi, A, M),\ \St(\Phi, XA[X])] \subseteq \overline{\St}(\Phi, A[X], XM[X])$. 

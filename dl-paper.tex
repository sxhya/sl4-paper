...
Let $M$ be an ideal of a local ring $A$. Denote by $\widetilde{P}_{\alpha, M}(0)$ the subgroup of $\overline{\St}(\Phi, A[X], M[X])$ generated by the following five families of elements:
\begin{enumerate} 
 \item $z_{\beta}(Xf, \xi),\ \beta \in \Phi \text{ such that }\alpha + \beta \in \Phi\text{ or } \alpha - \beta \in \Phi,\ f \in M[X],\ \xi \in A[X];$
 \item $z_{\beta}(f, X\xi),\ \alpha - \beta \in \Phi,\ f\in M[X],\ \xi\in A[X];$
 \item $z_{\beta}(f, \xi),\ \beta \perp \alpha,\ f\in M[X],\ \xi\in A[X];$
 \item $x_{-\alpha}(X^2f),\ f \in M[X];$
 \item $x_{\alpha}(f),\ f \in M[X]$. \end{enumerate}
In the sequel we continue to denote the elements of $M[X]$ (resp. $A[X]$) by $f$ (resp. $\xi$).
Notice also that by~\eqref{Zdef} one has $z_\beta(f, 0) = x_\beta(f)$.
...
\end{df}
%We denote by $P_{\alpha, M}(0)$ the image of $\widetilde{P}_{\alpha, M}(0)$ under the map $j_+ \colon \St(\Phi, A[X]) \to \St(\Phi, A[X, X^{-1}])$. 
...
\[\xymatrix{1 \ar[r] & \widetilde{P}_\alpha(0) \ar[r] & \widetilde{P}_\alpha(*) \ar@{->>}[r]^{p_\alpha} & (M, +) \ar@<5pt>@{-->}[l] \ar[r] & 1}.\] \end{lemma}
\begin{proof} We need to verify that the conjugate by $x_{-\alpha}(mX)$ to every generator $g$ of $\widetilde{P}_\alpha(0)$ listed in~Definition~\ref{defP0} belongs to $\widetilde{P}_\alpha(0)$.
The assertion is obvious for the generators of type 3 and 4.

Let $g = z_\beta(Xf, \xi)$ be a generator of type 1 such that $\alpha - \beta \in \Phi$. By~\cref{Zrels}\eqref{Z2} we get that \begin{equation} z_{\beta}(Xf, \xi) ^ {x_{-\alpha}(mX)} =  x_{-\alpha} (- mX^2f\xi) \cdot x_{\beta-\alpha} (N_{\beta, -\alpha}\cdot mX^2f) \cdot z_{\beta}(Xf, \xi). \label{eq3-1} \end{equation}
The first and the last factors in the right-hand side of the above equation are generators of type $4$ and $1$, respectively. It is easy to see that the second factor is also a generator of type 1, indeed:
\[ x_{\beta - \alpha} (N_{\beta,-\alpha} \cdot mX^2 f) = z_{\beta'}(Xf', 0)\text{ for } f' = N_{\beta,-\alpha} \cdot mX f\text{ and }\beta' = \beta - \alpha \text{ (clearly, $\alpha + \beta' \in \Phi$).}\]
Now consider the case when $g = z_\beta(f, X\xi)$ is a generator of type $2$. Again by~\cref{Zrels}\eqref{Z2}
\begin{equation}
...
Finally, let $g = x_\alpha(f)$ be a generator of type $5$. Substituting in~\eqref{Z5} $s = -\epsilon f$, $\xi = -\epsilon m$, $\eta=X$, $\epsilon = N_{\alpha, \beta}$ and expressing the fifth factor in the right-hand side of~\eqref{Z5} through other terms we obtain that
...
\begin{rem}\label{rem:c-DS} By~\eqref{Cdef} one has $c_\beta(f, X\xi) = x_{\beta} (f) \cdot z_{\beta}(-f, -X\xi)$. This implies that for $\beta \in Z_+(\alpha)$ (resp. $\beta \in Z_0(\alpha)$) the element $c_\beta(f, X\xi)$ is a product of two generators of type 2 (resp. 3).
Thus, the elements $c_{\beta}(f, X\xi)$ lie in $\widetilde{P}_{\alpha, M}(0)$ for all $\beta \in Z(\alpha)$.
...
In particular, if $M$ is the maximal ideal of $A$, the subgroup $\widetilde{P}_{\alpha, M}(0)$ contains relative Dennis--Stein and Steinberg symbols. It is also easy to see that, in fact, the image of $\widetilde{P}_{\alpha, M}(0)$ under the homomorphism $ev_{X=0}^*$ coincides with $Z_\alpha(A, M)$. \end{rem}
...
\begin{proof} Clearly, $\widetilde{P}_{\alpha, M}(*)$ contains the elements $x_\beta(Xf)$ for all $\beta \in \Phi$ and $z_\beta(Xf, \xi)$ for $\beta \in \Phi \setminus \{\pm \alpha\}$, $f \in M[X]$, $\xi\in A[X]$, therefore by~\cref{thm:Stepanov} $\widetilde{P}_{\alpha, M}(*)$ contains all of $\overline{\St}(\Phi, A[X], XM[X])$. The required assertion now follows from~\cref{Kgen-strong} and~\cref{rem:c-DS}. \end{proof}

\begin{rem} \label{Pstar-char} 
By the above lemma and~\cref{rem:c-DS} the subgroup $\widetilde{P}_{\alpha, M}(*)$ coincides with the preimage of the subgroup $Z_\alpha(A, M) \leq \overline{\St}(\Phi, A, M)$ under the homomorphism
 \[ev_{X=0}^*\colon \overline{\St}(\Phi, A[X], M[X]) \to \overline{\St}(\Phi, A, M).\] \end{rem} 
...
To obtain the assertion for $P_{\alpha, M}(0)$ consider the following commutative diagram. \[ \xymatrix{ \widetilde{P}_{\alpha, M}(0) \ar@{^{(}->}[r] \ar@{^{(}->}[d] & \widetilde{P}_{\alpha, M}(*) \ar@{->>}[r]^{p_{\alpha, M}} \ar@{^{(}->}[d] & (M, +) \ar@{^{(}->}[d] \\ \widetilde{P}_{\alpha, A}(0) \ar@{^{(}->}[r] & \widetilde{P}_{\alpha, A}(*) \ar@{->>}[r]^{p_{\alpha, A}} & (A, +)} \]
...
\begin{df} 
We define the following two subgroups of $\overline{\St}(\Phi, R, M[X, X^{-1}])$:
\[ P_{\alpha}(0) = j_+(\widetilde{P}_{\alpha, M}(0)), \ P_{\alpha}(*) = j_+(\widetilde{P}_{\alpha, M}(*)), \]
where $j_+ \colon \overline{\St}(\Phi, A[X], M[X]) \to \overline{\St}(\Phi, R, M[X, X^{-1}]$ is the obvious homomorphism.
For $m \in M$ denote by $P_\alpha(m)$ the coset $P_\alpha(0) \cdot x_{-\alpha}(mX)$.

From~\cref{P0_normal} it follows that $P_\alpha(*)$ coincides with the union of all $P_\alpha(m)$, $m\in M$. This allows to define the map $S_\alpha(a, -) \colon P_\alpha(*) \to \St(\Phi, A[X, X^{-1}])$ on each coset $P_\alpha(m)$, $m \in M$ via the following formula:
...
\end{lemma} \begin{proof} Since $g_2\in P_\alpha(m')$ for some $m' \in M$, we have that $g_1 \cdot g_2 \in P_\alpha(m + m')$. The assertion now follows from the definition of $S_\alpha(a, -)$:
\begin{multline*} S_\alpha(a, g_1 \cdot g_2) = x_\alpha(aX^{-1}) \cdot g_1 \cdot g_2 \cdot x_\alpha\left(-\tfrac{aX^{-1}}{1+am+am'}\right) \cdot \{X, 1 + am + am'\} = \\ = x_\alpha(aX^{-1}) \cdot g_1 \cdot x_\alpha\left(-\tfrac{aX^{-1}}{1+am}\right) \cdot x_\alpha\left(\tfrac{aX^{-1}}{1+am}\right) \cdot g_2 \cdot x_\alpha\left(-\frac{\tfrac{a}{1+am}\cdot X^{-1}}{1 + \tfrac{a}{1+am}\cdot m'}\right) \{X, (1+am)(1 + \tfrac{am'}{1+am})\} = \\ = S_\alpha(a, g_1) \cdot S_\alpha\left(\tfrac{a}{1+am}, g_2\right). \qedhere \end{multline*}
\end{proof}
...
\begin{proof} 
We need to verify that the conjugate by $x_\alpha(aX^{-1})$ to every generator $g$ from~\cref{defP0} lies in $P_\alpha(0)$. 
The assertion is clear for generators of type 3 and 5.
First of all, consider the case $\alpha + \beta \in \Phi$, which is only possible in the case of a generator of type 1:
\begin{equation}\label{eq:Zconj1} z_{\beta}(Xf, \xi) ^ {x_{\alpha}(aX^{-1})} = x_{\alpha} (- af\xi) \cdot x_{\alpha+\beta} (N_{\beta, \alpha}\cdot af) \cdot z_{\beta}(Xf, \xi). \end{equation}
Clearly, the expression in the right-hand side is a product of generators of type 5, 2, 1. 

Now consider the case $\alpha - \beta \in \Phi$, which is possible for generators of type 1 and 2. By~\cref{Zrels} we obtain that
...
Let us verify the assertion for a generator of type 4.
...
Now by~\cref{lem:Smult} and~\cref{P0_conj} one has
...
\begin{df}\label{def:Kab} Denote by $\widetilde{K}(\alpha, \beta)$ the subgroup of $\overline{\St}(\Phi, A[X], M[X])$ generated by the following 5 families of elements parametrized by $\xi \in A[X]$, $f\in M[X]$:
 \begin{enumerate}
  ...
  \item $c_{\delta}(f, X\xi)$ for some fixed root $\delta \in \Phi \setminus \Psi$. \end{enumerate}
...
By~\cref{thm:Stepanov} every element of $\St(\Phi, A[X], XM[X])$ can be presented as a product of generators $\mathcal{G}_1$ and generators $\mathcal{G}' = \{ x_{\gamma}(Xf) \mid \gamma \in \Psi\}$. In turn, by~\cref{Kgen-strong} one can express $g$ as a product of these generators and generators $\mathcal{G}_5$. 

By~\cref{thm:Stepanov} $\widetilde{K}(\alpha, \beta)$ contains normal subgroup $G_0 = \overline{\St}(\Phi, A[X], X^2M[X])$, so it suffices to obtain the required presentation of $g$ modulo $G_0$. By~\cref{Zrels} and~\cref{Crels} the elements of $\mathcal{G}'$ commute with the elements of $\mathcal{G}_1$ and $\mathcal{G}_5$ modulo $G_0$, therefore we can rewrite $g$ as $g_1 \cdot g_2$, for some $g_1 \in \langle \mathcal{G}_1, \mathcal{G}_5, G_0 \rangle \subseteq \widetilde{K}(\alpha, \beta)$ and $g_2 \in \langle \mathcal{G}' \rangle$.

It is clear that  $x_{\gamma_1}(mX)$ and $x_{\gamma_2}(m'X)$ commute modulo $G_0$ whenever $\gamma_1 \neq - \gamma_2$, $\gamma_1, \gamma_2 \in \Psi$. On the other hand, for $\gamma \in \Psi$ any commutator $c_{\gamma}(mX, m'X)$ is congruent to some generator of $\mathcal{G}_5$  modulo $G_0$ (cf. the proof of~\cref{Kgen-strong}) and hence commutes with the elements of $\mathcal{G}'$ modulo $G_0$. 

Now pick each factor $x_{\gamma}(Xf)$, $\gamma \in \{ -\alpha, -\beta \}$ appearing in the presentation for $g_2$, decompose it as $x_{\gamma}(X^2f') \cdot x_{\gamma}(m'X)$ for some $f', m'$ and then move the second factor to the rightmost position within $g_2$ conjugating all factors along the way. By the previous paragraph only elements of  $\mathcal{G}_5$ may appear modulo $G_0$ after simplification of these conjugates.
...
It is clear from the definition that $K(\alpha, \beta) \subseteq P_\alpha(0) \cap P_\beta(0) \cap j_+(K(A[X], M[X]))$. The reverse inclusion follows from the just proved decomposition and the fact that for $g\in j_+(K(A[X], M[X]))$ holds $p_{\alpha}(g) = m$ and $p_\beta(g) = m'$ (cf.~\cref{rem:palpha}). \end{proof}

...
\subsection{Presentation by Homogeneous elements}
...
modulo relations $\text{R}1_{d}$, $\text{R3}^\angle_{d, e}$, $\text{R3}_{d,e}^\bot$ for all $d,e \leq n$ and $\text{R2}_{d,e}$ for $d,e,d+e \leq n$.
...
\begin{lemma}\label{superfluous-relations}
 One can exclude from the presentation of $\St^{\leq n}(\Phi, B)$ all the relators $\text{R3}_{d,e}^\bot$ for which $\max(0,d) + \max(0,e) > 1$.
\end{lemma}
\begin{proof}
...
In the case $d \leq 0 \leq e \leq n$ for $\xi \in B_d$ one has
\begin{align*} 

...
We retain notation of the previous subsection and assume additionally that the map $\St(\Phi, B) \to \St(\Phi, R)$ is injective. This assumption allows us to consider $\St(\Phi, B)$ as a subgroup of $\St(\Phi, R)$.
...
\begin{theorem}
 If $\Phi$ is as in the statement of~\cref{lemma33}, then the map $j_-$ is injective.
 If, moreover, $\St(\Phi, A[X]) \to \St(\Phi, B)$ is injective, then the following square is pullback
 \begin{equation} \nonumber
  \xymatrix{ \St(\Phi, A) \ar[r] \ar[d] & \St(\Phi, A[X]) \ar[d]^{j_+} \\ \St(\Phi, A[X\inv]) \ar[r]^{j_-} & \St(\Phi, R).}
 \end{equation}
\end{theorem}
\begin{proof}
 Let us show that $j_-$ is injective. Set $v_0 = (1, [1], 1) \in \overline{V}$ and suppose $g \in \Ker(j_-)$.
 By~\cref{prop43} holds $v_0 = j_-(g) \cdot v_0 = (g, [g^{-1}], u)$ therefore $g \in G_M^0$ and consequently $g=1$.
 
 Now suppose $g_+ \in \St(\Phi, A[X])$ and $g_- \in \St(\Phi, A[X\inv])$ are such that $j_-(g_-) = j_+(g_-)$. From~\cref{field-injectivity} it follows that the image $j_+(g_+) = j_-(g_-)$ in $\St(\Phi, k[X, X\inv])$ belongs to $\St(\Phi, k)$ and therefore coincides with the image in $\St(\Phi, k)$ of some $g_0 \in \St(\Phi, A)$. Dividing $g_+$ and $g_-$ by $g_0$ we can achieve
\end{proof}

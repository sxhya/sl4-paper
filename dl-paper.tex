...
The main result of this section is~\cref{thm:P1glueing}, which is a direct generalization of~\cite[Proposition~4.3]{Tu83}. The proof of this theorem is rather long and technical, so before setting out technical details, we briefly describe the main steps of this proof.

The object playing a key role in the proof of~\cref{thm:P1glueing} is a certain action of the group $G = \St(\Phi, A[X\inv] + M[X])$ on a certain set $\overline{V}$. To be able to write an explicit formula for this action we need two major ingredients. The first one is a presentation of $G$, which involves much fewer generations and relations. We obtain this presentation in~\cref{lemma33}. The other ingredient are the subgroups $P_\alpha(0)$ of $\St(\Phi, A[X, X\inv])$, whose definition and properties are given in Sections~\ref{sec:Pa0-basic}--\ref{sec:Pa0-Steinberg}.

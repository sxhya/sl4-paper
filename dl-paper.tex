...
Recall that to every irreducible root system $\Phi$ and a commutative ring $R$ one can associate two groups: the {\it simply-connected Chevalley group} $\GG(\Phi, R)$ (see e.\,g.~\cite[\S~3]{St71} or~\cite{VP}) and the {\it Steinberg group} $\St(\Phi, R)$ (see~\cref{sec:Steinberg-intro} for the definition). There is a well-defined map $\pi \colon \St(\Phi, R) \to \GG(\Phi, R)$ sending each generator $x_\alpha(\xi)$ to the elementary root unipotent $t_\alpha(\xi)$. The cokernel and the kernel of this map are, by definition, the $\K_1(\Phi, R)$ and $\K_2(\Phi, R)$-functors modeled on the Chevalley group $\GG(\Phi, -)$, see~\cite{St78}.
...
\begin{itemize}
 ...
 \item $B = B_{A, M}$ denotes the subring $A[t] + M[t^{-1}] = A[X^{-1}] + M[X]$ of $R$.
...
Denote by $\overline{G}^{\geq 0}_M$ the image of the map $j_+^M$. \end{df}
Notice that the map $j_+^Mi_+^M$ is split by the homomorphism $ev^*_{X=a}$ of evaluation at $a\neq 0$, therefore $G_M^0$ can be considered as a subgroup of $\overline{G}^{\geq 0}_M$.
...
\begin{proof}
 ...
 Fix $v_0 = (g, h, u) \in V$. Since $p(g, h, u) \in \overline{G}^{\geq 0}_M$ there exists $g_1 \in \overline{\St}(\Phi, A[X], M[X])$ such that $j_+(g_1) = p(g, h, u).$ Notice that the map $i_+$ is split by $ev^*_{X=0}$.
...
\begin{enumerate}
 \item For any $h_1 \in \St(\Phi, A[X\inv])$ one has $j_B^-(h_1) \cdot [1, h, u] = [1, h_1 h, u]$, where $j_B^-$ denotes the map $\St(\Phi, A[X\inv]) \to \St(\Phi, B)$ (we identify $\overline{V}$ with $\overline{V}_T$ using the isomorphism~\eqref{eq:VVT}).
...
the image of the relative Steinberg group $\overline{\St}(\Phi, R, I)$ under the map $\pi \colon \St(\Phi, R) \to \GG(\Phi, R)$.
...
\begin{proof} Since $j_- = j_R j_B^-$, where $j_B^-$ is as in~\cref{prop43}, it suffices to show that $j_B^-$ is injective.
 ...
 Notice that $j_+i_+$ is split by the evaluation homomorphism $ev_{X=a}^*$, $a\neq 0$ and therefore is injective. Thus, we conclude that $g = 1$.

...
\begin{lemma}\label{lem:orbit-action} Let $\Phi$ be a simply-laced root system of rank $\geq 3$ and $(A, M)$ be a local pair. 
...
\begin{prop}
 The operators $T_\alpha(aX^{-1})$ satisfy relations $\text{R1}_1, $ $\text{R2}_1$.
\end{prop}
\begin{proof}
...
Invoking~\cref{SR:additivity} once again we obtain that
...
which implies the assertion for $\text{R1}_1$.

Now let us verify $\text{R2}_1$. Let $\alpha, \beta$ be a pair of roots such that $\alpha + \beta \in \Phi$. 
Notice that we could assume from the start that $p(g, h, u) \in j_+(K(A[X], M[X]) \cap P_\alpha(m) \cap P_{\alpha + \beta}(m'))$ for some $m,m' \in M$ (cf. the first part of the proof of~\cref{lem:orbit-action}). Now for $a, b \in A$ set $\epsilon = N_{?, ?}$ and $c = 1 + \epsilon abm'$.
It is clear that $\alpha$ and $\alpha+\beta$ form a sharp angle.
\begin{multline} \nonumber
[T_\alpha(a), T_\beta(bX^{-1})] \left( g, h, u \right) =
 ...
 \begin{multline} \nonumber
 h' = h\cdot x_\beta\left(\tfrac{b X^{-1}}{1-bm}\right) \cdot h_{\beta}^{-1}(1-bm)\cdot h_{\alpha}^{-1}(1 + abm')\cdot x_{\alpha}(a(1 + abm'))\cdot x_\beta\left(-\tfrac{b(1 - bm)X^{-1}}{1 + abm'}\right) \sim \\ \sim  h\cdot x_\beta\left(\tfrac{b X^{-1}}{1-bm}\right) \cdot h_{\beta}^{-1}(1-bm)\cdot h_{\alpha}^{-1}(1 + abm')\cdot x_{\alpha + \beta}\left(-N_{\alpha, \beta}\cdot ab(1-bm)X^{-1}\right)\cdot x_\beta\left(-\tfrac{b(1 - bm)X^{-1}}{1 + abm'}\right) = \\

...
The objects of $\catname{Pairs}$ are pairs $(R, I)$, in which $R$ is a commutative ring and $I$ is an ideal of $R$.
...
It turns out that the set  $Ext(Q, N; L)$  of isomoprhism classes of relative central extensions of $\nu$ by an abelian group $L$ can be classified by means of a certain cohomological invariant called {\it characteristic class}. More precisely, \cite[Th{\'e}or{\`e}me~1]{Lo78} asserts that there is a well-defined bijection $\xi \colon Ext(Q, N; L) \to \HH^3(Q, N; L)$.
 
For the rest of this subsection $S \xrightarrow{\varphi} P \subseteq G$ is a triple of group-valued functors on the category of commutative rings satisfying the following assumptions:
\begin{enumerate} [label=(A\arabic*)]
 \item \label{req:left-exact} $G(D_{R, I}) \cong G(R) \times_{G(R/I)} G(R)$.
 \item \label{req:subfunc} For every $R$ the group $P(R)$ is a perfect normal subgroup of $G(R)$.
 \item \label{req:uce} For every $R$ the map $ \varphi_R \colon S(R) \to P(R)$ is a universal central extension.
...
\end{comment}
\begin{lemma}
 Suppose  for every pair $(R, I)$ the map $\mu \colon S(R, I) \to S(R)$ is the universal relative central extension of $\nu \colon S(R) \to S(R/I)$, i.\,e. the initial object of the category of relative central extensions of $\nu$. 
 Moreover, $C_S(R, I)$ is naturally isomorphic to the relative homology group $\HH_3(S(R), S(R/I), \ZZ)$.
\end{lemma}
\begin{proof}
